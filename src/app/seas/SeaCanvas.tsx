...
/**
 * Cinematic sea — canvas port of the Claude Design hero background
 * (design-exports/Hero/hero-bg.js), wrapped as a client component with
 * proper cleanup. Palette: deep navy, emerald sea, gold sunlight,
 * lantern orange, storm gray. Honors prefers-reduced-motion (single
 * static frame) and pauses while the tab is hidden.
 */
...
type BattleShip = { x: number; y: number; s: number; ph: number };
...
export default function SeaCanvas() {
  ...
  useEffect(() => {
    ...
    const onResize = () => {
      resize();
      if (reduced || document.hidden) drawFrame(3);
    };
    ...
    /* ---------- static scene data ---------- */
    const HOR = 0.6; /* horizon as fraction of height */

    const islands: Island[] = [
      { x: 0.5, w: 0.085, h: 0.022, tone: "rgba(16,52,60,0.55)", seed: 11 },
    ...
    const ships: { x: number; y: number; s: number; v: number; sway: number }[] = [];
    (function () {
      const r = rng(99);
      for (let i = 0; i < 5; i++) {
        ships.push({
    ...
    const embers: { x: number; y: number; s: number; vx: number; vy: number; ph: number; sp: number }[] = [];
    (function () {
      const r = rng(55);
      for (let i = 0; i < 36; i++) {
        embers.push({
    ...
    const flash = { t0: -10, x: 0.2, life: 0.7, next: 2 };
    ...
    function sky(t: number) {
      ...
      const sg = ctx.createRadialGradient(W * 0.7, hor, 4, W * 0.7, hor, W * 0.42 * pulse);
      ...
      const hb = ctx.createLinearGradient(0, hor - H * 0.06, 0, hor);
      hb.addColorStop(0, "rgba(255,170,90,0)");
      hb.addColorStop(1, "rgba(255,170,90,0.14)");
      ctx.fillStyle = hb;
      ctx.fillRect(W * 0.3, hor - H * 0.06, W * 0.7, H * 0.06);

      const drift = (t * 0.0012) % 0.08;
      for (let i = 0; i < 4; i++) {
        const cx = W * (0.78 + i * 0.09 - drift);
        const cy = H * (0.1 + i * 0.045);
        const cr = W * (0.16 + i * 0.02);
    ...
    function water(t: number) {
      ...
      for (let i = 0; i < 26; i++) {
        const f = i / 26;
        const y = hor + 4 + f * (H - hor) * 0.8;
        const width = W * (0.05 + f * 0.16);
        const sw = Math.sin(t * 0.9 + i * 1.7) * width * 0.18;
        const a = (1 - f) * 0.12 * (0.65 + 0.35 * Math.sin(t * 1.3 + i * 2.1));
        if (a <= 0.004) continue;
        ctx.fillStyle = "rgba(255,185,100," + a.toFixed(3) + ")";
        const lw = width * (0.5 + 0.5 * Math.sin(t * 0.6 + i));
        ctx.fillRect(W * 0.7 - lw / 2 + sw, y, lw, Math.max(1, (H - hor) * 0.004));
      }
      for (let j = 0; j < 9; j++) {
        const fy = hor + (((j / 9 + ((t * 0.004) % (1 / 9))) % 1) * (H - hor));
        const fa = 0.028 * (1 - Math.abs((fy - hor) / (H - hor) - 0.35));
        if (fa <= 0) continue;
        ctx.fillStyle = "rgba(140,220,220," + fa.toFixed(3) + ")";
        const lx = (Math.sin(j * 7.3 + t * 0.12) * 0.5 + 0.5) * W;
    ...
    function glow(x: number, y: number, r: number, fill: string) {
      if (!ctx) return;
    ...
    function islandShape(isl: Island, t: number) {
      if (!ctx || !isl.pts) return;
      const hor = H * HOR;
      const bw = isl.w * W;
      const bh = isl.h * H;
      const x0 = isl.x * W - bw / 2;
      ctx.beginPath();
      ctx.moveTo(x0, hor + 1);
      for (let i = 0; i < isl.pts.length; i++) {
        const f = i / (isl.pts.length - 1);
      ...
      if (isl.fires) {
        const fx = x0 + bw * 0.32;
        const fy = hor - isl.pts[4] * bh + 1;
        const fl = 0.6 + 0.4 * Math.sin(t * 7 + 1.3) * Math.sin(t * 3.1);
        glow(fx, fy, 7, "rgba(255,140,60," + (0.35 * fl).toFixed(3) + ")");
        dot(fx, fy, 1.1, "rgba(255,190,120," + (0.9 * fl).toFixed(3) + ")");
      }
      if (isl.lighthouse) {
        const lx = x0 + bw * 0.62;
        const topY = hor + 1 - isl.pts[8] * bh;
        ctx.fillStyle = "rgba(2,12,16,1)";
        ctx.fillRect(lx - 2, topY - 14, 4, 14);
        const ang = (t * 0.5) % (Math.PI * 2);
        const vis = Math.max(0, Math.sin(ang));
        const bx = lx;
        const by = topY - 15;
        ctx.save();
        ctx.translate(bx, by);
        ctx.rotate(Math.sin(ang) * 0.22);
        const bg = ctx.createLinearGradient(0, 0, W * 0.22, 0);
        bg.addColorStop(0, "rgba(255,220,150," + (0.2 * vis).toFixed(3) + ")");
        ...
        for (let hIx = 0; hIx < 5; hIx++) {
          const hx = x0 + bw * (0.3 + hIx * 0.09);
          const ha = 0.5 + 0.5 * Math.sin(t * 2 + hIx * 2.2);
          dot(hx, hor - 1.5, 0.9, "rgba(255,170,90," + (0.5 + 0.3 * ha).toFixed(3) + ")");
        }
      }
    }

    function drawShip(x: number, y: number, s: number, t: number, sway: number, big: boolean) {
      if (!ctx) return;
      const bob = Math.sin(t * 0.8 + sway) * s * 0.05;
      ctx.save();
      ctx.translate(x, y + bob);
      ctx.rotate(Math.sin(t * 0.5 + sway) * 0.015);
      const hull = "rgba(2,11,15,0.96)";
      const sail = "rgba(8,26,33,0.95)";
      ctx.beginPath();
      ctx.moveTo(-s, 0);
      ...
      ctx.fill();
      const la = 0.55 + 0.45 * Math.sin(t * 3 + sway * 3);
    ...
    function fleet(t: number) {
      if (!ctx) return;
      const hor = H * HOR;
      ships.forEach((sh) => {
        const x = ((((sh.x + t * sh.v) % 1.1) + 1.1) % 1.1) - 0.05;
        drawShip(x * W, hor + sh.y * H, sh.s, t, sh.sway, false);
      });

      const span = 1.5;
      const fx = ((((flagship.x + t * flagship.v) % span) + span) % span) - 0.25;
      const fy = hor + flagship.y * H;
      ctx.save();
      ctx.globalCompositeOperation = "lighter";
      const wl = W * 0.16;
      const wg = ctx.createLinearGradient(fx * W - wl, fy, fx * W, fy);
    ...
    function mist(t: number) {
      if (!ctx) return;
      ctx.save();
      mists.forEach((m) => {
        const x = ((((m.x + t * m.v) % 1.3) + 1.3) % 1.3) - 0.15;
        const a = m.a * (0.7 + 0.3 * Math.sin(t * 0.2 + m.ph));
        const g = ctx.createRadialGradient(x * W, m.y * H, 0, x * W, m.y * H, m.rx * W);
    ...
    function particles(t: number) {
      ...
      ctx.globalCompositeOperation = "lighter";
      embers.forEach((p) => {
        const x = (((p.x + t * p.vx) % 1) + 1) % 1;
        const y = (((p.y - t * p.vy * 0.4) % 1) + 1) % 1;
        const tw = 0.5 + 0.5 * Math.sin(t * p.sp * 2 + p.ph);
        ctx.fillStyle = "rgba(255,175,95," + (0.3 * tw).toFixed(3) + ")";
        ctx.fillRect(x * W, y * H, p.s, p.s);
      });
      ctx.restore();
    }

    function cannons(t: number) {
      if (t > flash.next) {
        flash.t0 = t;
        flash.x = 0.05 + ((t * 13.7) % 1) * 0.3;
        flash.next = t + 2 + ((t * 7.3) % 1) * 3;
      }
      const age = t - flash.t0;
      if (age >= 0 && age < flash.life) {
        const f = 1 - age / flash.life;
        const hor = H * HOR;
    ...
    /* ---------- broadside battle ---------- */
    function smokePuff(x: number, y: number, r: number, a: number) {
      if (!ctx) return;
    ...
    function battleShip(sh: BattleShip, dir: number, t: number) {
      if (!ctx) return;
      const hor = H * HOR;
      const sx = sh.x + Math.sin(t * 0.05) * 0.008 * dir;
      const x = sx * W;
      const y = hor + sh.y * H;
      const s = sh.s;
      ...
      const phase = (t + sh.ph) % FIRE_PERIOD;
      const gx = x + dir * s * 1.05;
      const gy = y - s * 0.16;

      if (phase < FLASH_LIFE) {
        const f = 1 - phase / FLASH_LIFE;
        ctx.save();
        ctx.globalCompositeOperation = "lighter";
        glow(gx, gy, s * 1.8 * f + 2, "rgba(255,190,110," + (0.7 * f).toFixed(3) + ")");
        dot(gx, gy, s * 0.16 * f + 0.5, "rgba(255,240,200," + (0.95 * f).toFixed(3) + ")");
        const sl = s * 3.2 * (1 - f * 0.4);
        const lg = ctx.createLinearGradient(gx, gy, gx + dir * sl, gy);
      ...
      const sAge = phase;
      if (sAge < SMOKE_LIFE) {
        const k = sAge / SMOKE_LIFE;
        smokePuff(gx + dir * s * (0.8 + k * 2.6), gy - k * s * 1.4, s * (0.5 + k * 1.8), 0.14 * (1 - k));
        smokePuff(gx + dir * s * (0.3 + k * 1.6), gy - k * s * 0.7, s * (0.3 + k * 1.2), 0.1 * (1 - k));
      }
    }

    function burningShip(t: number) {
      if (!ctx) return;
      const hor = H * HOR;
      const x = 0.595 * W;
      const y = hor + 0.046 * H;
      ...
      ctx.globalCompositeOperation = "lighter";
      for (let i = 0; i < 3; i++) {
        const fx = x + (i - 1) * s * 0.55;
        const fl = 0.55 + 0.45 * Math.sin(t * 9 + i * 2.4) * Math.sin(t * 3.7 + i);
      ...
      for (let k = 0; k < 4; k++) {
        const kf = (k + ((t * 0.25) % 1)) / 4;
        smokePuff(
          x - kf * s * 1.6 + sway * kf * s * 1.2,
          y - s * 0.6 - kf * s * 4.2,
          s * (0.5 + kf * 1.7),
          0.13 * (1 - kf),
        );
    ...
    function battle(t: number) {
      const hor = H * HOR;
      smokePuff(0.6 * W, hor + 0.025 * H, W * 0.1, 0.045 + 0.015 * Math.sin(t * 0.3));
      battleA.forEach((sh) => battleShip(sh, 1, t));
      burningShip(t);
      battleB.forEach((sh) => battleShip(sh, -1, t));
    }
    ...
    /* paint one frame synchronously so the scene exists before the first rAF */
    drawFrame(3);

    /* entrance animation: arm only once the document is actually visible */
    const root = canvas.closest(".seas-root");
    function arm() {
      if (document.visibilityState === "visible") {
        root?.classList.add("seas-animate");
  ...
  return <canvas className="hero-bg" ref={ref} aria-hidden="true" />;
}
