...
export const metadata: Metadata = {
  title: "Easy Guides — Web3Guides",
  description: "Beginner-friendly crypto and Web3 guides. Start your journey into blockchain, DeFi, Bitcoin, and more with step-by-step easy articles.",
};
...
export default async function EasyPage({ searchParams }: Props) {
  const page = Math.max(1, parseInt(searchParams.page ?? "1", 10));
  const offset = (page - 1) * PAGE_SIZE;

  const [guides, total] = await Promise.all([
    getGuidesByDifficulty("beginner", { limit: PAGE_SIZE, offset }),
    countGuidesByDifficulty("beginner"),
  ]);
  ...
  return (
    <div style={{ background: "#fefbf6", minHeight: "100vh" }}>
      ...
      <header style={{ position: "sticky", top: 0, zIndex: 50, background: "rgba(255,255,255,0.95)", backdropFilter: "blur(12px)", borderBottom: "1px solid #e5e7eb" }}>
        <div style={{ margin: "0 auto", maxWidth: 1280, display: "flex", height: 56, alignItems: "center", justifyContent: "space-between", padding: "0 24px" }}>
          ...
          <div style={{ display: "flex", gap: 16 }}>
            <Link href="/medium" style={{ fontFamily: "'Space Mono', monospace", fontSize: "0.7rem", color: "#6b7280", textDecoration: "none" }}>⚡ Medium</Link>
      ...
      {/* Hero */}
      <section style={{ padding: "56px 24px 40px", background: "linear-gradient(135deg, rgba(16,185,129,0.08) 0%, transparent 60%)" }}>
        <div style={{ margin: "0 auto", maxWidth: 1280 }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 10, borderRadius: 50, padding: "6px 16px", marginBottom: 20, background: "rgba(16,185,129,0.1)", border: "1px solid rgba(16,185,129,0.3)", color: "#059669", fontFamily: "'Space Mono', monospace", fontSize: "0.7rem" }}>
            🌱 Easy Level
          </div>
          <h1 style={{ fontFamily: "'Bungee', cursive", fontWeight: 400, fontSize: "clamp(2.5rem, 6vw, 4rem)", lineHeight: 1.05, color: "#1a1a1a", marginBottom: 16 }}>
            Beginner{" "}
            <span style={{ color: "#10b981" }}>Guides</span>
          </h1>
          ...
          {total > 0 && (
            <div style={{ marginTop: 20, display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ height: 2, width: 40, borderRadius: 2, background: "#10b981", opacity: 0.5 }} />
              <span style={{ fontFamily: "'Space Mono', monospace", fontSize: "0.7rem", color: "#10b981" }}>
      ...
      <section style={{ margin: "0 auto", maxWidth: 1280, padding: "0 24px 64px" }}>
        {guides.length === 0 ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "80px 0", textAlign: "center" }}>
            ...
            <p style={{ fontFamily: "'Bungee', cursive", fontSize: "1.3rem", color: "#9ca3af", marginBottom: 8 }}>No easy guides yet</p>
          ...
          <>
            ...
            {totalPages > 1 && (
              <DifficultyPagination currentPage={page} totalPages={totalPages} />
            )}
          </>
        )}
      </section>
...
function DifficultyPagination({ currentPage, totalPages }: { currentPage: number; totalPages: number }) {
  ...
  return (
    <nav style={{ marginTop: 48, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
      ...
      {pages.map((p) => (
        <a key={p} href={`?page=${p}`}
           style={p === currentPage
             ? { display: "flex", height: 36, width: 36, alignItems: "center", justifyContent: "center", borderRadius: 8, fontFamily: "'Space Mono', monospace", fontSize: "0.8rem", fontWeight: 700, color: "#fff", background: "#10b981", textDecoration: "none" }
