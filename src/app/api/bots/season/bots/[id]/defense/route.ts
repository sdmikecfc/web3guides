...
import { SeasonError, seasonErrorResponse, requireSeason, setSeasonDefense, parseSeasonDefense } from "@/app/bots/_server/season";
...
export async function POST(req: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const input = parseSeasonDefense(body), { id } = await context.params;
    return NextResponse.json(await setSeasonDefense(botsDb(), auth.wallet, id, input));
