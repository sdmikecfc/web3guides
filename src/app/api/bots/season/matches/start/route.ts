...
import { SeasonError, seasonErrorResponse, requireSeason, startSeasonMatch, parseSeasonStart } from "@/app/bots/_server/season";
...
export async function POST(req: Request) {
  try {
    ...
    const input = parseSeasonStart(body);

    return NextResponse.json(await startSeasonMatch(botsDb(), auth.wallet, input));
