...
import { SeasonError, seasonErrorResponse, requireSeason, saveSeasonDraft, parseSeasonDraft } from "@/app/bots/_server/season";
...
export async function POST(req: Request) {
  try {
    ...
    const input = parseSeasonDraft(body);

    return NextResponse.json(await saveSeasonDraft(botsDb(), auth.wallet, input));
