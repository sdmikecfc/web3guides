/**
 * Season 5, the player's own HQ state. POST /api/s5/me  { t }
 *
 * Gated by a play-session token (the same token /api/s5/game-session mints;
 * POST so the token never lands in a URL/query log). Returns the caller's OWN
 * state only: display name, Medals, Shells, the resolved TANK
 * (lib/s5/model resolveTank over the hq JSONB + dollars held), and the
 * past-season TROPHIES (lib/s5/trophies). Never returns the wallet.
 */
import { NextResponse } from "next/server";
import { s5Db, SEASON_KEY } from "@/lib/s5/server";
import { resolveTank } from "@/lib/s5/model";
import { handleForPlayer } from "@/lib/s5/handles";
import { clampStats } from "@/lib/s5/games";
import { buildHqView, heldUsdOf } from "@/lib/s5/me";
import { getTrophies } from "@/lib/s5/trophies";
import { getSeasonSnapshot } from "@/lib/s5/data";
import { myCommitments } from "@/lib/s5/warEffort";
...
export type PayoutView = {
   ...
   * wallet's Medals share. Estimate only; settlement adds breadth + the
   * qualifier gate. */
  securedUsd: number;
  /** Pool dollars sitting on strongholds THIS wallet holds ($5+) that have not
   * breached yet: the "get it bonded" number. */
  lockedUsd: number;
  lockedCount: number;
  /** Up to 4 of those stronghold names for the warning line. */
  lockedNames: string[];
  /** This wallet's share of the Column's Medals, 0..1 (points only; the final
  ...
  /** What this wallet holds on EACH stronghold, in USD, keyed by lowercase
   * domain. Empty when signed out. The season total cannot answer "am I in
   * this one?", which is what a stronghold dossier has to show. */
 ...
 */
async function heldDaysOf(db: ReturnType<typeof s5Db>, wallet: string): Promise<number> {
  try {
    const days = new Set<string>();
    const page = 1000;
    for (let from = 0; ; from += page) {
      const { data, error } = await db
        .from("launch_wars_s5_ledger")
        .select("meta")
...
async function payoutOf(
  ...
  try {
    ...
    // Share of the Column: my Medals over everyone's (players count is small;
    ...
    // Strongholds this wallet actually holds at $5+.
    ...
    return {
      ...
      // PER-DOMAIN, not just the season total. A stronghold's dossier has to
...
export async function POST(req: Request) {
  ...
  // select("*") on purpose: the s5 schema lands via a later migration and this
  ...
  // THE WAR EFFORT: this wallet's OWN Shells commitments, named from the
  ...
  return NextResponse.json({
    ...
    player: {
      ...
      // yourself meant spotting your name among fifty commanders. It is the
    ...
    payout,
    // THE WAR EFFORT: Shells committed to specific strongholds (never cash).
    warEffort,
  });
}
