/**
 * Season 5 funnel events. POST /api/s5/events
 *
 * Fire-and-forget beacon sink (lib/s5/track.ts): inserts one row into
 * launch_wars_s5_funnel_events. Defensive by design:
 *
...
export async function POST(req: Request) {
  ...
  try {
    const { error } = await db.from("launch_wars_s5_funnel_events").insert({
