/**
 * Season 5 — link a wallet-first commander to a Discord account.
 *
 *   POST /api/s5/link-discord  { address, code, message, signature }
 *
 * WHY THIS EXISTS. The bot has shipped `/siege link` since launch: it writes a
 * one-time code into launch_wars_s5_link_codes and tells the player to finish
 * at `${homeUrl}/link`. The web half was never built for S5, so that URL 404'd
 * and every player who ran the command hit a dead end on launch day. This is a
 * straight port of the proven S4 route (itself a port of S3's) against the s5
 * tables.
 *
 ...
 *
 * ONE DIFFERENCE FROM S4, and it is the whole shape of this season: S5 is SOLO
 ...
 * day the standings rail was a column of identical "Commander" entries, because
 * a row already holding that word was not treated as a placeholder and so never
 * healed to the player's real Discord name. */
const GENERIC_NAMES = new Set(["commander", "player", "pilot", "captain", "recruit", "anon", "unknown"]);

// A display_name equal to the wallet (the s5_grant auto-create placeholder),
...
export async function POST(req: Request) {
  ...
  const ok = (extra: Record<string, unknown>) =>
    NextResponse.json({ ok: true, team: "front", teamName: "The Iron Column", accent: "#e0662e", ...extra });

  // 1) The code must exist, be unused, and be unexpired.
  const { data: codeRow } = await db
    .from("launch_wars_s5_link_codes")
  ...
  if (!codeRow || codeRow.used || new Date(codeRow.expires_at).getTime() < Date.now()) {
    return NextResponse.json(
      { ok: false, error: "That code is invalid or expired. Run /siege link in Discord again." },
      { status: 400 },
    );
  }
  ...
  // 3) Find this wallet's commander (if any).
  ...
  // 4a) Existing commander, no Discord yet: bind it (adopt the Discord name if none set).
  if (player) {
    ...
    const { error: upErr } = await db.from("launch_wars_s5_players").update(patch).eq("id", player.id);
    if (upErr) {
      return NextResponse.json(
  ...
  // 4b) No commander for this wallet: the signature already proved ownership,
