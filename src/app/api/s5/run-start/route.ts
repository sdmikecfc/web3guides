 ...
 *
 *   POST /api/s5/run-start  { t, game }  -> { ok, nonce, stats }
 *
 * Port of the proven S4 route. The game page trades the play-session token
 * for a single-use NONCE tied to the wallet and the game. /api/s5/score
 ...
 * from the ONE shared registry (lib/s5/games).
 *
...
import { GAME_RULES, clampStats } from "@/lib/s5/games";
import { hullForTier, resolveTank } from "@/lib/s5/model";
...
export async function POST(req: Request) {
  ...
  // whole time (api/s5/me reads `skin`, lib/s5/roster reads `skin`, and S4's
  ...
  const { error } = await db.from("launch_wars_s5_run_nonces").insert({
