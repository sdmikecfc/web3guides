 ...
 *
 *   POST /api/s5/score  { t, game, score, nonce, meta? }
 *
 * Port of the proven S4 route with the S5 economy simplification: a banked run
 * pays a FLAT POINTS_PER_RUN Medals (no per-game credit formulas yet), plus
 * SPRINT_RUN_BONUS_POINTS while a siege sprint is running, always bounded by
 ...
 * leaderboard separates skill), but Medals for the day are already banked.
 *
 * Mechanics, unchanged from S4:
 * - One best-of-N scored row per game per UTC day per wallet
 *   (UNIQUE(season_key, wallet, game, day_key) on launch_wars_s5_scores).
 * - The client score only passes through under maxScore (anti-forge ceiling).
 * - Anti-cheat: a single-use nonce from /api/s5/run-start claimed atomically,
 *   a per-game minimum play time (floorMs), a 45 min nonce TTL.
 * - Grants go through the s5_grant RPC (same contract as the proven s4_grant:
...
import { s5Db, SEASON_KEY, scoringOpen } from "@/lib/s5/server";
import {
  GAME_RULES,
  bankableRules,
...
export async function POST(req: Request) {
  ...
  const { data: sess } = await db
    .from("launch_wars_s5_game_sessions")
  ...
  // Season gate: the money write. No real Medals mint while the season is not
  ...
  // ── Anti-cheat: claim the single-use run nonce + enforce min play time ──────
  {
    ...
    const claim = await db
      .from("launch_wars_s5_run_nonces")
    ...
    //
    // So a short run still banks IF it scored like a win. The gap is not close:
    // 20% of maxScore is 400-1945 against idle scores of 0-84, so this cannot
    // launder an AFK run, while any real fast finish clears it easily.
    const fastWinBar = Math.round(rules.maxScore * (rules.fastWinFrac ?? 0));
  ...
  // Medals AND Shells. Shells are deliberately NOT clamped by `capRoom`:
  // that cap bounds the season's MEDALS payout, which is prize weight and real
  // money, whereas Shells are a shop token with no payout and are already
  // bounded by the attempts budget (four games x one paying run a day). Tying
  // them to the Medals room would mean a sprint day quietly paid fewer Shells,
  // which is backwards.
  ...
  // THE FRONT REACTS: a run banked while a stronghold is inside its siege
  ...
  // Shells, a score-board standing and raid damage, but no Medals.
  //
  ...
  const paidMedalsToday = rows.some((r) => (Number(r.points) || 0) > 0);
  const runPoints = paidMedalsToday ? 0 : base;

  if (!existing) {
    const points = Math.min(runPoints, capRoom);
    const { error: insErr } = await db.from("launch_wars_s5_scores").insert({
    ...
    // A run that banks pays Shells even once the Medals cap is full: the
    ...
    return NextResponse.json({
      ...
      // is banked, runPoints is 0 for the rest of the day (see paidMedalsToday
      // above), so anything but 0 here tells the player to grind for Medals
  ...
  if (used >= rules.attempts) {
    // PRACTICE LEDGER (Mike 2026-08-13): runs past the attempts budget pay
    // nothing and NEVER touch `score` - the bot's daily camo sweep and duel
    // resolution read that column - but the day's practice best is kept in
    // meta so the board can show it. Same session/nonce/clamp path as a
    // counted run, so forging a practice entry is exactly as hard.
  ...
  // Medals for this game today are already banked (flat POINTS_PER_RUN); an
  ...
  return NextResponse.json({
    ...
    dailyPointsLeft: 0,
    sprint: sprint.active,
    sprintBonus: 0, // this game's Medals for today are already banked
  });
}
