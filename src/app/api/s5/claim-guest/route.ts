/**
 * Season 5, GUEST PROGRESS CLAIM. POST /api/s5/claim-guest  { t, scores }
 *
 ...
 * - Medals: POINTS_PER_RUN per valid played game, total clamped to
 *   GAME_DAILY_POINTS_CAP (40), granted via s5_grant reason "guest-claim".
 * - IDEMPOTENT + fail-closed: the claim is recorded FIRST as a sentinel row in
 *   launch_wars_s5_scores (game "guest-claim", day_key "guest"; the table's
...
import { GAME_DAILY_POINTS_CAP, POINTS_PER_RUN, SHELLS_PER_RUN, bankableRules } from "@/lib/s5/games";
...
export async function POST(req: Request) {
  ...
  // Total: flat Medals per valid played game, hard-clamped to the combined cap.
  ...
  const { error: gErr } = await db.rpc("s5_grant", {
    p_season_key: SEASON_KEY,
    p_wallet: wallet,
    p_display_name: null,
    p_points: points,
    // Shells for the runs they already played. A guest who tried the arcade
    // before enlisting banked real runs; paying Medals for them and not Shells
  ...
  if (gErr) {
    ...
    console.warn(`[s5/claim-guest] grant failed for ${wallet}: ${gErr.message}`);
