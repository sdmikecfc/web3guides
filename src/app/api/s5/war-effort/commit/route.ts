 ...
 * POST /api/s5/war-effort/commit  { t, domain, shells }
 *
 * MONEY PATH. It spends Shells, so it follows the same discipline as
 * /api/s5/tank-unlock, which is the web mirror of the bot's fail-closed
 * spendOk() escrow:
 ...
 *   - the spend fails CLOSED: s5_grant returns ok:false (no throw) on an
...
async function sprintArmed(db: ReturnType<typeof s5Db>, domain: string): Promise<boolean> {
  try {
    const { data } = await db
      .from("launch_wars_boss_config")
      .select("value")
      .eq("key", "s5_sprint_state")
      .maybeSingle();
