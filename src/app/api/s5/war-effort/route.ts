/**
 * Season 5, THE WAR EFFORT state. POST /api/s5/war-effort  { t? }
 *
 ...
 * /api/s5/war-effort/commit and resolution lives in the bot.
 */
...
export async function POST(req: Request) {
  ...
  const strongholds = snapshot.targets.map((target) => {
    const agg: WarEffortTotal =
      totals.get(target.domain) || { domain: target.domain, committed: 0, commanders: 0, won: 0, lost: 0 };
    return {
