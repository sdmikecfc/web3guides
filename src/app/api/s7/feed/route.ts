/**
 * THE UPLINK FEED. GET /api/s7/feed -> { items: [{ t, kind, text }] }
 *
 * Read-only, public, shaped text only: display names, domains and Valor
...
export async function GET(req: Request) {
  ...
  try {
    ...
    // check. Every read in lib/s7/data.ts (which works in production) is
    ...
    const led = await db
      ...
      .limit(16);
    const we = await db
      .from("launch_wars_s7_war_effort")
    ...
    for (const r of led.data ?? []) {
      ...
      if (reason.startsWith("game:")) {
        items.push({ t, kind: "game", text: `${who} won ${reason.slice(5)} · +${pts} Valor` });
      } else if (reason === "hold") {
        items.push({ t, kind: "hold", text: `${who} held the line · +${pts} Valor` });
      } else if (reason === "raid") {
    ...
    for (const r of we.data ?? []) {
      ...
      items.push({
        t,
        kind: "commit",
        text: `${short(names.get(r.wallet as string))} committed ${Number(r.shells ?? 0)} Gold at ${r.domain}`,
      });
    }
    for (const r of tg.data ?? []) {
      const t = Date.parse(r.bonded_at as string) || 0;
      items.push({ t, kind: "bond", text: `${r.domain} RECLAIMED - the front advances` });
    }
    if (diag) {
      ...
      notes.rawCounts = {
        targets: await raw("launch_wars_s7_targets"),
        ledger: await raw("launch_wars_s7_ledger"),
        trades: await raw("launch_wars_s7_trades"),
        players: await raw("launch_wars_s7_players"),
      };
      const one = await db.from("launch_wars_s7_ledger").select("season_key, reason, is_test").limit(1);
    ...
    console.error("[s7 feed] threw:", notes.threw);
  }
