/**
 * Season 5, the player's own HQ state. POST /api/s7/me  { t }
 *
 * Gated by a play-session token (the same token /api/s7/game-session mints;
 * POST so the token never lands in a URL/query log). Returns the caller's OWN
 * state only: display name, Valor, Gold, the resolved TANK
 * (lib/s7/model resolveTank over the hq JSONB + dollars held), and the
 * past-season TROPHIES (lib/s7/trophies). Never returns the wallet.
 */
import { NextResponse } from "next/server";
import { s7Db, SEASON_KEY } from "@/lib/s7/server";
import { resolveTank } from "@/lib/s7/model";
import { handleForPlayer } from "@/lib/s7/handles";
import { clampStats } from "@/lib/s7/games";
import { buildHqView, heldUsdOf } from "@/lib/s7/me";
import { getTrophies } from "@/lib/s7/trophies";
import { getSeasonSnapshot } from "@/lib/s7/data";
import { myCommitments } from "@/lib/s7/warEffort";
...
export type PayoutView = {
   ...
   * wallet's Valor share. Estimate only; settlement adds breadth + the
   * qualifier gate. */
  securedUsd: number;
  /** Pool dollars sitting on keeps THIS wallet holds ($5+) that have not
   * breached yet: the "get it bonded" number. */
  lockedUsd: number;
  lockedCount: number;
  /** Up to 4 of those keep names for the warning line. */
  lockedNames: string[];
  /** This wallet's share of the Column's Valor, 0..1 (points only; the final
  ...
  /** What this wallet holds on EACH keep, in USD, keyed by lowercase
   * domain. Empty when signed out. The season total cannot answer "am I in
   * this one?", which is what a keep dossier has to show. */
 ...
 */
async function heldDaysOf(db: ReturnType<typeof s7Db>, wallet: string): Promise<number> {
  try {
    const days = new Set<string>();
    const page = 1000;
    for (let from = 0; ; from += page) {
      const { data, error } = await db
        .from("launch_wars_s7_ledger")
        .select("meta")
...
async function payoutOf(
  ...
  try {
    ...
    // Share of the Column: my Valor over everyone's (players count is small;
    ...
    // Keeps this wallet actually holds at $5+.
    ...
    return {
      ...
      // PER-DOMAIN, not just the season total. A keep's dossier has to
...
export async function POST(req: Request) {
  ...
  // select("*") on purpose: the s7 schema lands via a later migration and this
  ...
  // THE WAR EFFORT: this wallet's OWN Gold commitments, named from the
  ...
  return NextResponse.json({
    ...
    player: {
      ...
      // yourself meant spotting your name among fifty adventurers. It is the
    ...
    payout,
    // THE WAR EFFORT: Gold committed to specific keeps (never cash).
    warEffort,
  });
}
