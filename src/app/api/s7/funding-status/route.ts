/**
 * Season 5, the funding-wizard STUCK-STEP DETECTOR. GET/POST /api/s7/funding-status
 *
 ...
 *
 * States (thresholds in lib/s7/funding.ts): HOLDER when ANY stronghold token
 ...
 * same rule as /api/s7/me). GET is the tokenless probe: it returns NO_WALLET
...
export async function POST(req: Request) {
  ...
  try {
    wallet = await walletForSession(s7Db(), body.t);
  } catch {
