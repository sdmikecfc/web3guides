/**
 * Season 5 — link a wallet-first adventurer to a Discord account.
 *
 *   POST /api/s7/link-discord  { address, code, message, signature }
 *
 * WHY THIS EXISTS. The bot has shipped `/resist link` since launch: it writes a
 * one-time code into launch_wars_s7_link_codes and tells the player to finish
 * at `${homeUrl}/link`. The web half was never built for S7, so that URL 404'd
 * and every player who ran the command hit a dead end on launch day. This is a
 * straight port of the proven S4 route (itself a port of S3's) against the s7
 * tables.
 *
 ...
 *
 * ONE DIFFERENCE FROM S4, and it is the whole shape of this season: S7 is SOLO
 ...
 * day the standings rail was a column of identical "Adventurer" entries, because
 * a row already holding that word was not treated as a placeholder and so never
 * healed to the player's real Discord name. */
const GENERIC_NAMES = new Set(["adventurer", "player", "adventurer", "captain", "recruit", "anon", "unknown"]);

// A display_name equal to the wallet (the s7_grant auto-create placeholder),
...
export async function POST(req: Request) {
  ...
  const ok = (extra: Record<string, unknown>) =>
    NextResponse.json({ ok: true, team: "front", teamName: "The Guild", accent: "#d9963a", ...extra });

  // 1) The code must exist, be unused, and be unexpired.
  const { data: codeRow } = await db
    .from("launch_wars_s7_link_codes")
  ...
  // 3) Find this wallet's adventurer (if any).
  ...
  // 4a) Existing adventurer, no Discord yet: bind it (adopt the Discord name if none set).
  if (player) {
    ...
    const { error: upErr } = await db.from("launch_wars_s7_players").update(patch).eq("id", player.id);
    if (upErr) {
      return NextResponse.json(
  ...
  // 4b) No adventurer for this wallet: the signature already proved ownership,
