/**
 * Season 5 funnel events. POST /api/s7/events
 *
 * Fire-and-forget beacon sink (lib/s7/track.ts): inserts one row into
 * launch_wars_s7_funnel_events. Defensive by design:
 *
...
export async function POST(req: Request) {
  ...
  try {
    const { error } = await db.from("launch_wars_s7_funnel_events").insert({
