 ...
 *
 *   POST /api/s7/run-start  { t, game }  -> { ok, nonce, stats }
 *
 * Port of the proven S4 route. The game page trades the play-session token
 * for a single-use NONCE tied to the wallet and the game. /api/s7/score
 ...
 * from the ONE shared registry (lib/s7/games).
 *
...
import { GAME_RULES, clampStats } from "@/lib/s7/games";
import { getTracks } from "@/lib/s7/classes";
import { hullForTier, resolveTank } from "@/lib/s7/model";
...
export async function POST(req: Request) {
  ...
  // whole time (api/s7/me reads `skin`, lib/s7/roster reads `skin`, and S4's
  ...
  // THE CLASS LOADOUT (ADR-0129, the S7 novelty): the active class's track -
  // classId + level + gear - resolved SERVER-SIDE from launch_wars_s7_classes
  // and handed to the sims through the shell, exactly like stats. Null when
  // the wallet has not picked a class yet (sims run their level-1 default).
  // The sims prove loadout matters via the harness (b)/(b2) gates.
  let loadout: { classId: string; level: number; gear: { weapon: number; armor: number; trinket: number } } | null = null;
  try {
    const { active, tracks } = await getTracks(db, sess.wallet);
    if (active) {
      const track = tracks.find((t) => t.classKey === active);
      if (track) loadout = { classId: track.classKey, level: track.level, gear: track.gear };
    }
  } catch {
    loadout = null; // class read failed: run proceeds classless rather than erroring
  }
  ...
  const { error } = await db.from("launch_wars_s7_run_nonces").insert({
  ...
  return NextResponse.json({ ok: true, nonce, stats, tank, loadout });
}
