 ...
 *
 *   POST /api/s7/game-session  { address, message, signature }  -> { ok, token }
 *
 ...
 * SESSION_STORAGE_KEY in lib/s7/games) is exchanged for run nonces
 * (/api/s7/run-start) and banked scores (/api/s7/score).
 * Chain: game-session -> run-start -> score.
 */
import { NextResponse } from "next/server";
import { s7Db, verifyOwnership, SEASON_KEY } from "@/lib/s7/server";
...
export async function POST(req: Request) {
  ...
  const { error } = await db.from("launch_wars_s7_game_sessions").insert({
