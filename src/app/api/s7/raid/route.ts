/**
 * S7 RAID status feed (public, cached 60s like /api/s7/contracts):
 *
 *   GET /api/s7/raid -> { ok, date, resolveHourUtc, count, boss, oddsPct }
 *
 * Reads the bot's s7_battle_roster config row (JSON {date, wallets:[]}) and
 * counts it ONLY when its date matches the NEXT resolve date (join-first rule:
 * today before 14:00 UTC, else tomorrow), plus the s7_season window so the
 * schedule can place the tutorial and finale bosses. The boss schedule + odds
 * live in the client-safe lib/s7/raid.ts mirror.
 *
...
async function readRaidConfig(): Promise<{ rosterRaw: string | null; season: SeasonWindowLike }> {
  ...
  try {
    ...
    const { data, error } = await db
      ...
      .select("key, value")
      .in("key", ["s7_battle_roster", "s7_season"]);
    if (error || !data) return empty;
    const map = new Map((data || []).map((r) => [String(r.key), r.value]));
    const rosterRaw = typeof map.get("s7_battle_roster") === "string" ? (map.get("s7_battle_roster") as string) : null;
    let season: SeasonWindowLike = { launchAt: null, endAt: null };
    const seasonRaw = map.get("s7_season");
...
async function readLastResult(): Promise<{ name: string; won: boolean; dayKey: string } | null> {
  try {
    const db = s7Db();
    const { data } = await db
      .from("launch_wars_s7_raid_bosses")
