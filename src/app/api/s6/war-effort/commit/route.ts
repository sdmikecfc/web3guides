 ...
 * POST /api/s6/war-effort/commit  { t, domain, shells }
 *
 * MONEY PATH. It spends Shells, so it follows the same discipline as
 * /api/s6/tank-unlock, which is the web mirror of the bot's fail-closed
 * spendOk() escrow:
...
import { SPRINT_ARM_PCT, WAR_EFFORT, WAR_EFFORT_TABLE } from "@/lib/s6/warEffort";
...
async function sprintArmed(db: ReturnType<typeof s6Db>, domain: string): Promise<boolean> {
  try {
    const { data } = await db
