/**
 * Season 5, wallet-first WEB ENLIST. POST /api/s6/join
 *
 * Port of the proven S4 route (/api/s4/join) onto the s6 tables, one team:
 * everyone lands on "front" (The Resistance).
 *
 *   RETURNING wallet (already an s6 player) -> returned instantly, no signature.
...
import { getTheme } from "@/lib/s6/data";
...
export async function POST(req: Request) {
  ...
  async function attributeReferral(referredWallet: string) {
    try {
      ...
      const { error: edgeErr } = await db.from("launch_wars_s6_referrals").insert({
      ...
      await db.from("launch_wars_s6_funnel_events").insert({
  ...
  const carryIdentityFromS5 = async (w: string) => {
    try {
  ...
  if (existing) {
    ...
    if (!existing.discord_id) await carryIdentityFromS5(wallet);
  ...
  const { data: created, error } = await db
    ...
    .select("team_key")
    .single();
  await carryIdentityFromS5(wallet);
  if (error) {
