 ...
 *
 *   POST /api/s6/game-session  { address, message, signature }  -> { ok, token }
 *
 ...
 * SESSION_STORAGE_KEY in lib/s6/games) is exchanged for run nonces
 * (/api/s6/run-start) and banked scores (/api/s6/score).
 * Chain: game-session -> run-start -> score.
 */
import { NextResponse } from "next/server";
import { s6Db, verifyOwnership, SEASON_KEY } from "@/lib/s6/server";
...
export async function POST(req: Request) {
  ...
  const { error } = await db.from("launch_wars_s6_game_sessions").insert({
