/**
 * Season 5, the HQ loadout write. POST /api/s6/hq  { t, set: { tank?, camo?, commander? } }
 *
 * COSMETIC ONLY: fields an OWNED tank, swaps camo, swaps commander. No Shells
 * move here (unlocks are /api/s6/tank-unlock, the money path). Discipline:
 *
...
import { S6_CACHE_TAG } from "@/lib/s6/data";
import { walletForSession, hqObject, ensurePlayer } from "@/lib/s6/me";
import {
  CAMOS,
  ownedCamoKeys,
  ownedPilotKeys,
...
import { pilotByKey, tankByKey } from "@/lib/s6/tanks";

export const runtime = "nodejs";

const bad = (error: string, status = 400) => NextResponse.json({ ok: false, error }, { status });

type SetBody = { tank?: unknown; camo?: unknown; pilot?: unknown };
...
export async function POST(req: Request) {
  ...
  const wants: { tank?: string; camo?: CamoKey; pilot?: string } = {};
  ...
  if (set.pilot !== undefined) {
    const c = pilotByKey(set.pilot);
    if (!c) return bad("Unknown commander.");
    wants.pilot = c.key;
  }
  if (wants.tank === undefined && wants.camo === undefined && wants.pilot === undefined) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    // field a mech (the 2026-08-17 "Field does nothing" report).
    ...
    if (wants.pilot !== undefined) {
      const c = pilotByKey(wants.pilot);
      if (c?.prize && !ownedPilotKeys(hq).includes(wants.pilot)) {
    ...
    if (wants.pilot !== undefined) newHq.pilot = wants.pilot;
    ...
    if (!error && written && written.length) {
      ...
      const changedCmdr = wants.pilot !== undefined && wants.pilot !== hq.pilot;
      if (changedTank || changedCamo || changedCmdr) revalidateTag(S6_CACHE_TAG);

      return NextResponse.json({
        ok: true,
        tank: resolveTank(savedHq),
        // the client reads `pilot` off this response (panels.tsx); returning
        // `commander` made a pilot swap look like it worked until reload
        pilot: resolvePilotKey(savedHq),
