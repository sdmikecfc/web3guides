 ...
 *
 *   GET /api/s6/hq-card?h=<handle>  ->  image/png 1200x630
 *
...
import { resolvePublicHq, type PublicGarage } from "@/lib/s6/publicHq";
import { tankArt } from "@/lib/s6/model";
import type { CamoKey } from "@/lib/s6/model";
...
function tankCardArt(origin: string, tankKey: string, camo: CamoKey): string | null {
  ...
  // the needle was still the S5 path, so it NEVER matched and every share
  // card asked for /s6-art/tank/<key>.jpg - a file that does not exist at that
  // level. The 243 baked card JPEGs live one directory down, in tank/card/.
  return origin + art.replace("/s6-art/tank/", "/s6-art/tank/card/").replace(/\.webp$/, ".jpg");
}
