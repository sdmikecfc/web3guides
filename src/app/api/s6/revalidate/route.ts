 ...
 *
 *   curl -X POST https://tanks.web3guides.com/api/s6/revalidate \
 *     -H "content-type: application/json" \
 *     -d '{"key":"<S6_REVALIDATE_KEY>"}'
 *
 * The key lives in the S6_REVALIDATE_KEY env var on Vercel. Without it set,
...
export async function POST(req: Request) {
  const secret = process.env.S6_REVALIDATE_KEY;
  if (!secret) {
  ...
  revalidateTag(S6_CACHE_TAG);
  revalidatePath("/s6");

  return NextResponse.json({ ok: true, purged: [S6_CACHE_TAG, "/s6"], at: new Date().toISOString() });
}
