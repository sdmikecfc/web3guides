...
export const metadata: Metadata = {
  title: "Advanced Guides — Web3Guides",
  description: "Advanced crypto and Web3 guides for experienced builders. Deep dives into smart contract security, MEV, zero-knowledge proofs, and cutting-edge blockchain tech.",
};
...
export default async function AdvancedPage({ searchParams }: Props) {
  const page = Math.max(1, parseInt(searchParams.page ?? "1", 10));
  const offset = (page - 1) * PAGE_SIZE;

  const [guides, total] = await Promise.all([
    getGuidesByDifficulty("advanced", { limit: PAGE_SIZE, offset }),
    countGuidesByDifficulty("advanced"),
  ]);
  ...
  return (
    <div style={{ background: "#fefbf6", minHeight: "100vh" }}>
      ...
      {/* Hero */}
      <section style={{ padding: "56px 24px 40px", background: "linear-gradient(135deg, rgba(99,102,241,0.08) 0%, transparent 60%)" }}>
        <div style={{ margin: "0 auto", maxWidth: 1280 }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 10, borderRadius: 50, padding: "6px 16px", marginBottom: 20, background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.3)", color: "#4f46e5", fontFamily: "'Space Mono', monospace", fontSize: "0.7rem" }}>
            🔮 Advanced Level
          </div>
          ...
          <p style={{ fontFamily: "'DM Sans', sans-serif", maxWidth: 520, fontSize: "1.05rem", lineHeight: 1.65, color: "#6b7280" }}>
            For experienced builders and power users. Deep dives into smart contract security, ZK proofs, MEV, protocol mechanics, and frontier Web3 technology.
          </p>
          {total > 0 && (
            <div style={{ marginTop: 20, display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ height: 2, width: 40, borderRadius: 2, background: "#6366f1", opacity: 0.5 }} />
              <span style={{ fontFamily: "'Space Mono', monospace", fontSize: "0.7rem", color: "#6366f1" }}>
      ...
      <section style={{ margin: "0 auto", maxWidth: 1280, padding: "0 24px 64px" }}>
        {guides.length === 0 ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "80px 0", textAlign: "center" }}>
            ...
            <p style={{ fontFamily: "'Bungee', cursive", fontSize: "1.3rem", color: "#9ca3af", marginBottom: 8 }}>No advanced guides yet</p>
          ...
          <>
            ...
            {totalPages > 1 && (
              <DifficultyPagination currentPage={page} totalPages={totalPages} accentColor="#6366f1" />
            )}
          </>
        )}
      </section>
