import { statsFor, WEAPON, type BuildV4, type Module, type Slot } from "./engine";
...
export const FAMILY_LABEL = { brute: "Brute", hotshot: "Hotshot", deadeye: "Deadeye" };
...
export function partRows(b: BuildV4, slot: Slot | "weapon"): StatRow[] {
  ...
  if (slot.startsWith("leg")) rows.push({ key: "mobility", label: "Mobility", value: ({ brute: 15, hotshot: 32, deadeye: 22 })[p.family] + p.design * 2 }, { key: "evasion", label: "Evasion", value: ({ brute: 2, hotshot: 10, deadeye: 4 })[p.family] });
...
export function partName(slot: Slot | "weapon", b: BuildV4): string {
  ...
  if (slot === "armL" && p.family === "brute" && p.design === 1) return "Brute shield arm";
  return `${FAMILY_LABEL[p.family]} ${SLOT_LABEL[slot].toLowerCase()} ${p.design + 1}`;
}
export function partDescription(slot: Slot | "weapon", b: BuildV4): string {
  ...
  if (slot === "armL" && statsFor(b).shield) return "A folding shield protects the front while this arm is free. Blocking spends guard strength.";
