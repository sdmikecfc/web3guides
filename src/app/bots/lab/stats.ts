import { FLAME, RIFLE, statsFor, WEAPON, type BuildV4, type Module, type Slot } from "./engine";
export const WEAPON_LABEL = { hammer: "Hammer", baton: "Baton", rifle: "Rifle", flamethrower: "Flamethrower" };
...
export const FAMILY_DESCRIPTION = {
  ...
  deadeye: "Rifle + push-off. Shoves a close rival, dashes back, then fires a quick follow-up. Needs its left arm and both legs for the full escape.",
};
...
export function overallRows(b: BuildV4): StatRow[] {
  ...
  return [
    ...
    { key: "damage", label: b.weapon === "flamethrower" ? "Flame tick power" : "Hit power", value: Math.round(WEAPON[b.weapon].damage + s.force * (b.weapon === "flamethrower" ? .2 : 1)) },
...
export function partRows(b: BuildV4, slot: Slot | "weapon"): StatRow[] {
  if (slot === "weapon") { const w = WEAPON[b.weapon]; return [
    ...
    { key: "recovery", label: b.weapon === "flamethrower" ? "Cooling time" : "Recovery", value: +((w.recovery - (b.weapon === "flamethrower" ? FLAME.duration : 0)) / 60).toFixed(2), unit: "s", lower: true },
    ...(b.weapon === "flamethrower" ? [{ key: "burst", label: "Burst duration", value: FLAME.duration / 60, unit: "s" }, { key: "burn", label: "Burn duration", value: FLAME.burnDuration / 60, unit: "s" }] : []),
  ]; }
...
export function partName(slot: Slot | "weapon", b: BuildV4): string {
  if (slot === "weapon") return { hammer: "Forge hammer", baton: "Arc baton", rifle: "Longshot rifle", flamethrower: "Flamethrower" }[b.weapon];
...
export function partDescription(slot: Slot | "weapon", b: BuildV4): string {
  if (slot === "weapon") return { hammer: "A committed wind-up. A clean hit knocks the opponent down.", baton: "Quick combinations. Every third clean hit briefly stuns.", rifle: `Shove, dash back, then a quick aimed shot. The push uses your left arm; dashing needs both legs. Escape recharges in ${RIFLE.escapeCooldown / 60} seconds.`, flamethrower: "A short cone with up to four hits per burst, followed by cooling. Clean hits leave a 2.5-second burn. Shields block the heat; sideways movement and distance escape the burst." }[b.weapon];
