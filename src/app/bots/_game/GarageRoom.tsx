...
import LegacyGarageRoom from "./LegacyGarageRoom";
...
export default function GarageRoom(props: Parameters<typeof GarageRoomPreview>[0]) {
  return process.env.NEXT_PUBLIC_BOTS_ROOM_PREVIEW === "1" ? <GarageRoomPreview {...props} /> : <LegacyGarageRoom {...props} />;
}

export function GarageRoomPreview({ builds, parts, selectedBay, rows, sample, lookFor, onSelect, onBuild, onName, onFight, onParts, onTools, onEarn, onProgress, onExplore, nudge, onDismiss, onRecycle, busy = false }: {
  ...
  const actors = useMemo(() => photos.filter(photo => emptySockets(photo.build).length < 7).map(photo => ({ id: `bay-${photo.build.bay}`, bay: photo.build.bay, build: photo.robot, look: photo.look })), [photos]);
  ...
  return <section className={css.room} aria-label={sample ? "Sample robot collection" : "Your robot collection"}>
    <div className={css.stage}>
      <div className={css.scene}><LivingRoomScene actors={actors} selectedId={`bay-${selectedBay}`} onAnchors={setAnchors} /></div>
      ...
      <div className={css.standRow} aria-label="All five robot stands">
        {BAYS.map(bay => {
          ...
          const anchor = anchors.find(value => value.bay === bay);
          return <button key={bay} className={css.stand} data-empty={!filled} data-suggested={suggested} aria-pressed={chosen}
            style={anchor ? { left: anchor.x, top: anchor.y, width: anchor.width, height: anchor.height } : { left: `${bay * 17 - 1}%`, top: "76%", width: "16%", height: "44%" }}
    ...
    <footer className={css.garageFoot}>
      ...
      <div className={css.footerTools} aria-label="Workshop shortcuts">
        <button onClick={onParts}><span aria-hidden>✦</span> Parts cabinet</button>
        <button onClick={onTools}><span aria-hidden>⚒</span> Tool board</button>
        <button onClick={onProgress}><span aria-hidden>☆</span> Milestones</button>
      </div>
