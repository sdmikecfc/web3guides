...
export default function CommunityRoom({ battles, state, campaign, onWatch, onPractice, onScores, onRetry }: CommunityRoomProps) {
  ...
  const period = campaign?.period.key === "week1" ? "Week 1" : campaign?.period.key === "week2" ? "Week 2" : "Two-week final";
  ...
  return <section className={css.room} aria-label="Community room">
    <div className={css.stage}>
      ...
      <header className={css.heading}><p className={css.eyebrow}>SPROCKET ROW · THE COMMUNITY ROOM</p><h1>Meet the neighbourhood.</h1><p>Watch fights. Meet rivals. Find your next idea.</p></header>
      <div className={css.streetControls} aria-label="Robots on the street">
        {visitors.map(visitor => {
          ...
          return <button key={visitor.id} className={css.visitor} data-practice={!visitor.fightId} data-helper={visitor.bay === 5}
            ...
            <span className={css.visitorLabel}><small>{visitor.label}</small><strong>{visitor.name}</strong><span>{visitor.fightId ? "Watch replay" : visitor.bay === 5 ? "Try a practice fight" : "Practice preview"}<b aria-hidden>↗</b></span></span>
          </button>;
        })}
      </div>
      <aside className={css.scoreboard} aria-label="Battle points leaders"><span className={css.boardPin} aria-hidden /><p className={css.boardEyebrow}>{period}</p><h2>Battle points</h2>
        {standings.length ? <ol>{standings.map((row, index) => <li key={`${row.rank}:${index}`}><b>{row.rank}</b><span>{row.name}</span><strong>{row.score.toLocaleString("en-US", { maximumFractionDigits: 0 })}<small>pts</small></strong></li>)}</ol> : <p className={css.boardEmpty}>{campaign?.source === "available" ? campaign.campaign?.status === "draft" || campaign.period.status === "upcoming" ? "The first scores arrive after launch." : "No battle scores yet." : "Scores are not available yet."}</p>}
        <button onClick={onScores}>See all scores <span aria-hidden>→</span></button>
      </aside>
      <div className={css.spotlight}>{featured ? <><span>JUST FOUGHT</span><strong>{featured.names[0]} <i>vs</i> {featured.names[1]}</strong><button onClick={() => onWatch(featured.id)}>Watch their fight <span aria-hidden>▶</span></button></> : <><span>PRACTICE IS ALWAYS OPEN</span><strong>Pull up a seat at the arena.</strong><button onClick={onPractice}>Watch a practice fight <span aria-hidden>▶</span></button></>}</div>
    </div>
    <section className={css.fightBoard} aria-labelledby="community-fights-title">
      <div className={css.fightHeading}><div><h2 id="community-fights-title">Recent fights</h2><p role="status">{status}</p></div>{state === "failed" ? <button className={css.retry} onClick={onRetry}>Try again</button> : <button className={css.practice} onClick={onPractice}>Practice fight <span aria-hidden>↗</span></button>}</div>
      <div className={css.tickets} role="region" aria-label="Recent fight replays" tabIndex={0}>
        {recent.length ? recent.map(fight => <button key={fight.id} className={css.ticket} onClick={() => onWatch(fight.id)}><span className={css.ticketStamp}>{fight.mode === "pvp" ? "PLAYER FIGHT" : "ARENA CHALLENGE"}<small>{fight.seconds}s</small></span><strong>{fight.names[0]} <span>vs</span> {fight.names[1]}</strong><span className={css.ticketResult}>{fight.winnerName} won <b aria-hidden>▶</b></span></button>) : <button className={`${css.ticket} ${css.practiceTicket}`} onClick={onPractice}><span className={css.ticketStamp}>PRACTICE REPLAY</span><strong>{state === "loading" ? "Try a fight while we look." : "The arena is ready for you."}</strong><span className={css.ticketResult}>No coins or points needed <b aria-hidden>▶</b></span></button>}
      </div>
    </section>
  </section>;
}
