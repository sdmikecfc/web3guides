/**
 * S5 shared PAGE-FX layer — the promoted Gunner's Run (tankbuster) camera and
 ...
 * tape byte-identical across every page refactor (see scripts/s5-harness.ts
...
// sim.ts — every S5 sim already carries its own copy of exactly this pair;
...
export interface PageFxInput {
  ...
  w: number;
  h: number;
  /** The game's own view-scale factor (every S5 sim exposes s.k = viewW/480). */
  k: number;
