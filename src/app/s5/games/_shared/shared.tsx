 ...
 * s5 seams (lib/s5/games keys, /api/s5/* endpoints) with the S5 addition:
 * GUEST MODE. Every S5 game is playable with no wallet at all:
 *
 ...
 *   gets the same attempts/day as a commander (GAME_RULES[game].attempts);
 ...
 *   parked bests ONCE to /api/s5/claim-guest (server-side idempotent sentinel),
 *   converting them into Medals. localStorage marks the claim so this browser
 *   never re-posts.
 *
 * No Telegram path in S5 (the s5 tree has no TelegramProvider); wallet-first
 * only, same as the s5 join flow.
 */
"use client";
...
/** Default accent for S5 game chrome; games may pass their arcade-tile accent. */
...
// EIP-4361 shape, now SHARED with the garage's own sign-in (lib/s5/games
...
export function useS5Session() {
  ...
  const openSession = useCallback(async () => {
    ...
    try {
      ...
      const uri = `${window.location.origin}/s5/play`;
      const message = buildMessage(address, nonce, issuedAt, domain, uri);
      const signature = await signMessageAsync({ message });
      track("siwe_ok");
      const resp = await fetch("/api/s5/game-session", {
        method: "POST",
...
export type S5Session = ReturnType<typeof useS5Session>;
...
export type ScoreResult = {
  ok: boolean;
  points?: number;
  /** Play currency the bank paid. Shown next to the Medals line so the player
   ...
   * (see /api/s5/score). Display-only; the client never claims the bonus. */
  sprint?: boolean;
  /** Medals this specific run's bank paid on top of the flat per-run rate
   * because a sprint was active; 0 when sprint is false OR when this game's
   * Medals for today were already banked by an earlier attempt. */
  sprintBonus?: number;
  already?: boolean;
  error?: string;
};
...
export async function startRun(token: string, game: string): Promise<RunStartResult> {
  const r = await fetch("/api/s5/run-start", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function submitScore(
  ...
  const r = await fetch("/api/s5/score", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
// ── Guest progress (localStorage; the ONE shape /api/s5/claim-guest reads) ──
type GuestStore = {
  v: 1;
...
/**
 * Once a play session exists, convert parked guest bests into Medals, ONCE.
...
export function useGuestClaim(token: string | null) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch("/api/s5/claim-guest", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        if (data?.ok || data?.already) {
          ...
          fresh.scores = {};
          writeGuest(fresh);
          if (data?.ok) setClaimedPoints(Number(data.points) || 0);
        }
 ...
 * section (src/lib/s5/strings.ts). English defaults live inline below as the
