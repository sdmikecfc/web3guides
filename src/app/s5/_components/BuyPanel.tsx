"use client";
/**
 * S5 IN-APP BUY: one tap, $5, on the stronghold's bonding curve.
 *
 ...
 *     curve (see /api/s5/buy-info). Graduated or unknown falls back to the
 ...
 *   - BUY_IN_APP_ENABLED in lib/s5/launchpad.ts is the kill switch.
 *
...
import { USDC_E_ADDRESS, USDC_E_DECIMALS, buyLink, DOMA_PROFILE_URL } from "@/lib/s5/funding";
import {
...
export function BuyPanel({
  ...
  strings: {
    ...
    fallbackCta: string;
    /** Shown when the stronghold has not listed on-chain yet: there is no
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void fetch(`/api/s5/buy-info?domain=${encodeURIComponent(domain)}`)
