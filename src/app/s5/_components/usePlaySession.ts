 ...
 * button linked to /s5/join. But /s5/join ENLISTS A WALLET INTO THE SEASON. It
...
import { buildPlaySessionMessage, writeSessionToken } from "@/lib/s5/games";
...
export function usePlaySession(): PlaySessionState {
  ...
  const open = useCallback(async (): Promise<string | null> => {
    ...
    try {
      ...
      const uri = `${window.location.origin}/s5/hq`;
