 ...
 *
 * Three understated chips (EN / 한국어 / 中文). Clicking writes the `s5_lang`
 * cookie and reloads so server components re-render in the chosen locale
 * (getLocale in lib/s5/i18n) and client components re-read it on mount. The
...
import { LOCALE_COOKIE, LOCALES, clientLocale, type Locale } from "@/lib/s5/locale";
...
export function LanguageSwitcher({ ariaLabel = "Language" }: { ariaLabel?: string }) {
  ...
  function pick(code: Locale) {
    ...
    // 1 year, path=/ so it applies to every /s5 route; lax is fine (same-site).
