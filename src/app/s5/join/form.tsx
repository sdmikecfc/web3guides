 ...
 * /api/s5/join -> everyone lands on The Iron Column. Discord links later.
...
import { track, rememberRef, storedRef } from "@/lib/s5/track";
...
export function JoinForm() {
  ...
  async function postJoin(payload: Record<string, unknown>) {
    ...
    const ref = storedRef();
    const resp = await fetch("/api/s5/join", {
      method: "POST",
  ...
  async function handleEnlist() {
    ...
    try {
      ...
      if (result?.needsSignature) {
        ...
        const domain = typeof window !== "undefined" ? window.location.host : "tanks.web3guides.com";
        const uri =
          typeof window !== "undefined" ? `${window.location.origin}/s5/join` : "https://tanks.web3guides.com/s5/join";
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          <a href="/s5/map" style={primaryBtn(accent)}>
            {d.join.seeMap}
          </a>
          <a href="/s5#wizard" style={ghostBtn()} data-testid="join-fund">
            {d.join.fundCta}
          </a>
          <a href="/s5" style={ghostBtn()}>
            {d.links.hq}
          </a>
          <a href="/s5/play" style={ghostBtn()}>
            {d.join.playArcade}
          </a>
        </div>
      </div>
