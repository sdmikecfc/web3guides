 ...
 * In Discord run /siege link to get a one-time code, then here: connect the
 * wallet, enter the code, sign one ownership message (no gas) -> POST
 * /api/s5/link-discord. Port of the proven S4 form, itself a port of S3's.
...
export function LinkForm() {
  ...
  async function handleLink() {
    ...
    if (!codeOk) {
      setError("Enter the code from /siege link in Discord.");
      return;
    }
    setError(null);
    setSubmitting(true);
    try {
      ...
      const uri =
        typeof window !== "undefined" ? `${window.location.origin}/s5/link` : "https://web3guides.com/s5/link";
      const message = buildMessage({ domain, address, uri, nonce, issuedAt, code: code.trim().toUpperCase() });
      const signature = await signMessageAsync({ message });

      const resp = await fetch("/api/s5/link-discord", {
        method: "POST",
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        <div style={{ ...cardStyle, borderColor: `${linked.accent}55`, textAlign: "center" }}>
          ...
          <p style={{ fontSize: 14, color: MUTED, lineHeight: 1.6, margin: 0 }}>
            ...
            <b style={{ color: TEXT }}>/siege me</b> in the Doma Discord to see your {teamWord} and{" "}
            {DEFAULT_THEME.points}.
          </p>
        </div>
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center" }}>
          <a href="/s5/board" style={primaryBtn(linked.accent)}>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <div style={cardStyle}>
        ...
        <p style={{ fontSize: 13, color: FAINT, margin: 0, lineHeight: 1.5 }}>
          In the Doma Discord, run <b style={{ color: TEXT }}>/siege link</b>. The bot replies (only you
