"use client";
/**
 * Season 5 IRON SIEGE, YOUR PERSONAL HQ (the /s5 landing scene).
 *
 * THE STAGE IS A BAKED PLATE, PER TANK: campPlateFor(bayTank) resolves a 3:4
 * camp painting with the FIELDED tank baked under the bay (lib/s5/model.ts
 ...
 * trophies arrive from /api/s5/me when a play session exists (localStorage,
...
import { clampStats, GAMES, SEASON_MONEY_USD, readSessionToken } from "@/lib/s5/games";
import { DEFAULT_THEME } from "@/lib/s5/theme";
import { HERO_TANK_KEY, campPlateFor, commanderGrade, resolveCommanderKey, resolveTank, type ResolvedTank } from "@/lib/s5/model";
import { TANK_ROSTER } from "@/lib/s5/tanks";
import { commanderByKey, DEFAULT_COMMANDER_KEY } from "@/lib/s5/tanks";
import { RESOLVE_HOUR_UTC } from "@/lib/s5/raid";
import { STRINGS, fill, type S5Dict } from "@/lib/s5/strings";
import { clientLocale } from "@/lib/s5/locale";
import { isDemo, demoHqMe } from "@/lib/s5/demo";
import { markFtueHotspot } from "@/lib/s5/ftue";
import { track, rememberRef } from "@/lib/s5/track";
import { StatsBoard, type PayoutBoardView, type StatsView, type WarEffortView } from "./StatsBoard";
...
export type PoolView = {
  ...
  fullUsd: number;
  allBonded: boolean;
  /** Real dollars already paid out (config s5_paid_ledger via the snapshot).
 ...
 * is visible, and the in-scene hit-regions (.s5hq-obj--*) are percentages of
 * this box, literally coordinates on the painting. A plate change (guest
 * Panther swapping to the picked tank once /api/s5/me lands) crossfades over
...
function CampBackdrop({
  ...
  return (
    <div aria-hidden className={`s5hq-bg${broken ? " s5hq-bg--fallback" : ""}`}>
      {!broken ? (
        <div className="s5hq-plate-wrap">
          {prevPlate ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={prevPlate} alt="" aria-hidden className="s5hq-plateart" />
          ) : null}
          <SafeArt
            key={plate}
            src={plate}
            alt={dict.hq.campAlt}
            className={`s5hq-plateart${prevPlate ? " s5hq-plateart--fade" : ""}`}
            testId="hq-plate"
      ...
      {loopOn ? (
        sprint ? <div aria-hidden className="s5hq-sprinttint" /> : null
      ) : (
      ...
      <div className={`s5hq-phase${phase ? ` s5hq-phase--${phase}` : ""}`} />
      <div className="s5hq-lamp" />
      <div className="s5hq-vig" />
      <div className="s5hq-scrim" />
    </div>
  );
}
...
function TankSilhouette() {
  return (
    <div aria-hidden className="s5hq-sil">
      <div className="s5hq-sil-tracks" />
      <div className="s5hq-sil-hull" />
      <div className="s5hq-sil-turret" />
      <div className="s5hq-sil-barrel" />
    </div>
  );
}
...
function TankArt({ tank, className }: { tank: ResolvedTank; className?: string }) {
  ...
  return (
    <div className={`s5hq-tankart${className ? ` ${className}` : ""}`}>
      <div aria-hidden className="s5hq-tankshadow" />
      {!artBroken ? (
        <SafeArt
          key={tank.art}
          src={tank.art}
          alt=""
          className="s5hq-tankimg"
          testId="hq-tank"
...
/** Tier / name / camo, on its own dark chip so it reads over wet mud. */
function TankCaption({ tank, dict, className }: { tank: ResolvedTank; dict: S5Dict; className?: string }) {
  return (
    <div className={`s5hq-tankplate${className ? ` ${className}` : ""}`}>
      <div className="s5hq-tankplate-tier">
        {fill(dict.hq.tierClassLine, { tier: tank.tier, hull: tank.hullName })}
      </div>
      <div className="s5hq-tankplate-name">{tank.tankName}</div>
      <div className="s5hq-tankplate-camo">{fill(dict.hq.camoLine, { camo: dict.camo[tank.camo] })}</div>
    </div>
  );
}

/** The stacked art + caption used by the mobile band. */
function TankRig({ tank, dict }: { tank: ResolvedTank; dict: S5Dict }) {
  return (
    <div className="s5hq-rig">
      <TankArt tank={tank} />
      <TankCaption tank={tank} dict={dict} />
    </div>
  );
}

function TrophyShelf({ trophies, dict, compact }: { trophies: TrophiesView | null; dict: S5Dict; compact?: boolean }) {
  const t = trophies;
...
function SceneCommander({ ck, label, onOpen }: { ck: string; label: string; onOpen: () => void }) {
  ...
  return (
    ...
    >
      <span aria-hidden className="s5hq-cmdr-shadow" />
      <SafeArt
        key={png}
        src={png}
        alt=""
        className="s5hq-cmdr-img"
        onBroken={() => setBrokenSrc(png)}
      />
      <span aria-hidden className="s5hq-cmdr-light" />
    </button>
  );
}
 ...
 */
function LandingBand({ dict, session }: { dict: S5Dict; session: boolean }) {
  ...
  const steps: Array<[string, string, string?, string?]> = [
    [L.s1t, L.s1b, "/s5/play", L.gamesCta],
    [L.s2t, L.s2b, "/s5/join", L.ctaPlay],
    [L.s3t, L.s3b, "/s5#wizard", L.s3Cta],
  ];
  return (
    <section className="s5lb" id="s5-landing" data-testid="hq-landing">
      <div className="s5lb-in">
        <p className="s5lb-kicker">{L.welcomeKicker}</p>
        <h2 className="s5lb-title">{L.welcomeTitle}</h2>
        <p className="s5lb-lead">{L.welcomeBody}</p>

        {/* RETURNING PLAYERS are the most certain day-one cohort and the one
            we had nothing for: they arrive expecting TEAMS (S5 is solo,
        ...
        <div className="s5lb-vets" id="s5-vets" data-testid="landing-vets">
          <p className="s5lb-vets-h">{L.vetsTitle}</p>
          <ul className="s5lb-vets-list">
            {[L.vets1, L.vets2, L.vets3, L.vets4].map((t, i) => (
              <li key={i}>{t}</li>
            ))}
          </ul>
        </div>
        <div className="s5lb-cta">
          <Link href="/s5/play" className="s5lb-btn s5lb-btn--primary" data-testid="landing-play">
            {L.gamesCta}
          </Link>
          {!session ? (
            <Link href="/s5/join" className="s5lb-btn" data-testid="landing-join">
              {L.ctaPlay}
            </Link>
          ) : null}
        </div>

        <h3 className="s5lb-h">{L.howTitle}</h3>
        <div className="s5lb-grid">
          {steps.map(([t, b, href, cta]) => (
            <div className="s5lb-card" key={t}>
              <h4 className="s5lb-ct">{t}</h4>
              <p className="s5lb-cb">{b}</p>
              {href && cta ? (
                <Link href={href} className="s5lb-cardlink" data-testid={`landing-step-${href.replace(/\W+/g, "")}`}>
        ...
        <div className="s5lb-card s5lb-card--wide">
          <h4 className="s5lb-ct">{L.payTitle}</h4>
          <p className="s5lb-cb">{L.payBody}</p>
        </div>

        <h3 className="s5lb-h">{L.gamesTitle}</h3>
        <p className="s5lb-sub">{L.gamesBody}</p>
        <div className="s5lb-grid s5lb-grid--4">
          {GAMES.filter((g) => !g.comingSoon).map((g) => (
            <Link className="s5lb-card s5lb-card--link" key={g.key} href={`/s5/games/${g.key}`}>
              <h4 className="s5lb-ct">{g.name}</h4>
              <p className="s5lb-cb">
        ...
        <h3 className="s5lb-h">{L.mapTitle}</h3>
        <p className="s5lb-sub">{L.mapBody}</p>
        <Link href="/s5/map" className="s5lb-btn" data-testid="landing-map">
          {L.mapCta}
        </Link>
      </div>
    </section>
...
function CommanderSays({ dict, guest, raidSoon }: { dict: S5Dict; guest: boolean; raidSoon: boolean }) {
  ...
  if (!text) return null;
  return (
    <div className="s5hq-says" data-testid="hq-commander-says" aria-hidden key={text}>
      {text}
    </div>
  );
}
...
function SceneObjects({
  ...
  return (
    <>
      ...
      <button
        className="s5hq-obj s5hq-obj--tank"
        aria-label={spotText(HOTSPOT_BY_KEY.tank).label}
        onClick={() => onNav(HOTSPOT_BY_KEY.tank)}
      >
        <span className="s5hq-obj-lbl">{dict.hq.tagTank}</span>
      </button>
      ...
      <button
        className={`s5hq-obj s5hq-obj--label s5hq-obj--map${sprintOn ? " s5hq-obj--alert" : ""}`}
        data-testid="sky-map"
        aria-label={spotText(HOTSPOT_BY_KEY.maptable).label}
        onClick={() => onNav(HOTSPOT_BY_KEY.maptable)}
      >
        <span className="s5hq-obj-tag">{dict.hq.tagMap}</span>
      </button>
      ...
      <button
        className="s5hq-obj s5hq-obj--label s5hq-obj--arcade"
        data-testid="sky-arcade"
        aria-label={spotText(HOTSPOT_BY_KEY.arcade).label}
        onClick={() => onNav(HOTSPOT_BY_KEY.arcade)}
      >
        <span className="s5hq-obj-tag">{dict.nav.arcade}</span>
      </button>
      <button
        className="s5hq-obj s5hq-obj--label s5hq-obj--howto"
        data-testid="sky-how"
        onClick={() => onRoute("/s5/how-to-play", "scene-how-to-play")}
      >
        <span className="s5hq-obj-tag">{dict.links.howToPlay}</span>
      </button>
      {/* Every other station. The left rail only exists above 1400px and the
          ...
          them at all. */}
      <button
        className="s5hq-obj s5hq-obj--label s5hq-obj--more"
        data-testid="sky-more"
        aria-haspopup="dialog"
        onClick={onMore}
      >
        <span className="s5hq-obj-tag">{dict.hq.moreLabel}</span>
      </button>
      <button
        className="s5hq-obj s5hq-obj--label s5hq-obj--upgrades"
        aria-label={spotText(HOTSPOT_BY_KEY.workbench).label}
        onClick={() => onNav(HOTSPOT_BY_KEY.workbench)}
      >
        <span className="s5hq-obj-tag">{dict.hq.tagUpgrades}</span>
      </button>
    </>
  );
}
...
export function HqScene({
  ...
  targets?: TargetLink[];
  /** PREVIEW OVERRIDES (the /s5/hq/preview art gallery): force a specific tank
  ...
  // DEEP LINK: /s5#wizard opens the funding wizard on arrival (the how-to-play
  ...
  const [board, setBoard] = useState<{ stats: StatsView; payout: PayoutBoardView } | null>(null);
  ...
  const activeCommander =
    (forceCommander && commanderByKey(forceCommander) ? forceCommander : null) ||
    (commanderByKey(me.commander) ? me.commander : null) ||
    DEFAULT_COMMANDER_KEY;

  // The camp painting for the bay tank (guest = the Panther hero portrait;
  // once /api/s5/me lands with a picked tank, CampBackdrop crossfades to that
  ...
  // RESOLVE_HOUR_UTC, mirrored in lib/s5/raid.ts). Client-only mount: null
  ...
  useEffect(() => {
    ...
    // useHqMe); on /s5/hq the scene ran this fetch, found no session token,
    ...
    try {
      ...
      void fetch("/api/s5/me", {
        ...
        .then((resp) => {
          ...
          if (resp.stats && resp.payout) setBoard({ stats: resp.stats, payout: resp.payout });
          ...
          setMe((prev) => ({
            ...
            commander: resolveCommanderKey({ commander: resp.hq?.commander }),
            ownedCamos: Array.isArray(resp.hq?.ownedCamos)
              ? resp.hq.ownedCamos.filter((c): c is string => typeof c === "string")
              : prev.ownedCamos,
            ownedCommanders: Array.isArray(resp.hq?.ownedCommanders)
              ? resp.hq.ownedCommanders.filter((c): c is string => typeof c === "string")
              : prev.ownedCommanders,
  ...
  // Localized station copy for one hotspot (en mirrors lib/s5/hq.ts).
  const spotText = useCallback(
    (spot: Hotspot) => dict.hotspots[spot.key] ?? { label: spot.label, flavor: spot.flavor },
    [dict],
  );
  ...
  // as the on-stage overlay (.s5hq-hud, both stages under 1024px) AND as a
  // rail card beside the big desktop stage (.s5hq-railcard s5hq-railhud, the
  ...
  const callsign =
    me.session || forceCommander
      ? commanderByKey(activeCommander)?.name ?? dict.hq.hudGuest
      : dict.hq.hudGuest;
  const hudPoolShort =
  ...
  const renderHud = (variant: "stage" | "rail" = "stage") => (
    <div className={variant === "stage" ? "s5hq-hud" : "s5hq-railcard s5hq-railhud"} aria-hidden>
      <span className="s5hq-hud-chip s5hq-hud-callsign">{callsign}</span>
      {me.session ? (
        <span className="s5hq-hud-stats">
          <span className="s5hq-hud-chip">
            <b>{me.shells.toLocaleString("en-US")}</b> {dict.hq.hudShells}
          </span>
          <span className="s5hq-hud-chip">
            <b>{me.points.toLocaleString("en-US")}</b> {dict.hq.hudMedals}
          </span>
        </span>
      ) : (
        <span className="s5hq-hud-chip s5hq-hud-enlist">{dict.hq.hudEnlist}</span>
      )}
      ...
      {raidCountdown ? (
        <span
          className="s5hq-hud-chip s5hq-hud-raid"
      ...
      {hudPoolShort ? (
        <span className={`s5hq-hud-pill${hudLocked ? " s5hq-hud-pill--warn" : ""}`}>
  ...
  const dockButtons: Array<{ label: string; icon: string; onTap: () => void }> = [
    { label: dict.hq.dockArcade, icon: "▶", onTap: () => router.push("/s5/play") },
    { label: dict.hq.dockMap, icon: "✚", onTap: () => router.push("/s5/map") },
    { label: dict.links.howToPlay, icon: "?", onTap: () => router.push("/s5/how-to-play") },
    me.session
      ? { label: dict.nav.board, icon: "▦", onTap: () => router.push("/s5/board") }
      : { label: dict.links.enlist, icon: "⚑", onTap: () => router.push("/s5/join") },
    { label: dict.hq.moreLabel, icon: "⋯", onTap: openMore },
  ];

  return (
    /* No nav padding here: /s5 (the only mount) already clears the fixed 52px
       nav with the raid strip above, and doubling it left a dead band under
       the strip. */
    /* <main>, not <div>: /s5 is the highest-traffic page and was the only S5
       route without a main landmark (2026-07-27 audit). The id is the skip
       link's target. */
    <main
      id="s5-content"
      /* The ground now lives on .s5-ground in the S5 layout so it runs behind
    ...
    >
      ...
      <header className="s5hq-head" style={{ textAlign: "center", padding: "22px 16px 6px" }}>
        ...
        <p style={{ fontSize: 12.5, color: FAINT, margin: "6px auto 0", maxWidth: 620, lineHeight: 1.5 }}>
          {dict.hq.vetLine}{" "}
          <a href="#s5-vets" style={{ color: EMBER, textDecoration: "underline", textUnderlineOffset: 2 }}>
            {dict.hq.vetLink}
          </a>
        </p>
        ...
        >
          ...
          <a href="#s5-landing" style={{ color: MUTED, textDecoration: "underline", textUnderlineOffset: 2 }}>
            {dict.hq.step3}
          </a>
        </p>
        <p style={{ fontSize: 12.5, color: FAINT, margin: "10px auto 0", maxWidth: 560 }}>{poolText}</p>

        {/* L6 PAID TICKER: the s5_paid_ledger running total, stated as fact.
        ...
        {pool && !pool.empty ? (
          <p className="s5hq-headtick" style={{ margin: "10px auto 0" }}>
            <span
        ...
        {sprintOn ? (
          <p className="s5hq-sprint" role="status" aria-label={dict.sprint.aria} data-testid="hq-sprint">
            <span className="s5hq-sprint-tag" aria-hidden>
              {dict.sprint.tag}
            </span>
            <span className="s5hq-sprint-text">{fill(dict.sprint.hqLine, { domain: sprintNames })}</span>
          </p>
        ) : null}
      ...
      <div className="s5hq-desktop">
        <div className="s5hq-shell">
          ...
          <aside className="s5hq-lrail" aria-label={dict.hq.moreTitle}>
            <div className="s5hq-railcard">
              <p className="s5hq-lrail-title">{dict.hq.moreTitle}</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {LRAIL_KEYS.map((k) => {
                  ...
                  return (
                    <button
                      key={k}
                      className="s5hq-morerow"
                      data-testid={`lrail-${k}`}
                      onClick={() => handleNav(spot)}
                    >
                      <span className="s5hq-morerow-lbl">{t.label}</span>
                      <span className="s5hq-morerow-flavor">{t.flavor}</span>
                    </button>
                  );
                })}
              </div>
          ...
          <div
            className={`s5hq-stage${sprintOn ? " s5hq-stage--sprint" : ""}`}
            role="group"
          ...
          >
            ...
            />
            {renderHud()}
            <div className="s5hq-frameline" aria-hidden />
            <SceneObjects
              spotText={spotText}
              dict={dict}
              onNav={handleNav}
            ...
            <SceneCommander
              ck={activeCommander}
              label={spotText(HOTSPOT_BY_KEY.commander).label}
              onOpen={() => handleNav(HOTSPOT_BY_KEY.commander)}
            />
          ...
          <aside className="s5hq-rail">
            {renderHud("rail")}
            {!me.session ? (
              <Link
                href="/s5/join"
                className="s5hq-joincta"
                data-testid="hq-join-cta"
                onClick={() => track("cta_click", { ref: "rail-join" })}
              >
                <span className="s5hq-joincta-main">{dict.hq.joinCtaMain}</span>
                <span className="s5hq-joincta-sub">{dict.hq.joinCtaSub}</span>
              </Link>
            ) : null}
            {!me.session ? (
              <Link
                href="/s5/play"
                className="s5hq-playcta"
                data-testid="hq-play-cta"
                onClick={() => track("cta_click", { ref: "rail-play" })}
              >
                <span className="s5hq-playcta-main">{dict.hq.playCtaMain}</span>
                <span className="s5hq-playcta-sub">{dict.hq.playCtaSub}</span>
              </Link>
            ) : null}
            <DailyOrders me={me} dict={dict} />
            {pool && !pool.empty ? (
              <div className="s5hq-railcard" data-testid="hq-paid-rail">
                <span
                  style={{
                    fontFamily: "ui-monospace, Menlo, monospace",
                    fontSize: 11.5,
      ...
      <div className="s5hq-mobile">
        <section className="s5hq-band">
          <TrophyShelf trophies={trophies} dict={dict} compact />
        </section>
        <section className="s5hq-band" style={{ paddingTop: 18, paddingBottom: 18 }}>
          ...
          <div className="s5hq-rigframe">
            <TankRig tank={bayTank} dict={dict} />
          </div>
        </section>
        <section className="s5hq-band s5hq-band--stage">
          <div
            className={`s5hq-mstage${sprintOn ? " s5hq-stage--sprint" : ""}`}
            role="group"
            aria-label={dict.hq.stageAria}
          >
            <CampBackdrop
        ...
        {/* THE EXPLAINER ON PHONES (2026-07-27 audit finding F1): LandingBand
            used to live ONLY inside .s5hq-desktop, which is display:none below
        ...
        <section className="s5hq-band" style={{ paddingTop: 4 }}>
          <LandingBand dict={dict} session={!!me.session} />
        </section>
        <div style={{ height: 84 }} />
        <nav className="s5hq-dock" aria-label={dict.hq.dockAria}>
          {dockButtons.map((b) => (
            <button key={b.label} className="s5hq-dockbtn" onClick={b.onTap} aria-label={b.label}>
      ...
      {panel ? (
        <div
          className="s5hq-overlay"
          role="dialog"
          aria-modal="true"
          aria-labelledby="s5hq-panel-title"
          onClick={() => setPanel(null)}
        >
          <div
            ref={panelRef}
            className={`s5hq-panel${panel === "tank" ? " s5hq-panel--wide" : ""}`}
          ...
          >
            <PanelBody
              ...
              onHotspot={handleNav}
            />
            <button className="s5hq-open" style={{ marginTop: 16 }} onClick={() => setPanel(null)}>
              {dict.common.back}
            </button>
          </div>
        </div>
...
function PanelBody({
  ...
  const H = ({ children }: { children: React.ReactNode }) => (
    ...
    <h2 id="s5hq-panel-title" style={{ fontSize: 17, fontWeight: 800, margin: "0 0 10px", color: TEXT }}>
      {children}
    </h2>
  );
  ...
  switch (panel) {
    case "tank":
      return <TankPanel me={me} patchMe={patchMe} onOpenWizard={openWizard} dict={dict} />;
    case "commander":
      return <CommanderPanel me={me} patchMe={patchMe} dict={dict} />;
    case "workbench":
      return <WorkbenchPanel me={me} patchMe={patchMe} dict={dict} />;
    case "footlocker":
    case "quests":
      ...
      return (
        <div data-testid="more-sheet">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <Link href="/s5/rules" className="s5hq-morerow" data-testid="more-rules">
              <span className="s5hq-morerow-lbl">{dict.hq.rulesCta}</span>
            </Link>
          </div>
        </div>
      );
...
const HQ_CSS = `
.s5hq-desktop { display: none; }
.s5hq-mobile { display: block; }
@media (min-width: 760px) {
  .s5hq-desktop { display: block; }
  .s5hq-mobile { display: none; }
}
/* ── THE STAGE = THE PLATE ──
   Both stages carry camp-portrait.webp at its OWN 3:4 aspect, so the whole
   painting is on screen; the in-scene hit-regions (.s5hq-obj--*) below are
...
.s5hq-rail { display: none; }
@media (min-width: 760px) and (max-width: 1023.98px) {
  .s5hq-shell { flex-wrap: wrap; }
  .s5hq-rail {
    display: flex;
...
.s5hq-joincta:hover { filter: brightness(1.07); }
.s5hq-playcta {
...
.s5hq-playcta:hover { background: rgba(154,167,180,0.16); }
.s5hq-playcta:focus-visible { outline: 2px solid ${EMBER}; outline-offset: 3px; }
.s5hq-playcta-main { display: block; color: ${TEXT}; font-weight: 800; font-size: 14.5px; }
.s5hq-playcta-sub { display: block; color: ${MUTED}; font-size: 11.5px; font-weight: 600; margin-top: 2px; }
.s5lb-vets {
...
.s5lb-vets-list { margin: 0; padding-left: 18px; display: grid; gap: 6px; }
.s5lb-vets-list li { font-size: 13px; color: ${MUTED}; line-height: 1.55; }
.s5hq-joincta:focus-visible { outline: 2px solid #fff6ee; outline-offset: 3px; }
.s5hq-joincta-main {
...
.s5hq-railhud { display: flex; flex-wrap: wrap; align-items: center; gap: 8px 12px; }
.s5hq-railhud .s5hq-hud-pill { margin-left: 0; }
@media (min-width: 1024px) {
  /* MEASURED, then fixed (2026-07-28). NOTE: no backticks in this comment --
     ...
     The --s5-chrome var is the nav + title block above the stage. */
  .s5hq-head { padding-top: 10px !important; padding-bottom: 2px !important; }
  .s5hq-head h1 { margin-bottom: 2px !important; }
  .s5hq-stage {
    --s5-chrome: 208px;
    height: min(calc(100vh - var(--s5-chrome)), 760px);
    width: auto;
  ...
  .s5hq-desktop .s5hq-hud { display: none; }
  .s5hq-headtick { display: none; }
}
/* The phone stage runs edge to edge: every pixel of width goes to the
   painting. */
.s5hq-mobile .s5hq-band--stage { padding-left: 0; padding-right: 0; }
.s5hq-mstage {
  position: relative;
...
@media (prefers-reduced-motion: no-preference) {
  .s5hq-lamp { animation: s5hq-lantern 7s ease-in-out infinite; }
}
@keyframes s5hq-lantern {
  0%, 100% { opacity: 0.86; }
  50% { opacity: 1; }
}
...
.s5hq-bg--fallback .s5hq-lamp { display: none; }

/* ââ TANK ART (the mobile band's rig + the shared silhouette fallback) ââ */
.s5hq-tankart { position: relative; width: 100%; aspect-ratio: 900 / 603; }
.s5hq-tankimg {
  position: relative;
  display: block;
  width: 100%; height: 100%;
...
.s5hq-sil { position: absolute; inset: 0; }
.s5hq-sil-hull {
...
.s5hq-rig { display: flex; flex-direction: column; align-items: center; }
.s5hq-tankplate {
  text-align: center;
...
.s5hq-tankplate-name { font-size: clamp(13px, 1.9vw, 17px); font-weight: 800; color: ${TEXT}; }
.s5hq-tankplate-camo { font-size: clamp(9.5px, 1.25vw, 11px); color: ${FAINT}; text-transform: capitalize; }
.s5hq-band { padding: 14px 16px; }
.s5hq-mobile .s5hq-tankart { width: min(78vw, 340px); margin: 0 auto; }
.s5hq-open {
  margin-top: 10px;
  padding: 9px 16px;
  border-radius: 8px;
...
.s5hq-dockbtn:active { border-color: ${STEEL}55; color: ${TEXT}; }
.s5hq-overlay {
...
.s5fr-counters { display: flex; gap: 14px; font-size: 12.5px; color: ${MUTED}; font-weight: 700; }
.s5fr-guest { padding: 22px 18px; font-size: 13px; color: ${MUTED}; line-height: 1.6; margin: 0; }
.s5fr-grid { display: grid; grid-template-columns: 1fr; }
@media (min-width: 760px) { .s5fr-grid { grid-template-columns: 1fr 1fr 1.15fr; } }
.s5fr-cell { padding: 16px 18px; border-top: 1px solid ${BORDER}; }
@media (min-width: 760px) {
  .s5fr-cell { border-top: 0; }
  .s5fr-cell + .s5fr-cell { border-left: 1px solid ${BORDER}; }
}
.s5fr-h {
  margin: 0 0 10px;
...
.s5fr-tankname { margin: 0 0 10px; font-size: 15.5px; font-weight: 800; color: ${TEXT}; }
.s5fr-tier { font-size: 11.5px; font-weight: 700; color: ${STEEL}; }
.s5fr-rating { display: flex; align-items: center; gap: 8px; margin: 6px 0; }
.s5fr-rating-name { width: 72px; font-size: 11px; color: ${MUTED}; }
.s5fr-rating-track {
...
.s5fr-rating-num { width: 18px; text-align: right; font-size: 11px; color: ${FAINT}; font-variant-numeric: tabular-nums; }
.s5fr-stat { display: flex; align-items: center; gap: 8px; margin: 7px 0; }
.s5fr-stat-ico { width: 18px; text-align: center; font-size: 13px; }
.s5fr-stat-name { width: 64px; font-size: 11.5px; color: ${MUTED}; }
.s5fr-pips { display: flex; gap: 4px; }
.s5fr-pip {
...
.s5fr-cell--pay { background: rgba(9,11,14,0.45); }
.s5fr-est {
  margin: 0;
  font-family: ui-monospace, Menlo, monospace;
...
.s5fr-estlabel { margin: 2px 0 0; font-size: 11.5px; color: ${MUTED}; }
.s5fr-share { margin: 8px 0 0; font-size: 12px; font-weight: 700; color: ${STEEL}; }
.s5fr-disclaimer { margin: 10px 0 0; font-size: 10.5px; color: ${FAINT}; line-height: 1.5; }
.s5fr-warn {
...
.s5fr-warn-ico { font-size: 16px; }
.s5fr-warn-text { flex: 1 1 260px; font-size: 12.5px; color: ${TEXT}; line-height: 1.5; }
.s5fr-warn-text strong { color: ${EMBER}; font-variant-numeric: tabular-nums; }
.s5fr-warn-names { color: ${MUTED}; }
.s5fr-warn-cta {
  flex: 0 0 auto;
  padding: 8px 14px;
  border-radius: 9px;
...
.s5fr-warn-cta:active { transform: translateY(1px); }
...
.s5hq-sprint-text { flex: 1 1 240px; font-size: 12px; color: ${TEXT}; line-height: 1.55; }
@media (prefers-reduced-motion: no-preference) {
  .s5hq-sprint-tag { animation: s5hq-klaxon 2.2s ease-in-out infinite; }
}
@keyframes s5hq-klaxon {
  0%, 100% { opacity: 1; }
  50% { opacity: 0.45; }
}
/* The camp itself: a warning wash on the frame, never over the painting's
   readability (the border and a soft inner glow only). */
.s5hq-stage--sprint { border-color: ${EMBER}66; }
.s5hq-stage--sprint::before {
  content: "";
  position: absolute; inset: 0;
  pointer-events: none;
  z-index: 3;
...
/* The Field Report strip (same steel plate grammar as .s5fr-warn). */
.s5fr-sprint {
  display: flex; align-items: center; gap: 10px; flex-wrap: wrap;
  padding: 12px 18px;
...
.s5fr-sprint-ico { font-size: 16px; }
.s5fr-sprint-text { flex: 1 1 260px; font-size: 12.5px; color: ${TEXT}; line-height: 1.5; }
.s5fr-sprint-text strong { color: ${EMBER}; }
.s5fr-sprint-note { color: ${MUTED}; }
.s5fr-sprint-cta {
  flex: 0 0 auto;
  padding: 8px 14px;
  border-radius: 9px;
...
.s5fr-sprint-cta:active { transform: translateY(1px); }
...
.s5hq-plate-wrap { position: absolute; inset: 0; }
.s5hq-video {
  position: absolute; inset: 0;
  width: 100%; height: 100%;
...
.s5hq-sprinttint {
  ...
  mix-blend-mode: screen;
  animation: s5hq-sprintpulse 3.2s ease-in-out infinite;
}

@keyframes s5hq-sprintpulse {
  0%, 100% { opacity: 0.6; }
  50% { opacity: 1; }
}
@media (prefers-reduced-motion: reduce) {
  .s5hq-sprinttint { animation: none; opacity: 0.8; }
}
...
@media (prefers-reduced-motion: no-preference) {
  .s5hq-plateart--fade { animation: s5hq-platefade 250ms ease-out both; }
}
@keyframes s5hq-platefade {
  from { opacity: 0; }
  to { opacity: 1; }
}

/* ââ UTC DAY-PHASE: the camp follows the real clock. SSR renders the bare
   .s5hq-phase (opacity 0 = fully neutral); the client stamps day / dusk /
...
.s5hq-phase--night { opacity: 1; background: rgba(62,92,158,0.10); }
.s5hq-phase--dusk { opacity: 1; background: rgba(224,140,70,0.05); }
.s5hq-phase--night ~ .s5hq-lamp { filter: brightness(1.18); }
...
.s5hq-hudcmdr:hover, .s5hq-hudcmdr:focus-visible {
  outline: none;
  border-color: ${EMBER}aa;
  box-shadow: 0 0 20px ${EMBER}44;
  transform: translateY(-1px);
}
.s5hq-hudcmdr:active { transform: translateY(0); }

/* ── THE LANDING BAND ─────────────────────────────────────────────────────── */
.s5lb { padding: 34px 16px 64px; }
.s5lb-in { max-width: 980px; margin: 0 auto; }
.s5lb-kicker {
  font-family: ui-monospace, Menlo, monospace;
  font-size: 11px; font-weight: 700; letter-spacing: 0.18em;
  color: ${EMBER}; margin: 0 0 6px;
}
.s5lb-title { font-size: clamp(26px, 4.4vw, 40px); line-height: 1.1; margin: 0 0 10px; }
.s5lb-lead { font-size: 16px; line-height: 1.55; color: #cfd6dd; max-width: 62ch; margin: 0 0 18px; }
.s5lb-cta { display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 34px; }
.s5lb-btn {
...
.s5lb-btn:hover { border-color: ${EMBER}88; }
.s5lb-btn--primary { background: ${EMBER}; border-color: ${EMBER}; color: #14100c; }
.s5lb-h { font-size: 20px; margin: 30px 0 4px; }
.s5lb-sub { color: ${MUTED}; margin: 0 0 14px; font-size: 14px; }
.s5lb-grid { display: grid; grid-template-columns: 1fr; gap: 12px; }
@media (min-width: 700px) {
  .s5lb-grid { grid-template-columns: repeat(3, 1fr); }
  .s5lb-grid--4 { grid-template-columns: repeat(4, 1fr); }
}
.s5lb-card {
...
.s5lb-card--wide { margin-top: 12px; }
.s5lb-card--link { text-decoration: none; color: inherit; display: block; }
.s5lb-card--link:hover { border-color: ${EMBER}88; }
.s5lb-ct { margin: 0 0 5px; font-size: 15px; }
.s5lb-cb { margin: 0; font-size: 13.5px; line-height: 1.5; color: #c3ccd4; }
...
.s5lb-cardlink:hover, .s5lb-cardlink:focus-visible { text-decoration: underline; }
/* Global keyboard ring: most S5 controls are styled inline and had NO
   :focus-visible rule, leaving only the UA default (unreliable on dark in
   Safari). Scoped to the S5 tree by the surrounding stylesheet. */
...
.s5hq-cmdr:hover .s5hq-cmdr-img, .s5hq-cmdr:focus-visible .s5hq-cmdr-img {
...
@media (prefers-reduced-motion: no-preference) {
  .s5hq-cmdr { animation: s5hq-cmdrbreathe 4.6s ease-in-out infinite; }
}
...
.s5hq-cmdr:focus-visible { outline: 2px solid ${EMBER}; outline-offset: 3px; border-radius: 10px; }
/* ── SHE SAYS SOMETHING ──────────────────────────────────────────────────────
...
@media (prefers-reduced-motion: no-preference) {
  .s5hq-says { animation: s5hq-saysin .45s ease both; }
}
.s5hq-says::after {
  content: "";
  position: absolute;
...
@media (max-width: 1023.98px) { .s5hq-says { display: none; } }
...
.s5hq-cmdr:active .s5hq-cmdr-img { transform: translateY(1px); }

/* The raid countdown chip: same HUD grammar, the digits hold their width. */
.s5hq-hud-raid { white-space: nowrap; }
.s5hq-hud-raid b { font-variant-numeric: tabular-nums; }
...
/* (The no-hover always-visible label override lives at the END of this
   stylesheet: it must come after the .s5hq-obj rules it overrides.) */
...
.s5hq-hud-chip b { color: ${TEXT}; font-variant-numeric: tabular-nums; }
.s5hq-hud-callsign { color: ${STEEL}; font-weight: 800; }
.s5hq-hud-callsign::before { content: "▸ "; color: ${EMBER}; }
.s5hq-hud-stats { display: flex; gap: 12px; }
.s5hq-hud-enlist { color: ${EMBER}; font-weight: 700; }
.s5hq-hud-pill {
  margin-left: auto;
  font-family: ui-monospace, Menlo, monospace;
...
.s5hq-hud-pill--warn { background: ${EMBER}1c; border-color: ${EMBER}66; color: #f4c9ac; }
...
.s5hq-frameline::before, .s5hq-frameline::after {
  content: ""; position: absolute; width: 22px; height: 22px; border: 2px solid ${STEEL}55;
}
.s5hq-frameline::before { top: -1px; left: -1px; border-right: 0; border-bottom: 0; border-radius: 10px 0 0 0; }
.s5hq-frameline::after { bottom: -1px; right: -1px; border-left: 0; border-top: 0; border-radius: 0 0 10px 0; }
...
.s5hq-morerow:hover { border-color: ${STEEL}66; background: rgba(255,255,255,0.045); }
.s5hq-morerow:focus-visible { outline: 2px solid ${EMBER}; outline-offset: 2px; }
.s5hq-morerow-lbl { font-size: 13px; font-weight: 800; color: ${TEXT}; }
.s5hq-morerow-flavor { font-size: 11.5px; color: ${FAINT}; line-height: 1.5; }
...
.s5hq-obj--tank { left: 56%; top: 40%; transform: translate(-50%, -50%); width: 64%; height: 44%; }
/* Labeled crate/board objects = the menu, always-visible labels + hover glow. */
.s5hq-obj--map { left: 13%; top: 44%; transform: translate(-50%, -50%); width: 24%; height: 15%; }
.s5hq-obj--upgrades { left: 12%; top: 85%; transform: translate(-50%, -50%); width: 21%; height: 11%; }
...
.s5hq-obj--arcade { left: 19%; top: 14%; transform: translate(-50%, -50%); width: 22%; height: 12%; }
/* Bottom CENTRE, not bottom right: at 78% the manual tag sat squarely on the
   standing commander (measured 68-88% x, 67-99% y). */
.s5hq-obj--howto { left: 46%; top: 88%; transform: translate(-50%, -50%); width: 24%; height: 10%; }
.s5hq-obj--more { left: 94.5%; top: 6.5%; transform: translate(-50%, -50%); width: 10%; height: 7%; }
.s5hq-obj--more .s5hq-obj-tag { color: ${MUTED}; font-size: 9px; letter-spacing: 0.1em; }
/* The map board pulses when a sprint is live: the alert moved here with it. */
.s5hq-obj--alert { border-color: ${EMBER}cc; box-shadow: 0 0 22px ${EMBER}66; }
@media (prefers-reduced-motion: no-preference) {
  .s5hq-obj--alert { animation: s5hq-klaxon 2.2s ease-in-out infinite; }
}
.s5hq-obj--label { border-color: rgba(255,255,255,0.1); border-radius: 8px; }
.s5hq-obj--label:hover, .s5hq-obj--label:focus-visible {
...
.s5hq-obj--label:hover .s5hq-obj-tag, .s5hq-obj--label:focus-visible .s5hq-obj-tag { color: #fff; border-color: ${EMBER}88; }
.s5hq-obj--arcade .s5hq-obj-tag,
.s5hq-obj--more .s5hq-obj-tag { bottom: 50%; transform: translate(-50%, 50%); }
.s5hq-obj:hover, .s5hq-obj:focus-visible {
  border-color: ${EMBER}aa;
...
.s5hq-obj:hover .s5hq-obj-lbl, .s5hq-obj:focus-visible .s5hq-obj-lbl { opacity: 1; }
/* No hover on a phone: the tank / commander hover labels become always-
   ...
   out-cascade the base .s5hq-obj rules above. */
@media (max-width: 759.98px) {
  .s5hq-obj-lbl { opacity: 1; bottom: 6px; }
  .s5hq-obj { border-color: rgba(255,255,255,0.08); }
}
`;
