 ...
 *
 * Data flow: HqScene owns one HqMe state (guest defaults until /api/s5/me
 * lands) and hands panels `me` + `patchMe`. Writes go through:
 *   /api/s5/hq           field tank / camo / commander (cosmetic, free)
 *   /api/s5/tank-unlock  Shells spend (server-priced, fail-closed)
 *   /api/s5/vault-buy    the Armory Vault daily deal (server-priced, fail-closed)
 *   /api/s5/claim-ftue   the First Colors decal (idempotent)
 *   /api/s5/funding-status  the stuck-step detector (fail-soft)
 *
...
import { GAMES, GAME_DAILY_POINTS_CAP, POINTS_PER_RUN, STAT_LABELS, ZERO_STATS, statNextPrice, type PlayerStats, type StatKey } from "@/lib/s5/games";
import { RESOLVE_HOUR_UTC } from "@/lib/s5/raid";
import { breachDecalKey, vaultDayNumber, vaultDealForDay } from "@/lib/s5/vault";
import {
  DOMA_APP_URL,
  DOMA_CHAIN,
  STARGATE_BRIDGE_URL,
...
export function defaultHqMe(): HqMe {
  return {
    ...
    ownedCamos: ["olive"],
    ownedCommanders: [],
    commander: DEFAULT_COMMANDER_KEY,
    bondsTier: 0,
    streakDays: 0,
    crates: [],
    stats: { ...ZERO_STATS },
...
export function TankPanel({
  ...
  const field = useCallback(
    async (key: string) => {
      ...
      setBusyKey(key);
      setNote(null);
      const r = await postJson("/api/s5/hq", { t: me.token, set: { tank: key } });
      setBusyKey(null);
      if (r.ok && r.tank) {
        patchMe({
  ...
  const unlock = useCallback(
    async (t: Tank) => {
      ...
      setBusyKey(t.key);
      setNote(null);
      const r = await postJson("/api/s5/tank-unlock", { t: me.token, tank: t.key });
      setBusyKey(null);
      if (r.ok && r.tank) {
        patchMe({
          tank: r.tank as ResolvedTank,
  ...
  return (
    <div data-testid="tank-panel">
      ...
      {me.session && me.handle ? (
        <a
          href={`/s5/hq/${encodeURIComponent(me.handle)}`}
...
/** The living portrait of the currently-selected commander: the shared
...
function CommanderArt({ ck, size }: { ck: string; size: number }) {
  const [broken, setBroken] = useState(false);
  const src = `/s5-art/commander/${ck}.png`;
  if (!broken) {
    return (
...
export function CommanderPanel({
  ...
  const [note, setNote] = useState<{ tone: "good" | "warn" | "bad" | "muted"; text: string } | null>(null);
  const D = dict.panels;
  const cast = dict.commanders as Record<string, { role: string; blurb: string }>;

  const pick = useCallback(
    async (key: string) => {
      if (!me.token) {
        setNote({ tone: "muted", text: D.commanderNeedSession });
        return;
      }
      setBusyKey(key);
      setNote(null);
      const r = await postJson("/api/s5/hq", { t: me.token, set: { commander: key } });
      setBusyKey(null);
      if (r.ok) {
        patchMe({ commander: typeof r.commander === "string" ? r.commander : key });
        setNote({ tone: "good", text: D.commanderSwapped });
      } else {
        setNote({ tone: "bad", text: String(r.error || D.commanderSwapFailed) });
      }
    },
    [me.token, patchMe, D],
  );
    ...
    <div data-testid="commander-panel">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8 }} data-testid="commander-grid">
        {COMMANDERS.map((c) => {
          ...
          const locked = !!c.prize && !me.ownedCommanders.includes(c.key);
          return (
            <button
              key={c.key}
              ...
              onClick={() =>
                isCurrent
                  ? undefined
                  : locked
                    ? setNote({ tone: "muted", text: D.commanderEarnTop })
                    : pick(c.key)
              }
              disabled={busy}
              style={{
            ...
            >
              ...
              <span style={{ minWidth: 0 }}>
                ...
                <span style={{ display: "block", fontSize: 10.5, color: FAINT, lineHeight: 1.4, marginTop: 2 }}>
                  {busy ? D.swapping : locked ? D.commanderEarnTop : (cast[c.key]?.blurb ?? c.blurb)}
                </span>
              </span>
            </button>
          );
...
function camoEarnHint(c: CamoKey, dict: S5Dict): string {
...
export function WorkbenchPanel({
  ...
  const paint = useCallback(
    async (camo: CamoKey) => {
      ...
      setBusyCamo(camo);
      setNote(null);
      const r = await postJson("/api/s5/hq", { t: me.token, set: { camo } });
      setBusyCamo(null);
      if (r.ok && r.tank) {
  ...
  const buyStat = useCallback(
    async (stat: StatKey) => {
      ...
      setBusyStat(stat);
      setNote(null);
      const r = await postJson("/api/s5/upgrade", { t: me.token, stat });
      setBusyStat(null);
      if (r.ok) {
        patchMe({
          ...
          return (
            ...
            >
              ...
              {maxed ? (
                ...
                >
                  {busyStat === k ? D.swapping : `${price!.toLocaleString()} Shells`}
                </button>
              )}
            </div>
          );
...
export function DailyOrders({ me, dict }: { me: HqMe; dict: S5Dict }) {
  ...
  const rows: Array<{ id: string; title: string; detail: string; done: boolean | null; action?: React.ReactNode }> = [
    {
      ...
      action: allFlown ? undefined : (
        <a href="/s5/play" style={{ color: STEEL, fontWeight: 700, textDecoration: "underline", fontSize: 12 }}>
          {R.orderArcadeGo}
        </a>
      ),
    },
...
// would not charge.
function VaultSection({ me, patchMe, dict }: { me: HqMe; patchMe: PatchMe; dict: S5Dict }) {
  ...
  const buy = useCallback(async () => {
    ...
    setBusy(true);
    setNote(null);
    const r = await postJson("/api/s5/vault-buy", { t: me.token, item: deal.key });
    setBusy(false);
    if (r.ok) {
      patchMe({
        tank: {
...
function DecalCollection({ me, targets = [], dict }: { me: HqMe; targets?: TargetLink[]; dict: S5Dict }) {
  const R = dict.retention;
...
function stateText(D: S5Dict["panels"], state: FundingState): string {
  switch (state) {
    case "NO_WALLET":
...
export function FundingWizardPanel({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = me.token
          ? await fetch("/api/s5/funding-status", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
...
export function FootlockerPanel({
  ...
  const claim = useCallback(async () => {
    ...
    setClaiming(true);
    setClaimNote(null);
    const r = await postJson("/api/s5/claim-ftue", { t: me.token });
    setClaiming(false);
    if (r.ok && r.already) {
  ...
  const quests: QuestRow[] = [
    ...
    },
    {
      ...
      action: q2 ? undefined : link("/s5/play", D.q2Action),
    },
    {
      n: 3,
      title: D.q3Title,
      ...
      action: q3 ? undefined : link("/s5/join", D.q3Action),
    },
    {
      n: 4,
      title: D.q4Title,
