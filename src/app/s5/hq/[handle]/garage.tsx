...
import type { PublicGarage } from "@/lib/s5/publicHq";
import { commanderByKey } from "@/lib/s5/tanks";
import { BONDS_TIER_MAX, decalLabel, isKnownDecal } from "@/lib/s5/ftue";
...
function PublicCommanderCard({ commanderKey }: { commanderKey: string }) {
  const c = commanderByKey(commanderKey);
  const [artBroken, setArtBroken] = useState(false);
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 14 }} data-testid="public-commander">
      {!artBroken ? (
        <SafeArt
          key={commanderKey}
          src={`/s5-art/commander/${commanderKey}.png`}
          alt={c?.name || "Commander"}
      ...
      <div>
        <div style={{ fontSize: 15, fontWeight: 800, color: TEXT }}>{c?.name || "Commander"}</div>
...
export function PublicGarageView({
  ...
  const [shareUrl, setShareUrl] = useState(`https://tanks.web3guides.com/s5/hq/${view.handle}`);
  useEffect(() => {
    setShareUrl(`${window.location.origin}/s5/hq/${view.handle}`);
  }, [view.handle]);
...
`,
      ...
      <div className="s5pg-grid">
        ...
        <div className="s5pg-col">
          ...
          <section style={card()} aria-label="Commander">
            <div style={shelfLabel}>Commander</div>
            <PublicCommanderCard commanderKey={view.commanderKey} />
          </section>
        </div>
      ...
      <section style={card({ marginTop: 18, paddingTop: 18, paddingBottom: 18 })} aria-label="Share this garage">
        ...
        <div className="s5pg-cardwrap" style={{ maxWidth: 460, margin: "0 auto 14px" }}>
          ...
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={`/api/s5/hq-card?h=${encodeURIComponent(view.handle)}`}
        ...
        <div className="s5pg-share">
          <a
            ...
            href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(
              `${view.callsign} · ${view.rank} · fielding the ${view.tank.tankName} in Launch Wars IRON SIEGE. Free to play in your browser: ${shareUrl}`,
            )}`}
            target="_blank"
            rel="noreferrer"
