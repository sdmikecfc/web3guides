 ...
 * handles render the themed not-found panel. og:image is the /api/s5/hq-card
 * route so a shared garage unfurls as a card.
 */
import type { Metadata } from "next";
import Link from "next/link";
import { resolvePublicHq } from "@/lib/s5/publicHq";
...
export async function generateMetadata({
  ...
  const title = res ? `${res.view.callsign}'s HQ | Iron Siege` : `Iron Siege | ${DEFAULT_THEME.seasonName}`;
  const description = res
    ? `${res.view.callsign} fields the ${res.view.tank.tankName}. Visit the garage, then build your own HQ. Enlist free.`
    : "Every commander gets a bunker. Build your own HQ. Enlist free.";
  const img = `/api/s5/hq-card?h=${encodeURIComponent(res ? res.view.handle : String(params.handle || "").slice(0, 80))}`;
  return {
    title,
    description,
      ...
      <PageShell>
        ...
        <Panel style={{ textAlign: "center", padding: "40px 24px", maxWidth: 560, margin: "0 auto" }}>
          ...
          <div style={{ fontSize: 20, fontWeight: 800, color: UI.text, marginBottom: 10 }} data-testid="hq-not-found">
            No commander answers at this address.
          </div>
