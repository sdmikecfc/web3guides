 ...
 *
 * The SAME engine and the SAME board as /s5/world, with a second camera and a
 ...
 * lib/s5/roster.ts is server-only, so importing a value from it here would pass
 * tsc and break next build.
 */
import { useMemo, useState } from "react";
import Link from "next/link";
import { S5_WORLD } from "@/lib/s5/world";
import { targetArtFor, targetPos, type Vec2 } from "@/lib/world/types";
import type { RosterRow } from "@/lib/s5/roster";
import { STAT_LABELS } from "@/lib/s5/games";
...
export function ExploreMap({ bases, domainIndex, totalCommanders }: ExploreMapProps) {
  ...
  const placed = useMemo(() => {
    ...
    for (const [key, arr] of Array.from(groups.entries())) {
      const centre =
        key === "__muster" ? STAGING : targetPos(S5_WORLD, domainIndex[key]);
  ...
  return (
    <>
      <WorldViewport
        config={S5_WORLD}
        initial={S5_WORLD.openAt}
        places={places}
        ariaLabel="Other commanders on the front"
      >
        {(pz) => (
          <>
            {/* The strongholds stay on the board as landmarks, unclickable —
                this view is about the people standing at them. */}
            {S5_WORLD.targetSlots.map((p, i) => (
              <WorldSprite
                key={`slot-${i}`}
                pos={p}
                size={S5_WORLD.targetSize * 0.82}
                art={`${S5_WORLD.artRoot}/${targetArtFor(S5_WORLD, i).intact}.webp`}
                glyph="keep"
                accent="#6f7d8c"
                label=""
                tier={3}
      ...
      {open ? (
        <MapPopup
          title={open.name}
          ariaLabel="commander record"
          accent="#e0b45c"
          onClose={() => setOpen(null)}
          headerRight={<span className="wm-chip" style={{ color: "#e0b45c", borderColor: "#e0b45c" }}>#{open.rank}</span>}
        >
          <p className="wm-lead">
            {open.rankName} · {open.tankName}
          </p>
          <dl className="wm-facts">
            <div><dt>Medals</dt><dd>{open.points.toLocaleString("en-US")}</dd></div>
          ...
          {open.handle ? (
            <Link className="wm-cta" href={`/s5/hq/${open.handle}`}>
              Visit their garage
            </Link>
          ) : null}
        </MapPopup>
      ...
      <div className="wm-topbar">
        <p className="wm-poolline">
          {placed.length} of {totalCommanders} commanders on the field
        </p>
      </div>
