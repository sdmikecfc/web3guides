...
export interface ModelIconProps {kind:string;recipeId?:string;ingredientId?:string;vesselKind?:SceneFood['vesselKind'];label:string;size?:number;foodKind?:SceneFood['kind'];look?:number;stage?:string;cold?:boolean;tier?:number;color?:string;mastery?:number}
const iconKey=({kind,recipeId,ingredientId,vesselKind,foodKind='dish',look=0,stage,cold=false,tier=1,color,mastery=0}:ModelIconProps)=>JSON.stringify([kind,recipeId,ingredientId,vesselKind,foodKind,look,stage,cold,tier,color,mastery>=10?10:mastery>=3?3:0]);
...
async function renderIcon(props:ModelIconProps):Promise<string>{
  const {kind,recipeId,ingredientId,vesselKind,foodKind='dish',look=0,stage,cold,tier,color,mastery}=props,key=iconKey(props),cached=images.get(key);if(cached)return cached;
  const s=await getStudio();requireContext(s.renderer);
  const model=kind==='food'?s.models.createFoodModel({recipeId:recipeId??'classic_burger',kind:foodKind,ingredientId,vesselKind,stage,cold,mastery}):s.models.createModel(kind,{recipeId,look,tier,color});
