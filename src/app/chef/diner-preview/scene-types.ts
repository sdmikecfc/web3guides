...
export interface SceneFood { recipeId:string; kind:'raw'|'processed'|'dish'|'burnt'|'dirty'; stage?:string; cold?:boolean; mastery?:number }
export interface SceneSlot { food?:SceneFood|null; state?:'idle'|'working'|'ready'|'burning'; progress?:number }
export interface SceneObject extends ScenePoint { id:string; kind:string; rotation?:0|1|2|3; tier?:number; state?:'idle'|'working'|'ready'|'burning'; progress?:number; food?:SceneFood|null; slots?:SceneSlot[]; footprint?:[number,number]; color?:string }
export interface SceneSeat extends ScenePoint { id:string; status:string; item?:SceneFood|null; customerId?:string|null }
export interface SceneTable extends ScenePoint { id:string; capacity:2|4; rotation?:0|1|2|3; seats:SceneSeat[] }
