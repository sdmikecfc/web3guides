...
export function serviceScene(state:ServiceState,selectedId:string|null=null,paint="#b66751",sign="The Little Lunch Truck"):DinerSceneData {
  ...
  return {width:dims.w,height:dims.h,pavementWidth:dims.pavementW,pavementHeight:dims.pavementH,paint,sign,selectedId,tick:state.tick,paused:state.phase==='paused',
    objects:state.stations.map((s):SceneObject=>{const slot=s.slots.find(v=>v.item)??s.slots[0];return {id:s.id,kind:s.kind,x:s.x,y:s.y,rotation:s.facing,tier:s.tier,stock:s.kind==='plates'?state.cleanPlates:s.kind==='cups'?state.cleanCups:s.kind==='bowls'?(state.cleanBowls??0):undefined,basketRaised:s.slots.some(slot=>slot.batch?.phase==='raised'||slot.boil?.phase==='drained'),portions:s.slots.reduce((n,slot)=>n+(slot.batch?.remaining??0),0),food:plated(slot?.item),footprint:EQUIPMENT_BY_ID[s.kind]?.footprint,
      slots:s.slots.map(slot=>({food:plated(slot.item),state:slot.item?.kind==='burnt'?'burning':slot.batch?.phase==='raised'||slot.batch?.phase==='ready'||slot.boil?.phase==='drained'||slot.boil?.phase==='ready'||slot.job?.ready?'ready':slot.job?'working':'idle',progress:slot.job?1-slot.job.remaining/Math.max(1,slot.job.total):0})),
      state:slot?.item?.kind==='burnt'?'burning':slot?.batch?.phase==='raised'||slot?.batch?.phase==='ready'||slot?.boil?.phase==='drained'||slot?.boil?.phase==='ready'||slot?.job?.ready?'ready':slot?.job?'working':'idle',progress:slot?.job?1-slot.job.remaining/Math.max(1,slot.job.total):0};}),
    tables,people};
}
