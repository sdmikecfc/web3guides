...
export function serviceScene(state:ServiceState,selectedId:string|null=null,paint="#b66751",sign="The Little Lunch Truck"):DinerSceneData {
  ...
  return {width:dims.w,height:dims.h,pavementWidth:dims.pavementW,pavementHeight:dims.pavementH,paint,sign,selectedId,tick:state.tick,paused:state.phase==='paused',
    objects:state.stations.map((s):SceneObject=>{const slot=s.slots.find(v=>v.item)??s.slots[0];return {id:s.id,kind:s.kind,x:s.x,y:s.y,rotation:s.facing,tier:s.tier,food:plated(slot?.item),footprint:EQUIPMENT_BY_ID[s.kind]?.footprint,
      slots:s.slots.map(slot=>({food:plated(slot.item),state:slot.item?.kind==='burnt'?'burning':slot.job?.ready?'ready':slot.job?'working':'idle',progress:slot.job?1-slot.job.remaining/Math.max(1,slot.job.total):0})),
      state:slot?.item?.kind==='burnt'?'burning':slot?.job?.ready?'ready':slot?.job?'working':'idle',progress:slot?.job?1-slot.job.remaining/Math.max(1,slot.job.total):0};}),
    tables:state.tables.map(t=>({id:t.id,x:t.x,y:t.y,capacity:t.capacity,rotation:t.rotation,seats:t.seats.map(s=>({...s,item:plated(s.item)}))})),people};
}
