...
export function createDinerMusic(){
  ...
  function useFallback(id:TrackId){
    if(!context)return;
    // A missing recording is a session-level failure, not a reason to fetch
    // again on every game click. Keep the fallback's existing musical clock.
    failedTracks.add(id);
    for(const voice of voices)stopVoice(voice,context.currentTime,.2);
    current=null;pending=null;
    if(!fallback)nextNote=context.currentTime+.03;
    fallback=true;schedule();
  }
  ...
  function syncContext():Promise<void>{
    if(contextSync)return contextSync;
    const ctx=context;if(!ctx||disposed)return Promise.resolve();
    // Browser suspend/resume is asynchronous. Finish the outstanding operation,
    // then apply the latest mute/visibility choice before selecting a track.
    const work=(async()=>{
      while(!disposed&&ctx===context){
        const shouldPlay=playable();
        if(shouldPlay)await ctx.resume();else await ctx.suspend();
        if(disposed||ctx!==context)return;
        if(shouldPlay!==playable())continue;
        if(shouldPlay){if(bus)ramp(bus,amplitude(),ctx.currentTime,.15);void selectTrack();schedule();}
        return;
      }
  ...
  async function activate(){
    ...
    // must not touch gain envelopes, retry tracks or reschedule the score.
    if(context?.state==='running'&&!contextSync)return;
    try{
