...
export default function RoomReview(){
 ...
 const state=useMemo(()=>{const s=createDiner(1_800_000_000_000,'room-review'),blueprint=createRestaurantBlueprint(stage);s.cosmetics={...s.cosmetics,...stageDefaultSurfaces(stage)};s.home={...s.home,...blueprint.roomPlan,roomPlan:blueprint.roomPlan,layout:stage==='burger_shop'?s.home.layout:[...blueprint.layout,...stageDecorLayout(blueprint.roomPlan)],staff:blueprint.staff,finishes:stageDefaultFinishes(stage),name:stage==='burger_shop'?'Bun & Butter':stage==='diner'?'The Hungry Bear':'Sunday Supper'};s.home.menu.main=['classic_burger'];return s;},[stage]);
