/**
 * Season 5 arcade hub (/s7/play): a grid of the four field exercises from the
 * ONE game registry (src/lib/s7/games.ts). Each tile routes to
 * /s7/games/<key>; all four games are live (comingSoon in the registry only
...
import { getSeasonSnapshot, type GameBoardRow } from "@/lib/s7/data";
...
export const metadata = {
  ...
  description: "The arcade games. Small Valor, big bragging rights.",
};
...
// stays with the slot across all three retirement waves, see src/lib/s7/games.ts).
...
export default async function S7Play() {
  ...
  // reads. Both money lines on this page fill from this one bag.
  const leaderArgs = {
  ...
  // the arcade and not only on /s7/board, and it costs nothing: the snapshot is
  ...
  return (
    ...
    <PageShell
      ...
      backdrop={{
        ...
        backgroundImage: "url(/s7-art/front/wash.webp)",
    ...
    >
      ...
      >
        ...
        {/* The one cash line the arcade owns: the per-game prize, its source,
            and the tie rule, muted under the Valor line. */}
        <span style={{ flexBasis: "100%", fontSize: 12.5, color: UI.muted, lineHeight: 1.6 }}>
          {fill(d.play.cashNote, leaderArgs)}
        </span>
      </Panel>
      ...
      {featured ? (
        <Link
          className="s7lib-hero"
          href={`/s7/games/${featured.key}`}
          ...
          style={{
            ...
            ["--s7-accent" as string]: featuredAccent,
          }}
        >
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            className="s7lib-cover"
            src={`/s7-art/games/${featured.key}/card.webp`}
            alt=""
            style={{
              position: "absolute",
              inset: 0,
      ...
      >
        {GAMES.map((g) => {
          ...
          return (
            <Link
              key={g.key}
              className="s7lib-tile"
              href={`/s7/games/${g.key}`}
              style={{ textDecoration: "none", ["--s7-accent" as string]: accent, borderRadius: 14, display: "block" }}
            >
              <Panel
                style={{
              ...
              >
                ...
                >
                  ...
                  <img
                    className="s7lib-cover"
                    src={`/s7-art/games/${g.key}/card.webp`}
                    alt=""
                    loading="lazy"
                    style={{
                      position: "absolute",
      ...
      >
        {fill(d.play.footNote, leaderArgs)}
      </p>
      ...
      <nav style={{ display: "flex", justifyContent: "center", gap: 10, marginTop: 16, flexWrap: "wrap" }}>
        {[
          { href: "/s7", label: d.common.back },
          { href: "/s7/board", label: d.links.board },
        ].map((l) => (
          <Link
            key={l.href}
            href={l.href}
...
const SHELF_CSS =
  ".s7lib-tile,.s7lib-hero{transition:transform .2s ease,box-shadow .2s ease,filter .2s ease;}" +
  ".s7lib-tile:hover,.s7lib-tile:focus-visible,.s7lib-hero:hover,.s7lib-hero:focus-visible{" +
  "box-shadow:0 24px 54px rgba(0,0,0,.62),0 0 34px var(--s7-accent);outline:none;}" +
  ".s7lib-cover{transition:transform .45s ease;}" +
  ".s7lib-tile:hover .s7lib-play,.s7lib-tile:focus-visible .s7lib-play{" +
  "background:var(--s7-accent);color:#0b0d10;border-color:var(--s7-accent);}" +
  "@media (prefers-reduced-motion: no-preference){" +
  ".s7lib-tile:hover,.s7lib-tile:focus-visible{transform:translateY(-6px) scale(1.015);}" +
  ".s7lib-hero:hover,.s7lib-hero:focus-visible{transform:translateY(-3px);}" +
  ".s7lib-tile:hover .s7lib-cover,.s7lib-tile:focus-visible .s7lib-cover,.s7lib-hero:hover .s7lib-cover{transform:scale(1.055);}" +
  ".s7lib-hero:hover .s7lib-heroBtn{filter:brightness(1.08);}" +
  "}";
