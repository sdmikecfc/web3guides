/**
 * Season 5 /s7/join, WALLET-FIRST enlist form (port of the proven S4 form).
 * Connect wallet -> sign a one-line ownership message (no gas) -> POST
 * /api/s7/join -> everyone lands on The Guild. Discord links later.
...
import { track, rememberRef, storedRef } from "@/lib/s7/track";
...
export function JoinForm() {
  ...
  const [d, setD] = useState<S7Dict>(STRINGS.en);
  useEffect(() => {
  ...
  async function postJoin(payload: Record<string, unknown>) {
    ...
    const ref = storedRef();
    const resp = await fetch("/api/s7/join", {
      method: "POST",
  ...
  async function handleEnlist() {
    ...
    try {
      ...
      if (result?.needsSignature) {
        ...
        const uri =
          typeof window !== "undefined" ? `${window.location.origin}/s7/join` : "https://launchwars.xyz/s7/join";
      ...
      track("join_done");
      setEnlisted({
        teamName: typeof result.teamName === "string" ? result.teamName : "The Guild",
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          <a href="/s7" style={primaryBtn(accent)}>
            {d.join.seeMap}
          </a>
          <a href="/s7/hq#wizard" style={ghostBtn()} data-testid="join-fund">
            {d.join.fundCta}
          </a>
          <a href="/s7" style={ghostBtn()}>
            {d.links.hq}
          </a>
          <a href="/s7/play" style={ghostBtn()}>
            {d.join.playArcade}
          </a>
        </div>
      </div>
