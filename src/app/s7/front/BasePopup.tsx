 ...
 * ugly barracks thing you made"). Ported from S7's GaragePopup
 * (src/app/s7/world/WorldMap.tsx ~1910) under the law written on the donor:
 ...
 *
 * The hero tab shows the player's CURRENT hero (TankPanel reads `me`); the
 * adventurer tab swaps adventurers with their portraits (CommanderPanel renders the
 ...
 * hand off to the HQ page. NOTE the hash: S7's donor pointed at "#funding",
 ...
 * so the handoff goes to /s7/hq#wizard.
 *
 * /s7/hq itself STAYS for deep links; only the map interaction becomes this
...
export function BasePopup({
   ...
   *  signed-in adventurer to enlist (TankPanel reads it). */
  ready: boolean;
  /** The season's keeps. The decal collection needs them to list each
   *  standing wall's breach slot; without them this panel reported a different
   *  total here than it did in the HQ (the S7 lesson, kept). */
  targets: BaseTargetLite[];
  onClose: () => void;
}) {
  const TABS = [
    { key: "tank", label: d.world.tabTank },
    { key: "adventurer", label: d.world.tabAdventurer },
  ...
  const openWizard = () => {
    window.location.href = "/s7/hq#wizard";
  };
  ...
  return (
    <div className="s7bp">
      <MapPopup title={d.world.baseTitle} ariaLabel="your base" accent="#f0b340" onClose={onClose}>
        ...
        {/* No `!me` branch here, deliberately: s7's useHqMe seeds guest
        ...
        <div className="wm-tabbody">
          ...
          {tab === "adventurer" ? <CommanderPanel me={me} patchMe={patchMe} dict={d} /> : null}
 ...
 * WorldViewport (which is what injects MAP_POPUP_CSS on the S7 world map).
 * The tab styles are the donor's wm-tabs block from WorldMap.tsx's HUD_CSS,
 * byte-for-byte where possible.
 */
const BASE_POPUP_CSS = `
${MAP_POPUP_CSS}
/* Above the fixed S7TopNav (zIndex 1000); the shared shell's 120 was tuned
   for the S7 world map, which has no fixed season nav over it. */
.s7bp .wm-scrim{z-index:1200;}
