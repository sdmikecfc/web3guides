 ...
 * s7 seams (lib/s7/games keys, /api/s7/* endpoints) with the S7 addition:
 * GUEST MODE. Every S7 game is playable with no wallet at all:
 *
 ...
 *   gets the same attempts/day as a adventurer (GAME_RULES[game].attempts);
 ...
 *   parked bests ONCE to /api/s7/claim-guest (server-side idempotent sentinel),
 *   converting them into Valor. localStorage marks the claim so this browser
 *   never re-posts.
 *
 * No Telegram path in S7 (the s7 tree has no TelegramProvider); wallet-first
 * only, same as the s7 join flow.
 */
"use client";
...
/** Default accent for S7 game chrome; games may pass their arcade-tile accent. */
...
// EIP-4361 shape, now SHARED with the garage's own sign-in (lib/s7/games
...
export function useS7Session() {
  ...
  const openSession = useCallback(async () => {
    ...
    try {
      ...
      const uri = `${window.location.origin}/s7/play`;
...
export type S7Session = ReturnType<typeof useS7Session>;
...
export type ScoreResult = {
  ok: boolean;
  points?: number;
  /** Play currency the bank paid. Shown next to the Valor line so the player
   ...
   * (see /api/s7/score). Display-only; the client never claims the bonus. */
  sprint?: boolean;
  /** Valor this specific run's bank paid on top of the flat per-run rate
   * because a sprint was active; 0 when sprint is false OR when this game's
   * Valor for today were already banked by an earlier attempt. */
  sprintBonus?: number;
  /** Class XP this bank granted to the active class (run XP plus the daily
   * class quest bonus). 0 or absent when no class is picked, the run paid no
   * XP, or the grant failed (progression never voids a banked run). */
  classXp?: number;
  /** True when this bank's XP pushed the active class over a level line. */
  leveled?: boolean;
  /** The active class's level AFTER this bank. Present only when XP was
   * actually granted (the route drops the field otherwise). */
  classLevel?: number;
  already?: boolean;
  error?: string;
};
...
export async function startRun(token: string, game: string): Promise<RunStartResult> {
  const r = await fetch("/api/s7/run-start", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function submitScore(
  ...
  const r = await fetch("/api/s7/score", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
// ── Guest progress (localStorage; the ONE shape /api/s7/claim-guest reads) ──
type GuestStore = {
  v: 1;
...
/**
 * Once a play session exists, convert parked guest bests into Valor, ONCE.
...
export function useGuestClaim(token: string | null) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch("/api/s7/claim-guest", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
 ...
 * section (src/lib/s7/strings.ts). English defaults live inline below as the
