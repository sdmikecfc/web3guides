/**
 * S7 shared SFX kit — sample-first WebAudio engine with a synth fallback.
 ...
 * of it can be sample-backed. A missing /s7-art/audio directory therefore
...
export type Sfx = {
  ...
  stopAll: () => void;
  /** stopAll + close the AudioContext. Call on UNMOUNT only: Chrome caps
   * AudioContexts per document (~6), and App Router client nav keeps the
   * document alive, so a context leaked per game visit went silently mute
   * on the seventh (leak audit 2026-08-17). */
  dispose: () => void;
  muted: () => boolean;
