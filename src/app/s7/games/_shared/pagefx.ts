/**
 * S7 shared PAGE-FX layer — the promoted Gunner's Run (tankbuster) camera and
 ...
 * tape byte-identical across every page refactor (see scripts/s7-harness.ts
...
// sim.ts — every S7 sim already carries its own copy of exactly this pair;
...
export interface PageFxInput {
  ...
  w: number;
  h: number;
  /** The game's own view-scale factor (every S7 sim exposes s.k = viewW/480). */
  k: number;
