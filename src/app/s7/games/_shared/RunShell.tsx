/**
 * S7 RUN SHELL — the one custom page shell every Season 5 game runs on.
 ...
 *   "s7-<game>-YYYY-MM-DD" (localStorage `s7_<game>_daily`, overridable for
 ...
 * does NOT import the season dict. Six new keys were added in the S7
 ...
 * entries in src/lib/s7/strings.ts's dict for ko/zh (out of this phase's
 * scope; the merge `{...DEFAULT_STRINGS, ...strings}` degrades safely to
 * English for any key a locale pack has not caught up on yet).
 *
 * FIXED TIMESTEP (S7 quality pass phase 0): the rAF loop no longer feeds the
 * sim a variable per-frame dt. It accumulates real elapsed time and steps
 * the sim in fixed 1/60s quanta (matching scripts/s7-harness.ts's TAPE_DT
...
/** The active class track a scored run plays as (ADR-0129), server-resolved
 * by /api/s7/run-start; null for guests, practice, and the classless. */
...
/** The contract a Pixi scene hands back to the shell (ADR-0119). */
export interface SceneHandle<S> {
  /** Draw the current sim state. Called once per painted frame, after 0+
   * fixed sim steps; read-only on the state, exactly like `draw`. */
  render: (s: S, view: ShellView) => void;
  /** Re-fit to the canvas's CURRENT CSS box. Same cadence as
   * fitCanvasToSim: once at run start, once per ResizeObserver tick. */
...
export interface RunShellStrings extends SessionPanelStrings {
  ...
  netFail: string;
  startFail: string;
  /** {attempts} {points} {leaderPrize} */
  footReal: string;
  /** {leaderPrize}: the per-game season-best cash prize, on the result card. */
  leaderNote: string;
  footPractice: string;
  practiceLink: string;
  realLink: string;
  ...
  sprintNote: string;
  /** {xp} — class XP this bank granted (run XP plus the daily class quest). */
  classXpNote: string;
  /** {lvl} — shown emphasized when the bank pushed the class over a level. */
  levelUpNote: string;
  /** {n} {points} — the ARCADE Valor still available to this wallet today.
...
/** The arcade-prize figures, formatted once from the registry: footReal and
 * leaderNote carry {leaderPrize}/{leaderPool}, and an unfilled template
 * renders its raw token straight onto the page. */
const LEADER_PRIZE = `$${GAME_LEADER_PRIZE_USD.toLocaleString("en-US")}`;
const LEADER_POOL = `$${GAME_LEADER_POOL_MAX_USD.toLocaleString("en-US")}`;
...
/** TOUCH SKILL BUTTON support (2026-08-30): `space` was keyboard-only, so
 * phones could never fire a class skill in any game. True on coarse
 * pointers; the shell then renders an on-screen button that holds the same
 * flag the space bar holds - sims' edge/hold semantics untouched. */
...
const FIXED_DT = 1 / 60; // matches scripts/s7-harness.ts's TAPE_DT exactly
...
/** StrictMode serializer for Pixi stage builds/destroys (see the mount
 * effect): module-level so remounts and route hops share the one queue. */
...
export interface RunShellProps<S> {
  ...
  title: string;
  accent: string;
  /** Legacy localStorage override; default `s7_<game>_daily`. */
  dailyLsKey?: string;
  ...
  /** ADR-0119 PIXI SEAM. When present, the shell mounts this scene ONCE per
   * mount on the SAME canvas (a Pixi v8 renderer in manual-render mode) and
   * calls handle.render every painted frame INSTEAD of `draw`. While the
   * prop exists the 2d context is NEVER acquired (a canvas holds exactly one
   * context type; touching 2d first would break the WebGL init), so frames
   * before the async init resolves simply skip drawing - the launch overlay
   * covers them. If create REJECTS (WebGL refused), the shell falls back to
   * the 2d `draw` path for the rest of the mount. The prop must be stable
   * for the component's lifetime. */
  scene?: (canvas: HTMLCanvasElement) => Promise<SceneHandle<S>>;
  done: (s: S) => boolean;
  score: (s: S) => number;
  ...
  /** TOUCH SKILL BUTTON label (coarse pointers only; the button holds
   * `space`). Omit for the default "SKILL"; pass a short word when space
   * means something else in this game (gauntlet: "NEXT" ends the turn);
   * pass null to hide it entirely (ascent: holding anywhere already
   * charges, a second hold target would only get in the aim drag's way). */
...
export function RunShell<S>({
  ...
  /** The arena box. CSS owns its size, so it is the ONLY honest measurement
   * source for the renderer (the canvas is a thing we resize, not a ruler). */
  const arenaRef = useRef<HTMLDivElement | null>(null);
  // ADR-0119: the mounted Pixi scene (null until the async init resolves) and
  // the WebGL-refused latch that re-opens the 2d fallback path.
  const sceneRef = useRef<SceneHandle<S> | null>(null);
  ...
  // THE CLASS LOADOUT (ADR-0129): resolved server-side by run-start alongside
  // stats; the S7 sims read it through createSim. Guest/practice = null.
  const loadoutRef = useRef<RunLoadout | null>(null);
  ...
  const reducedLive = useReducedMotionLive();
  const coarse = useCoarsePointer(); // render-path motion gate (SSR-safe)
  ...
  const finish = useCallback(
    async (s: S) => {
      ...
      track("run_done", { game, mode: modeRef.current, score: sc });
  ...
  // the loop effect runs once; the scene prop is stable by contract
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      // ADR-0119: while a scene prop exists the 2d context is NEVER acquired
      // (one context type per canvas; 2d-first would break the WebGL init).
      const sceneH = sceneRef.current;
      const use2d = !hasScene || sceneFailedRef.current;
      const ctx = use2d ? cv.getContext("2d") : null;
      if (use2d && !ctx) return;
      ...
      if (sceneH) {
        sceneH.render(s, view);
      } else if (ctx) {
        ctx.clearRect(0, 0, simDims.w, simDims.h);
        fns.draw(ctx, s, view);
      }
      // scene still initializing: sim steps, nothing draws, the overlay covers
      if (fns.done(s)) {
        const finished = s;
    ...
    return () => {
      ...
      sfxRef.current?.dispose(); // unmount: loops die AND the AudioContext closes
    };
  }, []);
  ...
  // MEASURE THE ARENA BOX, NEVER THE CANVAS (2026-08-14). The canvas is a
  // thing the renderer writes to; measuring it and then resizing it made the
  // observer chase its own tail, and once the canvas stopped changing size the
  // observer went permanently silent. The wrapper is owned by CSS alone, so it
  // is the only honest source of truth for "how big is the arena".
  useEffect(() => {
    const box = arenaRef.current;
    const cv = canvasRef.current;
    if (!box || !cv || typeof ResizeObserver === "undefined") return;
    const ro = new ResizeObserver(() => {
      ...
      const sceneH = sceneRef.current;
      if (sceneH) {
        // ADR-0119: Pixi owns the backing store; hand it the fresh box
        const cssW = rect.width >= 50 ? rect.width : sim.w;
        const cssH = rect.height >= 50 ? rect.height : sim.h;
        sceneH.resize(cssW, cssH, Math.min(window.devicePixelRatio || 1, DPR_CAP), sim.w, sim.h);
  ...
  // ── ADR-0119: mount the Pixi scene once per shell mount ───────────────────
  // StrictMode law (Battlefield.tsx pixiChain): React dev double-mounts, and
  // two app.init() racing on ONE canvas kill each other's shaders ("could not
  // retrieve shader source", found live on IRON JAW 2026-08-14). Every build
  // AND destroy is serialized through the module-level chain below so a
  // canvas only ever hosts one stage at a time.
  useEffect(() => {
    const cv = canvasRef.current;
    if (!cv || !scene) return;
    let dead = false;
    sceneChain = sceneChain
      ...
      .then(
        (h) => {
          ...
          sceneRef.current = h;
          // measure the BOX, and prefer the game's PINNED world over the
          // 360x480 stand-in: simSizeRef is still null here (the scene mounts
          // on page load, `begin` only runs on Start), so the stand-in used to
          // lay the first frame out against a world no game actually uses.
          const pinned = worldSize?.(0, 0);
          const rect = (arenaRef.current ?? cv).getBoundingClientRect();
          h.resize(
            rect.width >= 50 ? rect.width : 360,
            rect.height >= 50 ? rect.height : 480,
            Math.min(window.devicePixelRatio || 1, DPR_CAP),
            simSizeRef.current?.w ?? pinned?.w ?? 360,
            simSizeRef.current?.h ?? pinned?.h ?? 480,
          );
        },
        () => {
          // WebGL refused (old device, blocked context): the 2d draw path
          // re-opens and the game stays fully playable on vectors
    ...
    // the scene prop is stable for the mount by contract
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // ── begin a run: mode -> seed (daily / nonce / entropy) -> sim ────────────
  const begin = useCallback(async () => {
    // A held key from the menu (Space on the Start button, a stuck arrow)
    // must not leak into frame 1 of the run.
    {
      const k = keysRef.current;
      k.left = k.right = k.up = k.downKey = k.space = false;
    }
    ...
    if (practice) {
      ...
      // /api/s7/run-start reads the hq JSONB SERVER-side and returns
      ...
      loadoutRef.current = (rs as { loadout?: RunLoadout | null }).loadout ?? null;
      viewRef.current.tank = rs.tank ?? null;
      daily = dailyOpen;
      seed = daily ? `s7-${game}-${today}` : rs.nonce;
    } else if (guestRunsLeft(game) > 0) {
      mode = "guest";
      nonceRef.current = null;
      daily = dailyOpen;
      seed = daily ? `s7-${game}-${today}` : `guest-${Date.now()}-${Math.random()}`;
    } else {
    ...
    // allowlisted since day one, fired since the 2026-08-16 CRO audit: without
    // it the funnel has no event that a run ever HAPPENED
    track("game_open", { game, mode });
    setResult(null);
    setGuestNote(null);
    setPhase("playing");
    requestAnimationFrame(() => {
      const cv = canvasRef.current;
      if (!cv) return;
      // the BOX is the ruler, not the canvas (see the ResizeObserver above).
      // Degenerate layouts (hidden tab, mid-transition) fall back to 3:4.
      const boxRect = (arenaRef.current ?? cv).getBoundingClientRect();
      const W = boxRect.width >= 50 ? boxRect.width : 360;
      const H = boxRect.height >= 50 ? boxRect.height : 480;
      const sim = worldSize ? worldSize(W, H) : { w: W, h: H };
      simSizeRef.current = sim;
      let cssW: number;
      let cssH: number;
      if (hasScene && !sceneFailedRef.current) {
        // ADR-0119: never touch the 2d context; the scene owns the store.
        // If the async init has not resolved yet, the resize lands in the
        // mount effect the moment it does.
        cssW = W;
        cssH = H;
        sceneRef.current?.resize(cssW, cssH, Math.min(window.devicePixelRatio || 1, DPR_CAP), sim.w, sim.h);
      } else {
        ({ cssW, cssH } = fitCanvasToSim(cv, sim.w, sim.h));
      }
      viewRef.current = { w: cssW, h: cssH, tank: viewRef.current.tank ?? null };
      stateRef.current = createSim(sim.w, sim.h, seed, REDUCED_MOTION, statsRef.current, loadoutRef.current);
  ...
  useEffect(() => {
    const setKey = (e: KeyboardEvent, v: boolean) => {
      ...
      const k = keysRef.current;
      // e.code first (layout-independent: KeyA is KeyA on AZERTY too), e.key
      // as the fallback for exotic keyboards that report only the character.
      const code = e.code;
      const key = e.key.length === 1 ? e.key.toLowerCase() : e.key;
      if (code === "ArrowLeft" || code === "KeyA" || key === "ArrowLeft" || key === "a") k.left = v;
      else if (code === "ArrowRight" || code === "KeyD" || key === "ArrowRight" || key === "d") k.right = v;
      else if (code === "ArrowUp" || code === "KeyW" || key === "ArrowUp" || key === "w") k.up = v;
      else if (code === "ArrowDown" || code === "KeyS" || key === "ArrowDown" || key === "s") k.downKey = v;
      else if (code === "Space" || key === " ") {
        k.space = v;
        if (v) e.preventDefault();
      }
      // Arrows scroll the page while a run is live; the sim owns them here.
      if (v && (code === "ArrowLeft" || code === "ArrowRight" || code === "ArrowUp" || code === "ArrowDown")) {
        e.preventDefault();
      }
    };
    const kd = (e: KeyboardEvent) => setKey(e, true);
    const ku = (e: KeyboardEvent) => setKey(e, false);
    // A key released while the window is blurred (alt-tab mid-hold) never
    // sends its keyup; without this clear the sim runs on a stuck key.
    const clearKeys = () => {
      const k = keysRef.current;
      k.left = k.right = k.up = k.downKey = k.space = false;
    };
  ...
  const arena = (
    <>
      ...
      >
        ...
        />
        {coarse && phase === "playing" && skillLabel !== null && (
          <button
              ...
              >
                ...
                {guestNote ? (
                  ...
                  <div style={{ color: "#86f0c4", fontSize: 13.5, lineHeight: 1.5 }}>
                    ...
                    {(result.classXp ?? 0) > 0 && (
                      <div style={{ color: "#c9d4dc", marginTop: 4 }}>
                        {fill(T.classXpNote, { xp: result.classXp ?? 0 })}
                      </div>
                    )}
                    {Boolean(result.leveled) && (result.classLevel ?? 0) > 0 && (
                      <div style={{ color: accent, marginTop: 4, fontWeight: 700 }}>
                        {fill(T.levelUpNote, { lvl: result.classLevel ?? 0 })}
                      </div>
                    )}
      ...
      <p style={{ color: "#87919b", fontSize: 12, marginTop: 12, textAlign: "center", lineHeight: 1.5 }}>
        {practice
          ? T.footPractice
          : fill(T.footReal, { attempts, points: THEME.points, leaderPrize: LEADER_PRIZE, leaderPool: LEADER_POOL })}
        <br />
        {practice ? (
          <a
            href={`/s7/games/${game}`}
