 ...
 * the whole /s7 tree, so the wallet connects ONCE and stays connected across
 * every page and game; the provider mounts here and never unmounts during
 * client-side nav). S7TopNav is a FIXED overlay, never in a page's layout
 * flow, so it cannot break the HQ scene or the full-screen games.
 */
import { WalletProviders } from "@/app/wallet/providers";
import { S7TopNav } from "./_components/S7TopNav";

export const metadata = {
  title: "Launch Wars S7: Realmfall | Your Personal HQ",
  description:
    "Every adventurer gets a camp. Hold a featured domain from $5, earn Valor daily, and reclaim keeps with The Guild.",
};
...
const GROUND_CSS = `
/* THE S7 GROUND. A single flat near-black behind a painted hero is what read
...
@media (prefers-reduced-motion: reduce) { .s7-ground { background-attachment: scroll; } }
`;

const SKIP_CSS = `
.s7-skip{position:absolute;left:-9999px;top:0;z-index:2000;
...
.s7-skip:focus{left:0;outline:2px solid #0b0d10;outline-offset:-4px;}
`;

export default function S7Layout({ children }: { children: React.ReactNode }) {
  return (
    <WalletProviders>
      <style dangerouslySetInnerHTML={{ __html: `${SKIP_CSS}
${GROUND_CSS}` }} />
      <a className="s7-skip" href="#s7-content">
        Skip to content
      </a>
      <S7TopNav />
      <div className="s7-ground">{children}</div>
    </WalletProviders>
  );
}
