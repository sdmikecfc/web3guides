/**
 * Season 5 THE RULES (/s7/rules): the pitch, the honest prize wording, the
 * hold economy numbers, the arcade cap, and the full field manual (the
 * education registry). Server component; copy comes from the s7 dict (cookie
 * locale, en byte-for-byte by default). The economy NUMBERS are threaded into
 * the dict templates from lib/s7/games.ts so all three languages always show
...
import { EDUCATION } from "@/lib/s7/hq";
...
export const metadata = {
  ...
  description:
    "Every rule of the season on one page: the pool, Valor, Gold, the hold tiers, and fair play.",
};
...
export default async function S7Rules() {
  ...
  const rewardArgs = {
    ...
    // lib/s7/warEffort.ts still mirrors the bot's sprint tripwire.
    weClose: 70,
    end: d.common.seasonEnd,
    total: money(SEASON_MONEY_USD),
    pool: money(POOL_FULL_USD),
    full: money(POOL_FULL_USD),
    // The slice BAND, derived from the two constants rather than typed: the
    // per-keep figure is the whole point of the 2026-08-16 rewrite, so
    // the floor and cap have to track games.ts and never a literal.
    sliceMin: money(SLICE_MIN_USD),
    sliceMax: money(SLICE_MAX_USD),
    // The arcade prize (per game) and its season-wide ceiling, from the same
    // registry the settlement engine reads, so no locale can drift from it.
    leaderPrize: money(GAME_LEADER_PRIZE_USD),
    leaderPool: money(GAME_LEADER_POOL_MAX_USD),
  };
  ...
  return (
    <PageShell>
      ...
      <Section id="pay" title={d.rules.payHeading}>
        ...
        <Panel>
          ...
          <p style={{ fontSize: 12.5, color: UI.faint, lineHeight: 1.6, margin: 0 }}>
            ...
            {" "}
            <a href={DOMA_HELP.graduation} target="_blank" rel="noreferrer" style={{ color: "#7dd3fc" }}>
              What bonding (graduation) means ↗
            </a>
          </p>
        </Panel>
      </Section>
      ...
      <Section id="rewards" title={d.rules.rewardsHeading}>
        ...
        <RewardGroup
          label={d.rules.rewardsSignal}
          what={d.rules.rewardsSignalWhat}
          rows={rewardRows(d.rules.rewardsSignalRows)}
          accent={UI.steel}
        />
        <RewardGroup
          label={d.rules.rewardsScrap}
          what={d.rules.rewardsScrapWhat}
          rows={rewardRows(d.rules.rewardsScrapRows)}
          accent="#f0b340"
        />
        <RewardGroup
      ...
      <p style={{ textAlign: "center", fontSize: 13, color: UI.faint, marginTop: 30 }}>
        ...
        {" · "}
        <Link href="/s7/join" style={{ color: UI.muted }}>{d.links.enlist}</Link>
        {" · "}
