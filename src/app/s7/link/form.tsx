/**
 * Season 5 /s7/link — bind a wallet-first player to a Discord account.
 * In Discord run /realm link to get a one-time code, then here: connect the
 * wallet, enter the code, sign one ownership message (no gas) -> POST
 * /api/s7/link-discord. Port of the proven S4 form, itself a port of S3's.
 * BUILT 2026-08-03, mid-launch: the bot had been handing out link codes and
 * pointing at ${homeUrl}/link since the season opened, but no S7 page existed,
...
// "Sign in with {domain}" panel. The server (api/s7/link-discord ->
...
export function LinkForm() {
  ...
  async function handleLink() {
    ...
    if (!codeOk) {
      setError("Enter the code from /realm link in Discord.");
      return;
    }
    setError(null);
    setSubmitting(true);
    try {
      ...
      const uri =
        typeof window !== "undefined" ? `${window.location.origin}/s7/link` : "https://web3guides.com/s7/link";
      const message = buildMessage({ domain, address, uri, nonce, issuedAt, code: code.trim().toUpperCase() });
      const signature = await signMessageAsync({ message });

      const resp = await fetch("/api/s7/link-discord", {
        method: "POST",
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        <div style={{ ...cardStyle, borderColor: `${linked.accent}55`, textAlign: "center" }}>
          ...
          <p style={{ fontSize: 14, color: MUTED, lineHeight: 1.6, margin: 0 }}>
            ...
            <b style={{ color: TEXT }}>/realm me</b> in the Doma Discord to see your {teamWord} and{" "}
            {DEFAULT_THEME.points}.
          </p>
        </div>
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center" }}>
          <a href="/s7/board" style={primaryBtn(linked.accent)}>
            See the War Board
          </a>
        </div>
      </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <div style={cardStyle}>
        ...
        <p style={{ fontSize: 13, color: FAINT, margin: 0, lineHeight: 1.5 }}>
          In the Doma Discord, run <b style={{ color: TEXT }}>/realm link</b>. The bot replies (only you
