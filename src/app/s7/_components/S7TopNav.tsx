/**
 * The persistent Season 5 nav: a FIXED, translucent top bar rendered by the /s7
 ...
 * single-provider pattern). Labels come from the s7 dict; the locale is read
...
import { STRINGS, type S7Dict } from "@/lib/s7/strings";
...
export function S7TopNav() {
  ...
  useEffect(() => {
    ...
    // read out by an English speech synthesizer (SC 3.1.1). The S7 nav mounts
    // on every /s7 route, so syncing here covers the season. Restores "en" on
    // unmount so leaving S7 cannot leave the document mislabelled.
    try {
  ...
  const nav = [
    ...
    // player looks first.
    { t: dict.nav.howToPlay, href: "/s7/how-to-play" },
    // NO MAP TAB. /s7 IS the map now (Mike, 2026-07-29: "if this is our main
    ...
    // the nav is decorative. NOTE (2026-08-15): the S7 sentence here used to
    // claim /s7/map still resolved for old links. It never did - S7's map
    // route was not cloned - so every link that pointed there 404'd until they
    // were repointed at /s7. Preflight now fails on any /s7 href with no route.
    { t: dict.nav.arcade, href: "/s7/play" },
    { t: dict.nav.board, href: "/s7/board" },
    { t: dict.nav.rules, href: "/s7/rules" },
  ];
  ...
  return (
    ...
    >
      ...
      >
        {nav.map(({ t, href }) => {
          const active = href === "/s7" ? path === "/s7" : path.startsWith(href);
          return (
            <Link
              key={href}
              href={href}
