 ...
 *
 * Three understated chips (EN / 한국어 / 中文). Clicking writes the `s7_lang`
 * cookie and reloads so server components re-render in the chosen locale
 * (getLocale in lib/s7/i18n) and client components re-read it on mount. The
...
import { LOCALE_COOKIE, LOCALES, clientLocale, type Locale } from "@/lib/s7/locale";
...
export function LanguageSwitcher({ ariaLabel = "Language" }: { ariaLabel?: string }) {
  ...
  function pick(code: Locale) {
    ...
    // 1 year, path=/ so it applies to every /s7 route; lax is fine (same-site).
