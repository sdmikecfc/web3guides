/**
 * Season 5 shared UI atoms (server-safe: no hooks, no client JS). The S7
 * bunker palette: gunmetal, steel, ember. Every player-visible WORD comes from
 * the Theme (src/lib/s7/theme.ts); never hardcode one here. Status COLORS are
 * data-viz semantics (green = breached, amber = under siege), stable across
 * reskins.
 */
import type { Theme, TargetStatus } from "@/lib/s7/theme";
import { statusLabel } from "@/lib/s7/theme";
...
export function PageShell({
  ...
  // Top padding clears the fixed 52px S7TopNav.
  return (
    <main
      id="s7-content"
      style={{
    ...
    >
      {backdrop ? (
        <div aria-hidden style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none" }}>
          ...
          <div className="s7-backdrop-art" style={{ position: "absolute", inset: 0, ...backdrop }} />
          {scrim ? (
            <div className="s7-backdrop-scrim" style={{ position: "absolute", inset: 0, ...scrim }} />
          ) : null}
        </div>
      ) : null}
...
/** The legal footer, rendered ONCE here rather than per page, because a page
 * that forgets it is exactly how these links went missing. The paths are
 * host-neutral and middleware now lets them through on the season subdomain
 * (they used to be rewritten into /s7/* and 404). Plain <a>, not next/link:
 * these leave the season tree, and a soft nav into the shared app tree buys
 * nothing. */
  ...
  return (
    ...
    >
      {links.map(([href, label]) => (
        <a key={href} href={href} style={{ color: UI.faint, textDecoration: "none" }}>
          {label}
        </a>
      ))}
      <span style={{ marginLeft: "auto" }}>Operator funded. Nothing here is guaranteed money.</span>
    </footer>
  );
}
...
/** The shared pool banner: renders the ONE pool line computed in lib/s7/data.ts. */
