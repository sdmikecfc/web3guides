"use client";
/**
 * S7 HOW TO PLAY, the interactive walkthrough (round-2 onboarding, Mike
 ...
 * existing /s7-art plates, a do-it-now deep link, and a done check ONLY where
 * this browser can honestly know the state:
 *   - Welcome        = a play session exists (localStorage token + /api/s7/me ok)
 *   - Get armed/Hold = heldUsd > 0 from the same me payload
 *   - Play           = the FTUE run flag or any parked guest score
 *                      (lib/s7/ftue helpers, REUSED never forked)
 ...
 *
 * Copy rules: no em-dashes, never "win $X", strings from the s7 dict only.
 */
...
import { readFtue, guestHasAnyScore } from "@/lib/s7/ftue";
...
/** The subset of the /api/s7/me payload this rail reads (junk-tolerant). */
type MeResp = {
  ok?: boolean;
  heldUsd?: number;
...
export function Walkthrough({ locale }: { locale: Locale }) {
  ...
  const dict: S7Dict = STRINGS[locale] ?? STRINGS.en;
  ...
  const steps: StepDef[] = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        cta: h.gs2Cta,
        href: "/s7/hq#wizard",
        art: "/s7-art/hq/camp-portrait.webp",
        check: "held",
      },
      {
        key: "hold",
      ...
      },
      {
        ...
        cta: h.gs4Cta,
        href: "/s7/play",
        art: "/s7-art/games/gauntlet/card.webp",
        check: "played",
      },
      {
        // THE RAID STEP, not a second copy of the money step. It carried gs7
      ...
      },
      {
        ...
        cta: h.gs6Cta,
        href: "/s7/board",
        art: "/s7-art/front/treasury.webp",
        check: "qualified",
      },
    ];
  }, [h, dict]);
  ...
  return (
    <div>
          ...
          return (
            <button
              ...
              onKeyDown={(e) => {
                ...
                setActive(next);
                document.getElementById(`s7wt-tab-${steps[next].key}`)?.focus();
              }}
      ...
      <div
        ...
        role="tabpanel"
        aria-live="polite"
        aria-labelledby={`s7wt-tab-${step.key}`}
        style={{
          background: UI.panel,
