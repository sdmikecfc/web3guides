"use client";
/**
 * S5 IRON SIEGE, the FIRST-ARRIVAL welcome (the /s5 homescreen only).
 *
 * Round-2 review (Mike, 2026-07-25): EXACTLY TWO DOORS. The primary CTA is
 * "Connect a wallet to play" (/s5/join); the only other way out is "Look
 ...
 *
 * All copy comes from the s5 dict. No em-dashes, never "win $X".
 */
...
export const WELCOME_STORAGE_KEY = "s6_welcome_seen" // was s5_welcome_seen: every S5 veteran had the S6 welcome suppressed;

/** Drop a background-removed commander cutout here and the portrait column
 * lights up on its own. Nothing else needs to change. */
export const WELCOME_PORTRAIT_SRC = "/s6-art/pilot/welcome-hero.webp";
...
export function WelcomeModal({ dict, onClose }: { dict: S6Dict; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div className="s5w-card" ref={cardRef} onClick={(e) => e.stopPropagation()}>
        {/* PORTRAIT SLOT: renders only when the cutout exists (see the header). */}
        {portraitOk ? (
          <div className="s5w-portrait">
        ...
        <div className="s5w-body">
          <p className="s5w-eyebrow">{DEFAULT_THEME.seasonName}</p>
          <h2 className="s5w-title">{dict.welcome.title}</h2>
          <p className="s5w-text">{dict.welcome.body}</p>

          <div className="s5w-actions">
            ...
            <button
              ...
              onClick={() => dismiss("welcome-enlist", "/s6/join")}
            >
          ...
          <p className="s5w-note">{dict.welcome.note}</p>
        </div>
      </div>
    </div>
  );
...
@media (prefers-reduced-motion: no-preference) {
  .s5w-scrim { animation: s5w-fade 220ms ease-out both; }
  .s5w-card { animation: s5w-rise 260ms cubic-bezier(0.2,0.7,0.3,1) both; }
}
@keyframes s5w-fade { from { opacity: 0; } to { opacity: 1; } }
@keyframes s5w-rise { from { opacity: 0; transform: translateY(14px); } to { opacity: 1; transform: none; } }
.s5w-card {
  display: flex;
...
.s5w-portrait {
  ...
  background:
    ...
    url("/s6-art/hq/camp-portrait.webp") 50% 30% / cover no-repeat #10141a;
}
...
@media (min-width: 560px) { .s5w-portrait { display: flex; } }
.s5w-body { flex: 1 1 auto; padding: 26px 26px 22px; }
.s5w-eyebrow {
  margin: 0 0 10px;
...
.s5w-title { margin: 0 0 10px; font-size: clamp(21px, 4.4vw, 26px); font-weight: 800; color: #e9edf1; }
.s5w-text { margin: 0 0 18px; font-size: 14px; line-height: 1.65; color: #cdd4dc; }
/* TWO DOORS, stacked: one loud primary, one quiet peer under it. */
.s5w-actions { display: flex; flex-direction: column; align-items: stretch; gap: 10px; }
.s5w-cta {
  padding: 15px 18px;
...
.s5w-cta:hover { filter: brightness(1.06); }
.s5w-cta:active { transform: translateY(1px); }
.s5w-ghost {
  padding: 12px 16px;
  border-radius: 10px;
...
.s5w-ghost:hover { color: #e9edf1; border-color: #9aa7b488; }
.s5w-cta:focus-visible, .s5w-ghost:focus-visible { outline: 2px solid #9aa7b4; outline-offset: 2px; }
.s5w-note { margin: 16px 0 0; font-size: 11.5px; line-height: 1.55; color: #87919b; }
`;
