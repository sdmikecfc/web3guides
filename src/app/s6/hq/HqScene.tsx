 ...
 * trophies arrive from /api/s6/me when a play session exists (localStorage,
...
import { clampStats, GAMES, SEASON_MONEY_USD, readSessionToken } from "@/lib/s6/games";
import { DEFAULT_THEME } from "@/lib/s6/theme";
import { HERO_TANK_KEY, campPlateFor, pilotGrade, resolvePilotKey, resolveTank, type ResolvedTank } from "@/lib/s6/model";
import { TANK_ROSTER } from "@/lib/s6/tanks";
import { pilotByKey, DEFAULT_COMMANDER_KEY } from "@/lib/s6/tanks";
...
import { isDemo, demoHqMe } from "@/lib/s6/demo";
import { markFtueHotspot } from "@/lib/s6/ftue";
import { track, rememberRef } from "@/lib/s6/track";
 ...
 * Panther swapping to the picked tank once /api/s6/me lands) crossfades over
...
function TankCaption({ tank, dict, className }: { tank: ResolvedTank; dict: S6Dict; className?: string }) {
  return (
...
function TankRig({ tank, dict }: { tank: ResolvedTank; dict: S6Dict }) {
  return (
...
function TrophyShelf({ trophies, dict, compact }: { trophies: TrophiesView | null; dict: S6Dict; compact?: boolean }) {
  const t = trophies;
...
function SceneCommander({ ck, label, onOpen }: { ck: string; label: string; onOpen: () => void }) {
  const png = `/s6-art/pilot/${ck}.png`;
  const grade = pilotGrade(ck);
  const [brokenSrc, setBrokenSrc] = useState<string | null>(null);
 ...
 */
function LandingBand({ dict, session }: { dict: S6Dict; session: boolean }) {
  ...
  const steps: Array<[string, string, string?, string?]> = [
    [L.s1t, L.s1b, "/s6/play", L.gamesCta],
    [L.s2t, L.s2b, "/s6/join", L.ctaPlay],
    [L.s3t, L.s3b, "/s6/hq#wizard", L.s3Cta],
  ];
  return (
    <section className="s5lb" id="s6-landing" data-testid="hq-landing">
      <div className="s5lb-in">
        ...
        <div className="s5lb-vets" id="s6-vets" data-testid="landing-vets">
        ...
        <div className="s5lb-cta">
          <Link href="/s6/play" className="s5lb-btn s5lb-btn--primary" data-testid="landing-play">
            {L.gamesCta}
          </Link>
          {!session ? (
            <Link href="/s6/join" className="s5lb-btn" data-testid="landing-join">
              {L.ctaPlay}
            </Link>
          ) : null}
        </div>
        ...
        <div className="s5lb-grid s5lb-grid--4">
          {GAMES.filter((g) => !g.comingSoon).map((g) => (
            <Link className="s5lb-card s5lb-card--link" key={g.key} href={`/s6/games/${g.key}`}>
        ...
        <Link href="/s6" className="s5lb-btn" data-testid="landing-map">
          {L.mapCta}
        </Link>
      </div>
    </section>
...
function CommanderSays({ dict, guest, raidSoon }: { dict: S6Dict; guest: boolean; raidSoon: boolean }) {
  ...
  return (
    <>
      ...
      <button
        ...
        data-testid="sky-how"
        onClick={() => onRoute("/s6/how-to-play", "scene-how-to-play")}
      >
...
export function HqScene({
  ...
  const activeCommander =
    (forceCommander && pilotByKey(forceCommander) ? forceCommander : null) ||
    (pilotByKey(me.pilot) ? me.pilot : null) ||
    DEFAULT_COMMANDER_KEY;

  // The camp painting for the bay tank (guest = the Panther hero portrait;
  // once /api/s6/me lands with a picked tank, CampBackdrop crossfades to that
      ...
      void fetch("/api/s6/me", {
        ...
        .then((resp) => {
          ...
          setMe((prev) => ({
            ...
            // `pilot`, and resolvePilotKey reads `src.pilot` - but this passed
            ...
            pilot: resolvePilotKey({ pilot: resp.hq?.pilot }),
            ownedCamos: Array.isArray(resp.hq?.ownedCamos)
              ? resp.hq.ownedCamos.filter((c): c is string => typeof c === "string")
              : prev.ownedCamos,
            ownedPilots: Array.isArray(resp.hq?.ownedPilots)
              ? resp.hq.ownedPilots.filter((c): c is string => typeof c === "string")
              : prev.ownedPilots,
  ...
  const callsign =
    me.session || forceCommander
      ? pilotByKey(activeCommander)?.name ?? dict.hq.hudGuest
      : dict.hq.hudGuest;
  const hudPoolShort =
  ...
  const dockButtons: Array<{ label: string; icon: string; onTap: () => void }> = [
    { label: dict.hq.dockArcade, icon: "▶", onTap: () => router.push("/s6/play") },
    { label: dict.hq.dockMap, icon: "✚", onTap: () => router.push("/s6") },
    { label: dict.links.howToPlay, icon: "?", onTap: () => router.push("/s6/how-to-play") },
    me.session
      ? { label: dict.nav.board, icon: "▦", onTap: () => router.push("/s6/board") }
      : { label: dict.links.enlist, icon: "⚑", onTap: () => router.push("/s6/join") },
    ...
    >
      ...
      <header className="s5hq-head" style={{ textAlign: "center", padding: "22px 16px 6px" }}>
        ...
        <p style={{ fontSize: 12.5, color: FAINT, margin: "6px auto 0", maxWidth: 620, lineHeight: 1.5 }}>
          {dict.hq.vetLine}{" "}
          <a href="#s6-vets" style={{ color: EMBER, textDecoration: "underline", textUnderlineOffset: 2 }}>
            {dict.hq.vetLink}
          </a>
        </p>
        ...
        >
          ...
          <a href="#s6-landing" style={{ color: MUTED, textDecoration: "underline", textUnderlineOffset: 2 }}>
            {dict.hq.step3}
          </a>
        </p>
      ...
      <div className="s5hq-desktop">
        <div className="s5hq-shell">
          ...
          >
            ...
            <SceneCommander
              ck={activeCommander}
              label={spotText(HOTSPOT_BY_KEY.pilot).label}
              onOpen={() => handleNav(HOTSPOT_BY_KEY.pilot)}
            />
      ...
      return (
        <div data-testid="more-sheet">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <Link href="/s6/rules" className="s5hq-morerow" data-testid="more-rules">
