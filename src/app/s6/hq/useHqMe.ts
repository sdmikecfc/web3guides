...
import { clampStats, readSessionToken } from "@/lib/s6/games";
import { resolvePilotKey, type ResolvedTank } from "@/lib/s6/model";
import { defaultHqMe, type HqMe } from "./panels";
import { demoHqMe, isDemo } from "@/lib/s6/demo";
...
type MeResponse = {
   ...
   * signal /api/s6/me currently returns; holdings are a season total only. */
        ...
        setMe((prev) => ({
          ...
          ownedTanks:
            Array.isArray(resp.hq?.ownedTanks) && resp.hq.ownedTanks.length > 0
              ? resp.hq.ownedTanks
              : prev.ownedTanks,
          // same half-finished rename as HqScene: the field is `pilot` and the
          // resolver reads `src.pilot` (fixed 2026-08-15)
