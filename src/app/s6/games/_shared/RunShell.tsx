/**
 * S6 RUN SHELL — the one custom page shell every Season 5 game runs on.
 ...
 *   "s6-<game>-YYYY-MM-DD" (localStorage `s6_<game>_daily`, overridable for
 ...
 * does NOT import the season dict. Six new keys were added in the S6
 ...
 * entries in src/lib/s6/strings.ts's dict for ko/zh (out of this phase's
 * scope; the merge `{...DEFAULT_STRINGS, ...strings}` degrades safely to
 * English for any key a locale pack has not caught up on yet).
 *
 * FIXED TIMESTEP (S6 quality pass phase 0): the rAF loop no longer feeds the
 * sim a variable per-frame dt. It accumulates real elapsed time and steps
 * the sim in fixed 1/60s quanta (matching scripts/s6-harness.ts's TAPE_DT
...
import { GAME_RULES, type PlayerStats } from "@/lib/s6/games";
import { markFtueRun } from "@/lib/s6/ftue";
...
export interface RunShellStrings extends SessionPanelStrings {
  ...
  sprintNote: string;
  /** {n} {points} — the ARCADE Signal still available to this wallet today.
...
const DEFAULT_STRINGS: RunShellStrings = {
  ...
  footReal: "Best of {attempts} runs a day counts, and a better score earns more {points}. Holding a domain is where the money is; the arcade earns {points} and Scrap for your garage.",
...
const FIXED_DT = 1 / 60; // matches scripts/s6-harness.ts's TAPE_DT exactly
...
export interface RunShellProps<S> {
  ...
  title: string;
  accent: string;
  /** Legacy localStorage override; default `s6_<game>_daily`. */
  dailyLsKey?: string;
...
export function RunShell<S>({
  ...
  const LS_DAILY = dailyLsKey ?? `s6_${game}_daily`;

  const session = useS6Session();
  ...
  const begin = useCallback(async () => {
    ...
    if (practice) {
      ...
      // /api/s6/run-start reads the hq JSONB SERVER-side and returns
      ...
      daily = dailyOpen;
      seed = daily ? `s6-${game}-${today}` : rs.nonce;
    } else if (guestRunsLeft(game) > 0) {
      mode = "guest";
      nonceRef.current = null;
      daily = dailyOpen;
      seed = daily ? `s6-${game}-${today}` : `guest-${Date.now()}-${Math.random()}`;
    } else {
