/**
 * S6 shared sprite kit — the S4 highnoon img()/ready()/spr() helpers promoted
 ...
 * per-game painted-art folders under public/s6-art/games/<game>/.
 *
 ...
 * /s6-art/games/<game>/<name>.<ext>. Missing files simply never become ready()
 ...
 * because key-s6-games.js writes them through pngjs, so one write path stays
 * one format. Regenerate plates with their generator, then run
 * `python downscale-s6-bg.py` and `python webp-s6-bg.py`.
 */
export function loadManifest<N extends string>(
  ...
  for (const n of names) {
    ...
    out[n] = img(`/s6-art/games/${game}/${n}.${ext}`);
  }
  return out;
}

// ── S4 benchmark utilities, ported (2026-07-26, S6 quality pass phase 0) ───
 ...
 * scale (every S6 sim exposes s.k = viewW/480).
 */
 ...
 * THREE.CanvasTexture; S6 games render on 2D canvas (ADR-0075 bans three.js
 ...
 * is the only place any S6 game ever needs a baked texture.
 */
