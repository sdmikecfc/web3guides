...
import { S6_WORLD } from "@/lib/s6/world";
import { targetArtFor, targetPos, type Vec2 } from "@/lib/world/types";
import type { RosterRow } from "@/lib/s6/roster";
import { STAT_LABELS } from "@/lib/s6/games";
...
export function ExploreMap({ bases, domainIndex, totalCommanders }: ExploreMapProps) {
  ...
  const placed = useMemo(() => {
    ...
    for (const [key, arr] of Array.from(groups.entries())) {
      const centre =
        key === "__muster" ? STAGING : targetPos(S6_WORLD, domainIndex[key]);
  ...
  return (
    <>
      <WorldViewport
        config={S6_WORLD}
        initial={S6_WORLD.openAt}
        places={places}
        ariaLabel="Other pilots on the front"
      >
        {(pz) => (
          <>
            {/* The strongholds stay on the board as landmarks, unclickable —
                this view is about the people standing at them. */}
            {S6_WORLD.targetSlots.map((p, i) => (
              <WorldSprite
                key={`slot-${i}`}
                pos={p}
                size={S6_WORLD.targetSize * 0.82}
                art={`${S6_WORLD.artRoot}/${targetArtFor(S6_WORLD, i).intact}.webp`}
                glyph="keep"
                accent="#6f7d8c"
                label=""
                tier={3}
      ...
      {open ? (
        ...
        >
          ...
          <dl className="wm-facts">
            <div><dt>Signal</dt><dd>{open.points.toLocaleString("en-US")}</dd></div>
          ...
          {open.handle ? (
            <Link className="wm-cta" href={`/s6/hq/${open.handle}`}>
              Visit their garage
            </Link>
          ) : null}
        </MapPopup>
      ...
      <div className="wm-topbar">
        <p className="wm-poolline">
          {placed.length} of {totalCommanders} pilots on the field
        </p>
      </div>
