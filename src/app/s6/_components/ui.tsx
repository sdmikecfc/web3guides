/**
 * Season 5 shared UI atoms (server-safe: no hooks, no client JS). The S6
 * bunker palette: gunmetal, steel, ember. Every player-visible WORD comes from
 * the Theme (src/lib/s6/theme.ts); never hardcode one here. Status COLORS are
 * data-viz semantics (green = breached, amber = under siege), stable across
 * reskins.
 */
import type { Theme, TargetStatus } from "@/lib/s6/theme";
import { statusLabel } from "@/lib/s6/theme";
...
export const UI = {
  ...
  warn: "#f0b340",
  bad: "#f87171",
  steel: "#9aa7b4", // The Resistance accent
  ember: "#e0662e",
...
export function PageShell({
  ...
  // Top padding clears the fixed 52px S6TopNav.
  return (
    <main
      id="s6-content"
      style={{
    ...
    >
      {backdrop ? (
        <div aria-hidden style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none" }}>
          ...
          <div className="s6-backdrop-art" style={{ position: "absolute", inset: 0, ...backdrop }} />
          {scrim ? (
            <div className="s6-backdrop-scrim" style={{ position: "absolute", inset: 0, ...scrim }} />
          ) : null}
        </div>
      ) : null}
 ...
 * (they used to be rewritten into /s6/* and 404). Plain <a>, not next/link:
...
/** The shared pool banner: renders the ONE pool line computed in lib/s6/data.ts. */
