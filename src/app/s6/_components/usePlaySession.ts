 ...
 * button linked to /s6/join. But /s6/join ENLISTS A WALLET INTO THE SEASON. It
...
import { buildPlaySessionMessage, writeSessionToken } from "@/lib/s6/games";
...
export function usePlaySession(): PlaySessionState {
  ...
  const open = useCallback(async (): Promise<string | null> => {
    ...
    try {
      ...
      const uri = `${window.location.origin}/s6/hq`;
