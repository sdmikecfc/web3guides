"use client";
/**
 * S6 IN-APP BUY: one tap, $5, on the mainframe's bonding curve.
 *
 ...
 *     curve (see /api/s6/buy-info). Graduated or unknown falls back to the
 ...
 *   - BUY_IN_APP_ENABLED in lib/s6/launchpad.ts is the kill switch.
 *
...
import { USDC_E_ADDRESS, USDC_E_DECIMALS, buyLink, DOMA_PROFILE_URL } from "@/lib/s6/funding";
import {
...
export function BuyPanel({
  ...
  strings: {
    ...
    fallbackCta: string;
    /** Shown when the mainframe has not listed on-chain yet: there is no
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void fetch(`/api/s6/buy-info?domain=${encodeURIComponent(domain)}`)
