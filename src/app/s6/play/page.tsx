/**
 * Season 5 arcade hub (/s6/play): a grid of the four field exercises from the
 * ONE game registry (src/lib/s6/games.ts). Each tile routes to
 * /s6/games/<key>; all four games are live (comingSoon in the registry only
 * marks a future slot that has not shipped yet). Server component, ISR.
 */
import Link from "next/link";
import { GAMES, GAME_DAILY_POINTS_CAP, POINTS_PER_RUN, SHELLS_PER_RUN } from "@/lib/s6/games";
import { getSeasonSnapshot, type GameBoardRow } from "@/lib/s6/data";
import { DEFAULT_THEME } from "@/lib/s6/theme";
import { dict, getLocale } from "@/lib/s6/i18n";
import { fill } from "@/lib/s6/strings";
...
export const metadata = {
  ...
  description: "Four field exercises. Small Signal, big bragging rights.",
};
...
// stays with the slot across all three retirement waves, see src/lib/s6/games.ts).
...
export default async function S6Play() {
  ...
  // the arcade and not only on /s6/board, and it costs nothing: the snapshot is
  ...
  return (
    ...
    <PageShell
      ...
      backdrop={{
        ...
        backgroundImage: "url(/s6-art/front/wash.webp)",
    ...
    >
      ...
      {featured ? (
        <Link
          className="s6lib-hero"
          href={`/s6/games/${featured.key}`}
          ...
          style={{
            ...
            ["--s6-accent" as string]: featuredAccent,
          }}
        >
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            className="s6lib-cover"
            src={`/s6-art/games/${featured.key}/card.webp`}
            alt=""
            style={{
              position: "absolute",
              inset: 0,
      ...
      >
        {GAMES.map((g) => {
          ...
          return (
            <Link
              key={g.key}
              className="s6lib-tile"
              href={`/s6/games/${g.key}`}
              style={{ textDecoration: "none", ["--s6-accent" as string]: accent, borderRadius: 14, display: "block" }}
            >
              <Panel
                style={{
              ...
              >
                ...
                >
                  ...
                  <img
                    className="s6lib-cover"
                    src={`/s6-art/games/${g.key}/card.webp`}
                    alt=""
                    loading="lazy"
                    style={{
                      position: "absolute",
      ...
      <nav style={{ display: "flex", justifyContent: "center", gap: 10, marginTop: 16, flexWrap: "wrap" }}>
        {[
          { href: "/s6", label: d.common.back },
          { href: "/s6/board", label: d.links.board },
        ].map((l) => (
          <Link
            key={l.href}
            href={l.href}
...
const SHELF_CSS =
  ".s6lib-tile,.s6lib-hero{transition:transform .2s ease,box-shadow .2s ease,filter .2s ease;}" +
  ".s6lib-tile:hover,.s6lib-tile:focus-visible,.s6lib-hero:hover,.s6lib-hero:focus-visible{" +
  "box-shadow:0 24px 54px rgba(0,0,0,.62),0 0 34px var(--s6-accent);outline:none;}" +
  ".s6lib-cover{transition:transform .45s ease;}" +
  ".s6lib-tile:hover .s6lib-play,.s6lib-tile:focus-visible .s6lib-play{" +
  "background:var(--s6-accent);color:#0b0d10;border-color:var(--s6-accent);}" +
  "@media (prefers-reduced-motion: no-preference){" +
  ".s6lib-tile:hover,.s6lib-tile:focus-visible{transform:translateY(-6px) scale(1.015);}" +
  ".s6lib-hero:hover,.s6lib-hero:focus-visible{transform:translateY(-3px);}" +
  ".s6lib-tile:hover .s6lib-cover,.s6lib-tile:focus-visible .s6lib-cover,.s6lib-hero:hover .s6lib-cover{transform:scale(1.055);}" +
  ".s6lib-hero:hover .s6lib-heroBtn{filter:brightness(1.08);}" +
  "}";
