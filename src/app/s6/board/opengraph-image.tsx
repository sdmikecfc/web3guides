...
export default async function BoardOpengraphImage() {
  ...
  const t = snap.theme;
  const top = snap.topPilots.slice(0, 5);
  ...
  try {
    const res = await fetch(`${proto}://${host}/s6-art/pilot/vega.png`, { cache: "no-store" });
    if (res.ok) {
        ...
        <div style={{ display: "flex", flexDirection: "column", padding: "48px 54px", width: 760, position: "relative" }}>
          ...
          <div style={{ fontSize: 25, color: "#34d399", marginBottom: 26, display: "flex" }}>
            {snap.pool.secured > 0
              ...
              : `${usd(POOL_FULL_USD)} season pool · a slice for every mainframe`}
          </div>
          ...
          {top.length ? (
            ...
            <div style={{ fontSize: 27, color: "#aab4bd" }}>
              The board opens with the season. Hold a mainframe from $5 to be on it.
            </div>
          )}
          ...
          <div style={{ fontSize: 22, color: "#87919b", marginTop: "auto", display: "flex" }}>
            {snap.empty
              ? "launchwars.xyz"
              : `${snap.totals.bonded} of ${snap.totals.total} ${t.target.plural} ${t.bondedWord} · ${snap.totals.players} pilots`}
          </div>
        </div>
      </div>
    ),
