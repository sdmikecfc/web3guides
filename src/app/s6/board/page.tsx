...
import { POOL_FULL_USD, SLICE_MAX_USD, SLICE_MIN_USD } from "@/lib/s6/games";
...
const SHARE_DESC =
  "Who is carrying the uprising, and where the season pool goes: one pot, a slice for every mainframe, paid out by the percent each one reached.";
...
export const metadata = {
  title: SHARE_TITLE,
  description: "Top pilots, the war effort, and the pool.",
...
export default async function S5Board() {
  ...
  return (
    ...
    <PageShell
      backdrop={{
        backgroundImage:
          "linear-gradient(100deg, rgba(8,10,13,0.96) 0%, rgba(8,10,13,0.92) 40%, rgba(8,10,13,0.62) 70%, rgba(8,10,13,0.74) 100%), url(/s6-art/pilot/vega.png)",
    ...
    >
      ...
      {/* WHERE ALL THE MONEY IS. ONE envelope since 2026-08-16: the bounties
          ...
          per mainframe and paid out by the percent each one reached. So the
          headline is the constant itself, with no second number to reconcile,
          and the pair below is "the whole pool" plus "what one mainframe is
      ...
      <section style={{ marginBottom: 26 }}>
        ...
        >
          {/* the model in one line, carrying no figure of its own: with a
              single envelope there is nothing left to reconcile against the
              headline */}
          {d.board.moneyLead}
        </p>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
          ...
          {/* NOT a second envelope: the slice BAND, so the player can see what
              one mainframe is worth without reading a table. The per-mainframe
              figure and what it has earned live on that mainframe's own card. */}
          <MoneyCard
      ...
      <p style={{ textAlign: "center", fontSize: 13, color: UI.faint, marginTop: 34 }}>
        <Link href="/s6" style={{ color: UI.muted }}>{d.common.back}</Link>
        {" · "}
        <Link href="/s6" style={{ color: UI.muted }}>{d.links.map}</Link>
        {" · "}
        <Link href="/s6/play" style={{ color: UI.muted }}>{d.links.arcade}</Link>
      </p>
    </PageShell>
  );
}
