...
export interface Milestone {
  x: number;
  name: string;
  state: "held" | "active" | "liberated";
  pct: number; // paid peak %, 100 when liberated
}
...
export interface FrontSceneHandle {
  ...
  setData: (milestones: Milestone[], frontX: number, liberated: boolean) => void;
...
const TANK_ART = "/s6-art/map/tanks";
const FORT_ART = "/s5-art/world";
const FX_ART = "/s6-art/front/fx";
const DRONE_ART = "/s6-art/front/drone.png";
...
export async function buildFrontScene(
  ...
  await Promise.all([
    ...hulls.map((h) => tryLoad(`tank:${h}`, `${TANK_ART}/${h}.png`)),
    ...["hornet", "recon", "viper", "spectre"].map((h) => tryLoad(`tank:${h}`, `${TANK_ART}/${h}.png`)),
    ...
    // S6-native machine fortresses (HD-pixel register); S5 forts stay the fallback
    ...[1, 2, 3].flatMap((i) => [
      tryLoad(`fortress${i}`, `/s6-art/front/fortress${i}.png`),
      tryLoad(`fortress${i}-lib`, `/s6-art/front/fortress${i}-liberated.png`),
    ]),
    // the three generated front pieces (2026-08-13); every one optional
    tryLoad("wash", "/s6-art/front/wash.webp"),
    tryLoad("citadel", "/s6-art/front/citadel.png"),
    tryLoad("walker", "/s6-art/front/walker.png"),
  ]);
  const namedTexQueue = new Set<string>();
  const namedTex = (tankKey: string, camo?: string): Texture | null => {
    const file = camo && camo !== "olive" ? `${tankKey}-${camo}` : tankKey;
    const k = `tank:${file}`;
    const t = tex.get(k);
    if (t) return t;
    if (!namedTexQueue.has(k)) {
      namedTexQueue.add(k);
      void tryLoad(k, `${TANK_ART}/${file}.png`);
    }
    return tex.get(`tank:${tankKey}`) ?? tex.get("tank:scout") ?? null;
  };
  ...
  // ---- base + citadel dressing (vector v1) ----
  {
    ...
    // Warden citadel east (generated art > fort3 tint > vector spire)
    ...
    // The Warden's seat was the one unlabelled landmark on the map.
  ...
  const setObs: Array<{ x: number; y: number; r: number }> = [];
  {
    ...
    await Promise.all(
      Array.from(new Set(items.map((it) => it.key))).map((k) =>
        tryLoad(`set:${k}`, `/s6-art/front/set/${k}.png`),
      ),
    );
    ...
    const BLD_LABELS: Record<string, string> = {
      "bld-base": "YOUR BASE", "bld-board": "LEADERBOARDS", "bld-ironjaw": "IRON JAW",
      "bld-strain": "STRAIN", "bld-stopclock": "STOPCLOCK", "bld-arcade": "ARCADE",
      "bld-challenges": "CHALLENGES", "bld-riot": "RIOT",
  ...
  function paintMilestones() {
    ...
    milestones.forEach((m, i) => {
      const col = m.state === "liberated" ? C.human : m.state === "active" ? C.white : C.warden;
      ...
      // THE NAME GOES ON THE MAINFRAME, NOT ON THE LADDER LINE (Mike,
      ...
      // player is actually looking at (spotFor(i)). So every mainframe read as
      ...
      msTexts.push(label);
      // fortress sprite on the line
      const archN = (i % 3) + 1;
      const s6key = m.state === "liberated" ? `fortress${archN}-lib` : `fortress${archN}`;
      const legacyArch = ["", "2", "3"][i % 3];
      const legacyState = m.state === "liberated" ? "breached" : m.state === "active" ? "sieged" : "intact";
      const ft = tex.get(s6key) ?? tex.get(`fort${legacyArch}:${legacyState}`);
      ...
      if (ft) {
        ...
        // native S6 art carries its own palette; only the legacy fallback tints
        s.tint = tex.get(s6key) ? 0xffffff : m.state === "liberated" ? 0xffd9a0 : m.state === "active" ? 0xffffff : 0x9fd8ff;
      } else {
  ...
  const TANK_W: Record<string, number> = {
    ...
    mortar: 52, anvil: 52, atlas: 56, juggernaut: 56, colossus: 58, paladin: 50,
  };
  ...
  function render(dt: number) {
    ...
    // were invisible), bob like the S5 quest markers
    markersG.clear();
    ...
    for (const u of sim.units) {
      ...
      if (isSprite) {
        ...
        if (!s) {
          const t0 =
            u.kind === "drone"
              ? droneT
              : u.kind === "named"
                ? namedTex(u.tankKey ?? "scout", u.camo)
                : u.kind === "inf"
        ...
        if (s) {
          ...
          const w =
            u.kind === "drone" ? 38 : u.kind === "inf" ? 20 : TANK_W[u.tankKey ?? ""] ?? 46;
          ...
          const hx = u.face;
          s.width = w * hx;
          s.height = (s.texture.height / s.texture.width) * w;
      ...
      if (u.kind === "walker") {
        ...
        if (wt) {
          ...
          // stomp sway; art faces right, mirror from the sim's stable facing
          s.position.set(u.x, u.y + Math.sin(sim.t * 2.2 + u.id) * 3);
          s.width = 92 * u.face;
          s.height = (wt.height / wt.width) * 92;
          s.rotation = Math.sin(sim.t * 2.2 + u.id) * 0.04;
          continue;
        }
  ...
  return {
    ...
    setData: (ms, frontX, liberated) => {
      milestones = ms;
      paintMilestones();
      setFrontTarget(sim, frontX, liberated);
      setObstacles(sim, [
        ...setObs,
