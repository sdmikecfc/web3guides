 ...
 * unit carries a role derived from its tank key's class in lib/s6/tanks.
 ...
 *             band and hold it
 * The Warden mirrors the grammar: walkers lob from deep, drones swarm close,
 * its infantry holds the line.
 */

import { tankByKey } from "../../../lib/s6/tanks";
...
export type Side = 0 | 1; // 0 = UPRISING (west), 1 = WARDEN (east)
...
/** Sprite pools for the unnamed UPRISING armor (scene draws u.tankKey). */
...
export function createFrontSim(opts: {
  ...
  for (const nm of (opts.named ?? []).slice(0, 40)) {
    ...
    u.camo = nm.camo;
    // the pilot's REAL hull decides the band: a mortar main lobs from deep
      ...
      if (hit) {
        ...
        if (target.hp <= 0) {
          if (target.kind === "named") {
            // named pilots never die on camera: fast fallback + heal
