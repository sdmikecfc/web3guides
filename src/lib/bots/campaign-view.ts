...
export const CAMPAIGN_PERIODS = ["week1", "week2", "week3", "week4", "final"] as const;
export type CampaignPeriod = typeof CAMPAIGN_PERIODS[number];
export const CAMPAIGN_CATEGORIES = ["roi", "volume", "profit", "battles"] as const;
export type CampaignCategory = typeof CAMPAIGN_CATEGORIES[number];
export const CAMPAIGN_LABEL: Record<CampaignCategory, string> = { roi: "Trading return", volume: "Trading volume", profit: "Trading profit", battles: "Battle points" };
export const CAMPAIGN_PRIZES = { weekly: [25, 15, 10], final: [150, 100, 50] } as const;
...
export const emptyCampaign = (period: CampaignPeriod = "final", source: CampaignView["source"] = "unavailable"): CampaignView => ({
  ...
  standings: { roi: [], profit: [], volume: [], battles: [] }, earning: null, domaMcp: "pending",
});
...
export function publicStandings(raw: unknown, names: ReadonlyMap<string, string>): Record<CampaignCategory, CampaignStanding[]> {
  const result = { roi: [], profit: [], volume: [], battles: [] } as Record<CampaignCategory, CampaignStanding[]>;
