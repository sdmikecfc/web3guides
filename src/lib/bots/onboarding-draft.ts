...
export function parseDraftOffers(raw: unknown): DraftOffers | null {
  ...
  for (const socket of BEGINNER_ORDER) {
    ...
    const offer = beginnerOffer(value[socket]);
...
export function draftPreview(view: OnboardingView): { build: Build; parts: OwnedPart[] } {
  ...
  for (const socket of BEGINNER_ORDER) {
    const offer = beginnerOffer(view.draftOffers?.[socket]);
    if (!offer || offer.part.slot !== EQUIPMENT_KIND[socket]) continue;
    const part: OwnedPart = { ...offer.part, s: [1, 1, 1], uid: `draft-v2-${socket}`, salvage: 0, provenance: "Starter build preview", ...(offer.color ? { paint: offer.color } : {}) };
