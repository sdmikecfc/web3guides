...
export interface HitProxyV6 { slot:BodySocketV6; center:Vec3; half:Vec3; shape?:"box"|"capsule"; a?:Vec3;b?:Vec3;radius?:number;
  /** Runtime-only rigid segment mapping back into the canonical unposed part. */
  pose?:{segment:"upper"|"lower";origin:Vec3;restOrigin:Vec3;rotation:QuatV6;restCenter:Vec3;restHalf:Vec3};
}
...
export interface ContactKeyV6 { time:number; point:Vec3; normal:Vec3;/** Optional authored twist about the striking normal, in milliradians. */roll?:number }
...
export interface BodyCollisionV6 { radius:number; height:number; proxies:readonly HitProxyV6[];mounts:Readonly<Record<AttachmentNameV6,AttachmentFrameV6>>;arms?:Readonly<Record<"left"|"right",ArmRigV6>> }
...
export interface CollisionSnapshotV6 { version:string; rigVersion:string; manifestHash:string; radius:number; height:number; proxies:readonly HitProxyV6[];mounts:Readonly<Record<AttachmentNameV6,AttachmentFrameV6>>;arms:Readonly<Record<"left"|"right",ArmRigV6>>;weapon:WeaponProxyV6 }
...
export function footprintSupportV6(proxies:readonly HitProxyV6[],alive:readonly number[],direction:Vec3):number {
  ...
  for(const p of proxies){if(alive[BODY_SOCKETS_V6.indexOf(p.slot)]<=0)continue;const value=p.shape==="capsule"?Math.max(p.a![0]*n[0]+p.a![2]*n[2],p.b![0]*n[0]+p.b![2]*n[2])+p.radius!:p.center[0]*n[0]+p.center[2]*n[2]+Math.abs(n[0])*p.half[0]+Math.abs(n[2])*p.half[2];support=Math.max(support,value);}
  return Math.max(100,support);
}
export function footprintRadiusV6(proxies:readonly HitProxyV6[]):number{return Math.ceil(Math.max(...proxies.map(p=>p.shape==="capsule"?Math.max(Math.hypot(p.a![0],p.a![2]),Math.hypot(p.b![0],p.b![2]))+p.radius!:Math.hypot(Math.abs(p.center[0])+p.half[0],Math.abs(p.center[2])+p.half[2]))));}
...
export function validateCollisionManifestV6(raw:CollisionManifestV6):boolean {
  ...
  return bodies.every(b=>b&&Number.isFinite(b.radius)&&b.radius>=250&&b.radius<=1100&&Number.isFinite(b.height)&&b.height>=800&&b.height<=4000&&(!b.arms||[b.arms.left,b.arms.right].every(a=>a&&vector(a.upper)&&vector(a.lower)&&vector(a.pole)&&Math.hypot(...a.upper)>50&&Math.hypot(...a.lower)>50&&a.upperRadius>0&&a.upperRadius<=500&&a.lowerRadius>0&&a.lowerRadius<=500))&&b.mounts&&[...BODY_SOCKETS_V6,"handL","handR","shoulderL","shoulderR"].every(k=>{const f=b.mounts[k as AttachmentNameV6];return f&&vector(f.position)&&vector(f.rotation,6284);})&&Array.isArray(b.proxies)&&b.proxies.length===6&&new Set(b.proxies.map(p=>p.slot)).size===6&&b.proxies.every(p=>BODY_SOCKETS_V6.includes(p.slot)&&vector(p.center)&&vector(p.half)&&p.half.every((n:number)=>n>0&&n<=1500)&&(!p.shape||p.shape==="box"||p.shape==="capsule")&&(p.shape!=="capsule"||vector(p.a)&&vector(p.b)&&Number.isFinite(p.radius)&&p.radius!>0&&p.radius!<=1000)))&&weapons.every(w=>w&&vector(w.grip)&&vector(w.muzzle)&&vector(w.strikePoint)&&vector(w.strikeNormal)&&(!w.offhandGrip||vector(w.offhandGrip))&&(!w.backupMuzzle||vector(w.backupMuzzle))&&(!w.specialMuzzle||vector(w.specialMuzzle))&&(!w.strikeSegments||Array.isArray(w.strikeSegments)&&w.strikeSegments.length>=1&&w.strikeSegments.length<=8&&w.strikeSegments.every(e=>vector(e.a)&&vector(e.b)&&Number.isFinite(e.radius)&&e.radius>=5&&e.radius<=300))&&Number.isFinite(w.radius)&&w.radius>=5&&w.radius<=500&&Array.isArray(w.active)&&w.active.length===2&&w.active[0]>=0&&w.active[1]<=1&&w.active[0]<w.active[1]&&Array.isArray(w.path)&&w.path.length>=2&&w.path.length<=32&&w.path[0].time===0&&w.path[w.path.length-1].time===1&&w.path.every((k,i)=>Number.isFinite(k.time)&&k.time>=0&&k.time<=1&&(!i||w.path[i-1].time<k.time)&&vector(k.point)&&vector(k.normal)&&Math.hypot(...k.normal)>.5&&(k.roll===undefined||Number.isFinite(k.roll)&&Math.abs(k.roll)<=6284)));
}
...
export function sweepRobotV6(from:Vec3,to:Vec3,previous:{x:number;z:number;yaw:number},current:{x:number;z:number;yaw:number},proxies:readonly HitProxyV6[],alive:readonly number[],radius=0,previousProxies:readonly HitProxyV6[]=proxies):SweepContactV6|null {
  ...
  for(const proxy of proxies){if(alive[BODY_SOCKETS_V6.indexOf(proxy.slot)]<=0)continue;const prior=previousProxies.find(p=>p.slot===proxy.slot&&p.pose?.segment===proxy.pose?.segment)??proxy;const contact=proxy.shape==="capsule"?sweepMovingCapsuleV6(localFrom,localTo,prior.a??proxy.a!,prior.b??proxy.b!,proxy.a!,proxy.b!,proxy.radius!,radius):sweepBoxV6(localFrom,localTo,proxy.center,proxy.half,radius);if(!contact||nearest&&contact.t>=nearest.t)continue;
...
export function sampleWeaponPathV6(proxy:WeaponProxyV6,time:number,mount:"left"|"right",targetHeight?:number):{point:Vec3;normal:Vec3;roll?:number}{
  ...
  if(mount==="left"){p[0]*=-1;n[0]*=-1;}return {point:p,normal:n,...(a.roll!==undefined||b.roll!==undefined?{roll:(a.roll??0)+angle(((b.roll??0)-(a.roll??0))/1000)*1000*u}:{})};
}
