...
import { DEFAULT_COLLISION_VERSION_V6 } from "./default-collision";
...
export function statsV6(raw:CombatBuild):StatsV6 {
  ...
  const armour=BODY_SOCKETS_V6.map(slot=>{const c=cards[slot];if(!c)return 0;return Math.round((slot==="torso"?150+c.gp*2+a.health*4:slot==="head"?40+c.gp:35+c.gp*.95+c.s[1])*1.8);});
  ...
  return {...a,gp,armour,plating,guard:8+a.block*2,movement:35+a.speed*1.5+legs[1]*.8,turnRate:75+Math.min(50,a.speed*3),aimError:Math.round(235/(1+a.acc*.035)),evasion:Math.min(45,8+a.dodge*1.1+a.speed*.35),force:a.dmg+a.str*.4,power,attackRate:1+a.atkSpd*.014,heatCapacity:100,cooling:34/60};
}
...
function collisionSnapshot(parts:Record<SocketV6,PartV6>,version:string):CollisionSnapshotV6 {
  ...
  return {version,rigVersion:manifest.rigVersion,manifestHash:hashV6(manifest),radius:Math.max(body.radius,footprintRadiusV6(proxies)),height:body.height,proxies,arms,mounts:cloneV6(body.mounts),weapon:scaled};
}
