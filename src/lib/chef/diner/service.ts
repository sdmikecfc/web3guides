...
export function sanitizeService(raw:unknown):ServiceState|null {
  try {
    ...
    for(const station of s.stations){
      const capacity=EQUIPMENT_BY_ID[station.kind].tiers[station.tier-1].capacity;
      ...
      for(const slot of station.slots){
        ...
        if(slot.batch&&(!s.config.batchVersion||station.kind!=='fryer'||slot.item?.recipeId!=='fries'||!validateFryBatch(slot.batch)||slot.batch.createdTick>s.tick||!slot.job||(slot.batch.phase==='cooking'&&slot.job.ready)||(slot.batch.phase!=='cooking'&&!slot.job.ready)||(slot.batch.phase==='raised'&&slot.job.burnRemaining!==null)||(slot.batch.phase==='burnt'&&slot.item.kind!=='burnt')||(slot.batch.phase!=='burnt'&&slot.item.kind==='burnt')))return null;
        if(slot.boil){
          ...
          const output=RECIPE_BY_ID[item.recipeId].steps[0].output,drained=item.recipeId==='tomato_pasta'?'drained_pasta':'drained_noodles';
...
export function serviceTargetIntent(s:ServiceState,targetId:string,seatId?:string,recipeId?:string,itemId?:string):ServiceIntent {
  ...
  if(table){const seats=seatId?table.seats.filter(seat=>seat.id===seatId):table.seats;if(!held)return seats.some(seat=>seat.item?.kind==='dirty')?{label:'Clear dirty dish',disabled:false,hold:false}:stop('No dirty dish here');
  ...
  if(s.config.physicalSupplies&&['plates','cups','boxes','bowls'].includes(station!.kind)){const kind:VesselKind=station!.kind==='cups'?'cup':station!.kind==='boxes'?'fry_box':station!.kind==='bowls'?'bowl':'plate',name=SERVING_VESSELS[kind].name,count=kind==='fry_box'?Infinity:stockFor(s,kind).length;return held?.kind==='plate'&&vesselOf(held)===kind?{label:`Return ${name}`,disabled:false,hold:false}:held?stop('Put down held item first'):count>0?{label:`Take ${name}${Number.isFinite(count)?' · '+count+' left':''}`,disabled:false,hold:false}:stop(`Wash a ${name} first`);}
  ...
  if(preparedFood(held)&&station!.slots.some(slot=>slot.item?.kind==='plate'&&vesselOf(slot.item)===foodVessel(s,held!)))return {label:'Fill the clean serving dish',disabled:false,hold:false};
  if(held?.physical&&station!.kind==='prep'){const assembly=station!.slots.find(slot=>slot.item&&((held.kind==='ingredient'&&held.ingredientId&&missingAssembly(slot.item).includes(held.ingredientId))||(slot.item.kind==='ingredient'&&slot.item.ingredientId&&missingAssembly(held).includes(slot.item.ingredientId))));if(assembly){const main=missingAssembly(held).length?held:assembly.item!;return {label:RECIPE_BY_ID[main.recipeId].steps[main.step]?.label??'Combine ingredients',disabled:false,hold:false};}}
  if(station!.kind==='bin')return held?.kind==='dirty'?stop('Wash this dish at the sink'):held?{label:'Discard held food',disabled:false,hold:false}:stop('Nothing to discard');
  if(!held&&station!.slots.some(slot=>slot.item&&slot.boil?.phase==='ready'))return {label:'Lift & drain noodles',disabled:false,hold:false};
...
function routeInteraction(s:ServiceState,targetId:string,recipeId:string|null,seatId:string|null,ingredientId:string|null=null,itemId:string|null=null):void {
  ...
  if(itemId&&(station?.kind!=='pass'||s.chef.held||!station.slots.some(slot=>slot.item?.id===itemId))){notice(s,s.chef.held?'Put down held item first.':'That stored item is no longer here.');clearTarget(s);return;}
  ...
  s.chef.path=path;s.chef.targetId=targetId;s.chef.targetRecipeId=recipeId;s.chef.targetSeatId=seatId;s.chef.targetIngredientId=ingredientId;s.chef.targetItemId=itemId;s.chef.holding=false;
...
function interactStation(s:ServiceState,station:ServiceStation):void {
  ...
  if(s.config.physicalSupplies&&['crate','fridge','plates','cups','boxes','bowls'].includes(station.kind)){
    if(['plates','cups','boxes','bowls'].includes(station.kind)){
      const kind:VesselKind=station.kind==='cups'?'cup':station.kind==='boxes'?'fry_box':station.kind==='bowls'?'bowl':'plate';
  ...
  if(!held){const readyBoil=station.slots.find(slot=>slot.item&&slot.boil?.phase==='ready');if(readyBoil){readyBoil.boil!.phase='drained';readyBoil.item!.stage=readyBoil.item!.recipeId==='tomato_pasta'?'drained_pasta':'drained_noodles';notice(s,'Basket lifted and drained. Take the noodles to the prep counter.');clearTarget(s);return;}}
  const ready=station.slots.find(slot=>slot.item&&(!slot.job||slot.job.ready));
  if(!held&&ready) {
    chef.held=ready.item;ready.item=null;ready.job=null;ready.batch=null;ready.boil=null;notice(s,`Picked up ${itemLabel(chef.held).toLowerCase()}.`);clearTarget(s);return;
  }
  if(!held) {
  ...
  if(step) {
    ...
    empty.item=held;chef.held=null;if(s.config.batchVersion&&station.kind==='fryer'&&held.recipeId==='fries')empty.batch=createFryBatch(s.tick);if(station.kind==='boiler')empty.boil=createBoilBasket(held.recipeId,s.tick);empty.job={action:step.action,remaining:step.ticks,total:step.ticks,burnRemaining:step.burnTicks??null,ready:false};
...
function interactTable(s:ServiceState,table:ServiceTable):void {
  ...
  if(!seat){notice(s,held?'Nobody here ordered that dish.':'There is no dirty dish here.');clearTarget(s);return;}
  if(!held) {
...
function finishCooking(s:ServiceState,station:ServiceStation,slot:StationSlot):void {
  ...
  if(slot.boil)slot.boil.phase='ready';
