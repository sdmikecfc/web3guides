/**
 * S7 FTUE quest chain, client-safe helpers.
 *
 * The footlocker panel is a 5-quest log that walks a new adventurer from first
 * tap to first hold. Quests 1 and 2 are PRE-WALLET and tracked in localStorage
 * under "s7_ftue" (merged visually with the server truth once a session
...
export const FTUE_STORAGE_KEY = "s7_ftue";
...
/** Known earned decals; anything else renders as a generic field banner.
 * KEYS ARE FROZEN (they live in hq.decals and the vault rotation); only the
 * display names wear the season. */
export const DECAL_LABELS: Record<string, string> = {
  "first-colors": "First Banner",
  "division-star": "Guild Star",
  "iron-discipline": "Iron Oath",
  convoy: "Caravan",
};

/** Display label for one decal key (known name, else a tidied generic banner). */
...
/** 1 bond XP per 100 Valor (display math only; the bot owns the tier). */
