/**
 * SEASON 5 (REALMFALL) — THE WORLD DESCRIPTOR.
 *
 ...
 * keeps or Valor. S7 = copy this file, move the coordinates, rename the
 ...
 *   UPPER TWO     THE FRONT. Eleven keeps in a serpentine advance, in
 *   THIRDS        LISTING ORDER (first listing bottom-left, last top-left), so
 *                 the map reads as a campaign moving up the board.
 *   RIGHT EDGE    The muster field (other adventurers) and the Power Station,
 *                 fenced off and cold.
 *
 ...
 * English on purpose: STRINGS in lib/s7/strings.ts is typed `S7Dict = typeof
...
const S7_WORLD_AUTHORED: WorldConfig = {
  // S5 world art on purpose (season.config documents this): an s7 world art
  // folder was never shipped, and the fort*/bg-land plates live in s5-art.
  artRoot: "/s5-art/world",
  ...
  /**
   * A painted PLACE maps to whichever game currently sits on it. The S7 slate
   ...
   * keeps stand in the scorched west - t0-t4 the near rank along the
   ...
   *   2. the front road following the line of the keeps
  ...
  sites: [
    ...
    //    popups off the base instead. ────────────────────────────────────────────
    {
      ...
      kind: "home",
      label: "Your Base",
      blurb: "Your tank, your adventurer, your kit.",
      pos: { x: 0.815, y: 0.76 },
    ...
    // ── THE RIGHT EDGE ───────────────────────────────────────────────────────
    {
      ...
      glyph: "signpost",
      accent: TAN,
      // WAS a route to /s7/explore, a second whole page for looking at other
      // adventurers. They stand on THIS map now, so that page was showing an
 ...
 */
export const S7_TARGET_SCALE = [1.05, 1.3, 1.04, 1.0, 1.08, 1.22, 1.16, 1.5, 1.06, 1.06];
...
export const S7_CAMPS = FIT_CAMPS;

export const S7_GAME_FALLBACK = {
  label: "Proving Ground",
  blurb: "Field exercises. Small Valor, big bragging rights.",
  href: "/s7/play",
};
 ...
 */
export const S7_WORLD_PATHS = FIT_PATHS.length ? FIT_PATHS : FIT_ROADS;

export const S7_WORLD_PADS = FIT_PADS;
export const S7_WORLD_TRACKS = FIT_TRACKS;

export const S7_WORLD: WorldConfig = {
  ...S7_WORLD_AUTHORED,
  ...
  sites: S7_WORLD_AUTHORED.sites.map(
    (s): WorldSite => {
      const pos = FIT_SITES[s.key] || FIT_SITES[FIT_KEY_ALIAS[s.key]];
      return pos ? { ...s, pos } : s;
    },
  ),
  targetSlots: FIT_SLOTS.length ? FIT_SLOTS : S7_WORLD_AUTHORED.targetSlots,
};
