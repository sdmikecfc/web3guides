/**
 * S7 funding wizard constants (client-safe: shared by the HQ wizard UI and the
 * /api/s7/funding-status route). Chain facts for Doma plus the two funding
...
/** Deep link to buy one keep on the Doma app. */
...
/** Enough USDC.e to enter a keep: $5 (in 6-decimal units). */
...
export type FundingState =
  ...
  | "FUNDED_NOT_BOUGHT" // armed: USDC.e + gas, no keep token yet
  | "HOLDER" // holds at least one keep token
...
export type FundingStatus = {
  ...
  usdcUsd: number;
  /** Keep tokens with a balance > 0 (raw token units as strings;
