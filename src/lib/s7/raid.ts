/**
 * S7 RAID schedule + odds, pure client-safe logic (no IO, no secrets, no
 * imports). This is a WEB MIRROR of the bot's boss scheduler: the bot rolls
 * the raid at 14:00 UTC against the roster in the s7_battle_roster config row;
 ...
 *
 * Join-first rule: a adventurer who enlists before 14:00 UTC fights TODAY, so
...
export const SEASON_URL = "https://launchwars.xyz/?ref=rally";
 ...
 * KEYS are frozen (they live in the s7_battle_roster DB row and the bot's
 * scheduler rolls on them); only the display NAMES wear the season. */
export const RAID_BOSSES: Record<string, RaidBoss> = {
  quartermaster: { key: "quartermaster", name: "THE CRYPT KEEPER", difficulty: 0.55 },
  landship: { key: "landship", name: "THE BONE DRAGON", difficulty: 0.95 },
  "foundry-beast": { key: "foundry-beast", name: "THE GRAVE TITAN", difficulty: 1.0 },
  "rail-colossus": { key: "rail-colossus", name: "THE BONE COLOSSUS", difficulty: 1.05 },
...
/** Roster count out of the s7_battle_roster config value (JSON
...
export function adventurersWord(n: number): string {
...
export function stripLine(boss: RaidBoss, count: number, odds: number): string {
  return `🚨 ${boss.name} rolls at 14:00 UTC · ${count} ${adventurersWord(count)} enlisted · estimated odds ${odds}%. Every adventurer raises them.`;
}
...
export function tweetText(boss: RaidBoss, count: number, odds: number): string {
  return `🚨 ${boss.name} rolls at 14:00 UTC. ${count} ${adventurersWord(count)} enlisted, estimated odds ${odds}%. Enlist before 14:00 UTC. ${SEASON_URL}`;
}
...
export function resultTweetText(bossName: string, won: boolean): string {
  return won
    ? `⚔️ THE GUILD BROUGHT DOWN ${bossName.toUpperCase()}. Daily co-op raid in Launch Wars REALMFALL: free browser hero games, real domain reclaims. Join tomorrow's raid: ${SEASON_URL}`
    : `🩸 ${bossName.toUpperCase()} HELD THE WALL. The Guild needs more adventurers for tomorrow's raid. Joining is free and every extra player raises the odds: ${SEASON_URL}`;
}
