/**
 * THE WAR EFFORT (web side), Launch Wars S7.
 *
 * A adventurer COMMITS Gold (the play currency) to ONE keep as declared
 * war effort. Breach = the commitment returns DOUBLED plus that keep's
 * cosmetic decal. Fail = the Gold are gone. The window CLOSES once a
 * keep enters its sprint band (the 85% tripwire), so nobody can back a
 ...
 * touches Valor, the pool, or any USD figure: settlement weight stays
 * Valor x breadth and no cash cut can move because of this file.
 *
...
export const WAR_EFFORT_TABLE = "launch_wars_s7_war_effort";
 ...
 * so committing Gold to it was close to risk free and the "bet" was a
...
/** Public, per-keep aggregate. No wallets, no dollars. */
export type WarEffortTotal = {
  domain: string;
  /** Gold riding on this wall right now (open commitments only). */
  committed: number;
  /** How many adventurers have declared for it. */
  adventurers: number;
...
/** A keep's own decal key (mirrors modules/season5 warEffortDecal). */
...
/**
 * Is the commitment window open on this keep? The web copy of the bot's
...
export async function warEffortTotals(db?: Db): Promise<Map<string, WarEffortTotal>> {
  ...
  try {
    ...
    for (const r of data || []) {
      ...
      const agg = out.get(domain) || { domain, committed: 0, adventurers: 0, won: 0, lost: 0 };
