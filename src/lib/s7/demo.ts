 ...
 * watch the loop at all. `?demo=1` gives it to you: a funded adventurer, the
 ...
 * crosses to the server, no leaderboard moves, no Gold are minted. The banner
 * (see DemoBadge) is there so nobody is confused about which they are looking
 * at, not because the state is dangerous.
 */
// TYPE-ONLY import from the S7 tree (erased at compile time, zero runtime
// coupling): the s7 app tree does not exist yet. Re-point at
// "@/app/s7/hq/panels" when the day-3 surfaces port lands.
import type { HqMe } from "@/app/s7/hq/panels";
import { resolveTank, STAT_MILESTONE_TANKS } from "@/lib/s7/model";
import { statNextPrice, type StatKey } from "@/lib/s7/games";
import { TANK_ROSTER, TANK_TIER_PRICES } from "@/lib/s7/tanks";
import { vaultDayNumber, vaultDealForDay } from "@/lib/s7/vault";
...
/**
 * A adventurer who can afford EVERYTHING.
 *
 * This started at 420 Gold so the can't-afford path stayed reachable, which
...
export function demoHqMe(): HqMe {
  return {
    session: true,
    // The demo adventurer has no public garage to link to, so the "see your own
    ...
    // FIELD names track the S7 HqMe alias above until the s7 app tree lands
    // (day 3): flip commander/ownedAdventurers to adventurer/ownedAdventurers with it.
    adventurer: "diesel",
...
export function demoReply(url: string, body: unknown): Record<string, unknown> {
  ...
  if (url.includes("/tank-unlock")) {
    ...
    if (price > wallet.shells) {
      return { error: `Not enough Gold. ${price} needed, ${wallet.shells} banked.` };
    }
    wallet.shells -= price;
  ...
  if (url.includes("/vault-buy")) {
    ...
    if (deal.price > wallet.shells) {
      return { error: `Not enough Gold. ${deal.price} needed, ${wallet.shells} banked.` };
    }
  ...
  // /api/s7/upgrade: the stat shop, simulated with the REAL price curve and
  ...
  // /api/s7/hq: cosmetic sets (tank, adventurer, camo) are free. Mutate the
  ...
  return {
    ok: true,
    // panels sends `set: { adventurer }`; checking `set.commander` made adventurer
    // picking a silent no-op under ?demo=1 (fixed 2026-08-15)
    ...(typeof set.adventurer === "string" ? { adventurer: set.adventurer } : {}),
