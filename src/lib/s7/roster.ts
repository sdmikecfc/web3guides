import "server-only";
/**
 * THE ROSTER — every adventurer in the season, readable by anyone.
 *
 * PUBLIC-SAFE BY CONSTRUCTION, and this is the load-bearing rule of the file,
 * copied deliberately from lib/s7/publicHq.ts: no row that leaves here may
 ...
 *
 * WHY ITS OWN FILE rather than an extension of lib/s7/data.ts: getSeasonSnapshot
 * is one cached blob that EVERY s7 page awaits, including the landing. Folding a
 * 500-row roster read into it would make the camp pay for the roster. This
 * follows the lib/s7/warEffort.ts precedent instead — a sibling server-only
...
/** One adventurer, safe for any public surface. */
export type RosterRow = {
  /** Position in the Valor ordering, 1-based. */
  rank: number;
  name: string;
  /** Public garage handle for /s7/hq/<handle>. May be "" when the wallet is
  ...
  stats: RosterStats;
  /** Which keep they camp at on the explore map, or null to muster. */
  domain: string | null;
  /** Active class id, validated against the shipped whitelist; null = none
   * picked yet. Public by design (the TOP ADVENTURERS board already shows
   * class); leaks nothing per the file's law. Drives the battle map's
   * class-figure render (Mike 2026-08-30: "Top players fight as their class"). */
  cls: ClassId | null;
};
...
/** 0..5 from a raw skin value. Mirrors statsOf in /api/s7/me. */
 ...
 * to "everyone musters in the staging area". Mirrors readTopAdventurers. */
type Db = NonNullable<ReturnType<typeof s7Db>>;
...
async function readRoster(cursorRaw: string | null, pageSize: number): Promise<RosterPage> {
  ...
  try {
    // KEYSET, never OFFSET. Valor move continuously (every banked run calls
    // s7_grant), so an OFFSET page 2 taken a second after page 1 duplicates or
    // drops rows. The (points, id) tiebreak is not optional either: everyone
    // starts on 0 Valor, so ties are the COMMON case in week one and without
    // a stable second key the ordering is not deterministic between pages.
    let q = db
      .from("launch_wars_s7_players")
      .select("id, display_name, wallet, points, hq, skin, active_class")
      .eq("season_key", SEASON_KEY)
      .eq("is_test", false)
...
/**
 * The adventurers DRAWN on the explore map.
 *
