/**
 * S6 UPRISING: the pick-first identity roster (ADR-0066).
 *
...
export type Tank = {
  ...
  tier: 1 | 2 | 3 | 4 | 5;
  ratings: TankRatings;
  /** Scrap price to unlock (0 = free starter pick). */
  shellsPrice: number;
  /** One-line in-world blurb for the loadout panel. No em-dashes. */
  blurb: string;
};

/** Scrap price per tier (tier 1 free; tuned vs Scrap income in the econ sim). */
...
/** The enlist default (a free Tier 1 pick until the pilot chooses). */
...
// ── The pilot cast (free pick, swap anytime; outfits are week-1 drops) ──
export type Pilot = {
  key: string;
  name: string;
  gender: "f" | "m";
  ...
  role: string;
  blurb: string;
  /** Earned, never free-picked: fielding it is gated on hq.pilots_owned
   * (today: bigmike, the daily arcade top's prize). Absent = free cast. */
  prize?: true;
};

export const COMMANDERS: Pilot[] = [
  ...
  { key: "granite", name: "Granite", gender: "m", role: "Pilot", blurb: "Old enough to know better, armored enough not to care." },
...
/** The pilot a brand-new visitor meets in the camp before they pick one.
...
export function pilotByKey(key: unknown): Pilot | null {
