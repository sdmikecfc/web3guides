 ...
 *
 * Mike's brief for the mainframe dossier: clicking an enemy base should show
 ...
 *
 * It imports ONLY from lib/s6/games.ts, which is client-safe by design and is
 ...
 * the pool depends on every other pilot's Signal too, so no per-player
 * dollar figure is knowable in advance. lib/s6/hq.ts EDUCATION["why-hold"]
...
export type HoldRow = {
  usd: number;
  /** Base Signal per day before freshness and breadth. */
  medalsPerDay: number;
  /** Signal per day per dollar held, the number Mike actually asked for. */
  perDollar: number;
};
...
/** Build the preview table for a given breadth (how many distinct mainframes
...
/**
 * Freshness multiplier for a newly listed mainframe: up to x(1 + FRESH_BONUS)
...
/** Breadth bonus: +5% daily hold per distinct mainframe held $5+ past the
...
export function curveNote(): string {
  return `Every dollar you hold pays a flat ${HOLD_RATE_PER_USD_DAY} Signal a day, counted up to your ceiling. Holding more mainframes raises the ceiling.`;
}
