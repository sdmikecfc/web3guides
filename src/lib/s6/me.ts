/**
 * Season 5, the session -> player helpers shared by the authed s6 API routes
 * (/api/s6/me, /api/s6/hq, /api/s6/tank-unlock, /api/s6/funding-status,
 * /api/s6/claim-ftue). Server-only.
 *
 * The hq JSONB is SHARED with the bot: the web owns hq.tank / hq.camo /
 * hq.pilot / hq.tanks_owned, the bot owns hq.crates / hq.streak /
 ...
 */
import "server-only";
import type { s6Db } from "./server";
import { ownedCamoKeys, ownedPilotKeys, ownedTankKeys, resolvePilotKey } from "./model";

type Db = ReturnType<typeof s6Db>;

/**
 * Resolve the wallet behind a play-session token (the token /api/s6/game-session
 * mints). Null if malformed, unknown, or expired. Never trust a client address.
 */
import { SEASON_KEY } from "@/lib/s6/server";
...
export type HqView = {
  ...
  ownedCamos: string[];
  /** Earned prize pilots (hq.pilots_owned; today bigmike or nothing). */
  ownedPilots: string[];
