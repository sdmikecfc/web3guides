/**
 * S6 RAID schedule + odds, pure client-safe logic (no IO, no secrets, no
 * imports). This is a WEB MIRROR of the bot's boss scheduler: the bot rolls
 * the raid at 14:00 UTC against the roster in the s6_battle_roster config row;
 ...
 *
 * Join-first rule: a pilot who enlists before 14:00 UTC fights TODAY, so
...
export const RAID_BOSSES: Record<string, RaidBoss> = {
  ...
  siegebreaker: { key: "siegebreaker", name: "THE WARDENBREAKER", difficulty: 1.25 },
};
...
/** Roster count out of the s6_battle_roster config value (JSON
...
export function pilotsWord(n: number): string {
  return n === 1 ? "pilot" : "pilots";
}

/** The strip's one line of copy. */
export function stripLine(boss: RaidBoss, count: number, odds: number): string {
  return `🚨 ${boss.name} rolls at 14:00 UTC · ${count} ${pilotsWord(count)} enlisted · estimated odds ${odds}%. Every pilot raises them.`;
}
...
export function tweetText(boss: RaidBoss, count: number, odds: number): string {
  return `🚨 ${boss.name} rolls at 14:00 UTC. ${count} ${pilotsWord(count)} enlisted, estimated odds ${odds}%. Enlist before 14:00 UTC. ${SEASON_URL}`;
}
...
export function resultTweetText(bossName: string, won: boolean): string {
  return won
    ? `⚔️ THE COLUMN BROUGHT DOWN ${bossName.toUpperCase()}. Daily co-op raid in Launch Wars UPRISING: free browser mech games, real domain liberations. Fly tomorrow's sortie: ${SEASON_URL}`
