/**
 * Launch Wars Season 5 (UPRISING), the season snapshot (service-role read;
 ...
 * the old eleven). Tagging them means POST /api/s6/revalidate can drop every
 * S6 read the moment the operator changes the database.
 */
export const S6_CACHE_TAG = "s6-season";
import { s6Db, SEASON_KEY } from "./server";
import { POOL_FULL_USD, SLICE_MIN_USD, SLICE_MAX_USD, GAMES } from "./games";
import { STRINGS, fill, type S6Dict } from "./strings";
...
export type SeasonTarget = {
  ...
  /** Operator-set pool dollars for THIS mainframe (the S4 SQL 037 money-table
   * pattern); wins over weight math when set. */
  poolShareUsd: number | null;
  /** Operator-set flat breach bounty for THIS mainframe; null when unset.
...
/** One row of the TOP COMMANDERS board: PUBLIC-safe. Name and Signal only,
 * NEVER the wallet and NEVER a personal dollar figure. */
export type TopPilot = {
  rank: number; // leaderboard position 1..N
  name: string; // display_name, or "Pilot" when unnamed. Never the wallet.
  points: number; // Signal, rounded
  /** Public garage handle for /s6/hq/<handle>: the display-name slug, else
  ...
  camo: string;
  pilotKey: string;
  /** The mainframe they hold the most of, lowercased, or null if they hold
...
export type SeasonPool = {
  ...
  unlocked: number;
  full: number;
  /** USD already EARNED for holders under the ADR-0076 peak rule: liberated
   * slices in full plus every standing wall's peak percent of its slice. Each
   * mainframe's part of this is paid to ITS OWN holders. The remainder
  ...
  secured: number;
  /** Real dollars already paid out this season (config key s6_paid_ledger; 0
   * until the first payout is recorded). */
  paidOutUsd: number;
};

/** Season window from the s6_season config row (JSON {launchAt,endAt}). */
 ...
 * SPRINT_ARM_PCT 0.85, SPRINT_HOURS 48, config row s6_sprint_state shaped
...
export type SeasonSprint = {
  /** true while at least one mainframe is inside its 48h window. */
  active: boolean;
...
/** Merged theme (DEFAULT_THEME + the s6_theme config JSON), junk-tolerant. */
async function readTheme(db: Db): Promise<Theme> {
  const raw = await readConfigValue(db, "s6_theme");
...
async function readSeason(db: Db): Promise<SeasonWindow> {
  const raw = await readConfigValue(db, "s6_season");
...
/** Dollars already paid out, from the s6_paid_ledger config row. Accepts a bare
 * number ("250") or JSON ({"total": 250} / {"paid": 250}); anything else is 0. */
async function readPaidOutUsd(db: Db): Promise<number> {
  const raw = await readConfigValue(db, "s6_paid_ledger");
  if (!raw) return 0;
...
/** One usable entry off the s6_sprint_state row. Anything that does not parse
 * into this shape is dropped, so a half-written row degrades to "no sprint"
 * instead of throwing. */
type SprintRow = { domain: string; activeUntilMs: number };

/**
 * Parse the s6_sprint_state config value. Junk-tolerant by construction: a
...
async function readSprintRows(db: Db): Promise<SprintRow[]> {
  return parseSprintRows(await readConfigValue(db, "s6_sprint_state"));
}
...
async function _getSprintRows(): Promise<SprintRow[]> {
  try {
    return await readSprintRows(s6Db());
  } catch {
    return [];
  }
}
const getSprintRows = unstable_cache(_getSprintRows, ["s6-sprint-state"], {
  revalidate: 60,
  tags: [S6_CACHE_TAG],
});
...
export async function getSprint(): Promise<SeasonSprint> {
  try {
    ...
    try {
      return sprintFrom(await readSprintRows(s6Db()), Date.now());
    } catch {
      return NO_SPRINT;
    }
  }
...
async function readTargets(db: Db, nowMs: number): Promise<SeasonTarget[]> {
  try {
    ...
    if (!rows) throw new Error("s6 targets read failed (both column sets)");
    return rows.map((r) => {
    ...
    throw e instanceof Error ? e : new Error("s6 targets read failed");
  }
}

// A cached display_name that IS a raw wallet address must never reach a public
// board. Shorten it to the truncated 0x form; null/blank becomes "Pilot".
...
async function readTopPilots(db: Db): Promise<TopPilot[]> {
  try {
 ...
 * (a banked run pays a flat 10 Signal whatever you scored), so it is pure
...
async function readGameBoards(db: Db): Promise<GameBoard[]> {
  ...
  for (const g of GAMES) {
    ...
    try {
      // Uses s6_scores_board_idx (season_key, game, score DESC) directly. A
      ...
      const { data: raw } = await db
        ...
        .limit(40);
      // PRACTICE RANKS TOO (S5 parity): shown = max(score, meta.practice_best);
...
// (the FDV raise required), so pricey mainframes are worth far more to breach.
 ...
 * map and the money always agree on how big each mainframe is. */
 ...
 * difficulty, floor + cap"). Every mainframe gets a slice of the ONE pot,
 ...
 *
 * Why clamped: pure weighting failed twice on the evidence. Every mainframe
 * needing <= $3.2k has been liberated and every one needing >= $3.5k stalled
 ...
 * pennies (S5 liberated hotcommerce for a $17 slice). The floor makes every
 * mainframe worth showing up for; the cap stops one backdrop hoarding the pot.
 *
 * WATER-FILLING, not a naive clamp: clamping alone breaks the sum. Each pass
 * spreads the remaining pool across the still-free mainframes by weight, pins
 ...
 *
 * STATUS-BLIND (ADR-0076): a mainframe keeps its slice all season whatever
...
function sliceTable(targets: SeasonTarget[], poolUsd: number): Map<SeasonTarget, number> {
  ...
  // Infeasible bands (too many mainframes for the floor, too few for the cap)
  ...
  // Found 2026-08-16 by the bot's cross-check, on S5's real weights: when a
  // pass pins EVERY remaining mainframe to a bound, `rest` empties and the
  ...
  // mainframes that still have headroom under the cap, in proportion to their
  // weight, repeating as each one fills. It terminates because every iteration
  // either spends the residual or fills at least one mainframe to its cap.
...
/** USD a mainframe has EARNED FOR ITS OWN HOLDERS: liberated = its whole
 ...
 * preview (/s6?preview=1) both go through it, so a preview render can
...
async function _getSeasonSnapshot(): Promise<Snapshot> {
  ...
  const topPilots = db ? await readTopPilots(db) : [];
...
const _cachedSnapshot = unstable_cache(_getSeasonSnapshot, ["s6-season-snapshot"], {
  revalidate: 60,
  tags: [S6_CACHE_TAG],
});
...
export const getTheme = unstable_cache(_getTheme, ["s6-theme"], {
  revalidate: 60,
  tags: [S6_CACHE_TAG],
});
 ...
 */
export function poolLine(snapshot: Snapshot, d: S6Dict = STRINGS.en): string {
