/**
 * S6 funnel tracking, client-safe beacon helper.
 *
 * Fire-and-forget: posts { session_id, event, ref, utm, path } to
 * /api/s6/events via navigator.sendBeacon (fetch keepalive fallback). The
 * anonymous session id is a uuid persisted in localStorage "s6_sid"; no wallet
...
const SID_KEY = "s6_sid";
const LANDING_DAY_KEY = "s6_lv_day";
const REF_KEY = "s6_ref"; // first-touch referral code, read at join (ADR-0068 §3)
const INVITE_DAY_KEY = "s6_ic_day"; // invite_clicked dedupe, mirrors landing_view
