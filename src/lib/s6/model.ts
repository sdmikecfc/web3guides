/**
 * S6 tank model resolution (the Uprising counterpart of s4/model.ts).
 *
 * PICK-FIRST (ADR-0066): a pilot's tank is their CHOICE from the real-tank
 * roster (lib/s6/tanks.ts), never derived from dollars held. `hq.tank` holds
 ...
 * Signal, sprints and bounties. Art convention (15 renders x 5 camo recolors):
 *
 *   /s6-art/tank/tank-<tankKey>-c{j}.png     j = camo index 1..5
 *
 ...
 * pilot. Ownership lives in hq.camos_owned (bot-swept, web-read);
 * /api/s6/hq rejects a swap to a camo the wallet has not earned.
 *
 * Cosmetics-only invariant (ADR-0067): these awards carry zero Signal, zero
...
export const CAMO_LADDER: ReadonlyArray<{ camo: CamoKey; from: "free" | "game" | "top"; game?: string }> = [
  ...
  { camo: "desert", from: "game", game: "ironjaw" },
  { camo: "winter", from: "game", game: "strain" },
  { camo: "night", from: "game", game: "stopclock" },
  { camo: "urban", from: "game", game: "riot" },
  { camo: "gold", from: "top" },
];
 ...
 * into hq.tanks_owned. Granted instantly by /api/s6/upgrade; the bot's daily
...
/** Extra pilots beyond the free 8-strong cast (today: bigmike only). */
export const PRIZE_COMMANDERS = ["bigmike"] as const;

/** Pilots this hq may field: the free cast plus any earned prize keys. */
export function ownedPilotKeys(hq: unknown): string[] {
  const src =
    hq && typeof hq === "object" && !Array.isArray(hq)
      ? (hq as Record<string, unknown>)
      : {};
  const extra: string[] = [];
  if (Array.isArray(src.pilots_owned)) {
    for (const k of src.pilots_owned) {
 ...
 * /s6-art/tank/<key>.webp. All 20 roster keys are covered since the is3 studio
 ...
 */
// All five schemes recoloured 2026-08-14 (scripts/s6-mech-recolour.py:
// the recovered S5 HSV remap + amber-protect). Files verified on disk.
...
export function tankArt(tankKey: string, camo: CamoKey): string {
  const base = HERO_TANK_ART[tankKey] ?? (TANK_ART_ON_DISK.has(tankKey) ? `/s6-art/tank/${tankKey}.webp` : "");
  if (base) return CAMO_RECOLOURED.has(camo) ? base.replace(/\.webp$/, `-${camo}.webp`) : base;
  const cj = CAMOS.indexOf(camo) + 1 || 1;
  return `/s6-art/tank/tank-${tankKey}-c${cj}.png`;
}
 ...
 *
 * The pilot is a studio cutout standing in a painted, graded camp, so she
 ...
 * behind him). Only 18 of 112 pilot/plate pairings were in a sane band.
 *
 * These pairs are SOLVED, not eyeballed: for each pilot, the saturate and
...
/** Falls back to a neutral pass so an unknown pilot is never mis-graded. */
export function pilotGrade(ck: string): { sat: number; bri: number } {
  return COMMANDER_GRADE[ck] || { sat: 1, bri: 1 };
}

export const CAMP_PLATES: Record<string, { plate: string; loop?: string }> = {
  // S6 ships ONE hangar scene for every mech (per-mech plates + the living
  // loop are a week-1 art beat); the S5 per-tank rows were dead lookups (S5
  // tank keys can never match the S6 mech roster) so they are gone.
  [HERO_TANK_KEY]: { plate: "/s6-art/hq/camp-portrait.webp" },
};
...
/**
 * Pilots with a landed idle-loop clip at /s6-art/pilot/anim/<key>.mp4
 * (plain h264 so it plays everywhere including iOS Safari). The S5-face
 * copies that shipped in the folder were never wired (showing last season's
 * cast beats nothing but confuses everyone); on 2026-08-17 all nine were
 * REPLACED with real S6 loops generated from the v5 portraits (seedance
 * fast, 4s, identity-locked - Mike approved the vega test), so the whole
 * cast idles, Big Mike included.
 */
...
/** The pilot key from hq.pilot (validated; defaults to the cast lead). */
export function resolvePilotKey(hq: unknown): string {
  const src =
    hq && typeof hq === "object" && !Array.isArray(hq)
      ? (hq as Record<string, unknown>)
      : {};
  return pilotByKey(src.pilot) ? String(src.pilot) : DEFAULT_COMMANDER_KEY;
}

/**
 * Which tank to draw for a pilot. `hq` is the player's hq JSONB value
