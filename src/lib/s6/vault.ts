/**
 * S6 ARMORY VAULT, the daily deal registry (#221, ADR-0067).
 *
 ...
 * and price always shown, Scrap only, cosmetics only. The rotation reuses the
 * existing decal registry (lib/s6/ftue.ts DECAL_LABELS): the registries are
 ...
 * purpose: every camo is already a free swap (/api/s6/hq), so there is
 * nothing to sell. first-colors is NOT here either: it is the FTUE chain's
 * earned moment and stays quest-only.
 *
 * The server route (/api/s6/vault-buy) computes today's deal from THIS list
...
export type VaultItem = {
  ...
  key: string;
  /** Server price in Scrap (play currency, never cash). */
  price: number;
};
 ...
 * price can never be charged today. Prices sit inside the existing Scrap
...
/**
 * A mainframe's War Effort decal key. CLIENT-SAFE MIRROR of the server-only
 * warEffortDecal (lib/s6/warEffort.ts, itself mirroring the bot): the
