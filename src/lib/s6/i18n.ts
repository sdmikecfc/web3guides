/**
 * Launch Wars S6, i18n runtime (mirrors src/lib/s4/i18n.ts).
 *
 * Cookie + dictionary, no routing. A server component calls getLocale(), which
 * reads the `s6_lang` cookie (Next 14 cookies() is synchronous), validates it,
...
import { STRINGS, type S6Dict } from "./strings";
...
export function dict(locale: Locale): S6Dict {
