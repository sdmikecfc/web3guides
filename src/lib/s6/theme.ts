/**
 * Launch Wars Season 5 (UPRISING) THEME: the ONE place every player-visible
 * word lives. Mirrors src/lib/s4/theme.ts exactly (the proven one-seam rule):
 * no themed word may appear anywhere outside this object; the s6_theme config
 * row in launch_wars_boss_config may override any of it with zero logic change.
 *
 * S6 twist: ONE team. Everyone fights for The Iron Column; the season is the
 * players against the mainframes, not team against team. The team KEY "front"
...
export type Theme = {
  seasonName: string;
  target: WordPair; // mainframe
  team: WordPair; // column
  player: WordPair; // pilot
  points: string; // Signal
  playCurrency: string; // Scrap (cosmetics/arcade only, never prize weight)
...
export const DEFAULT_THEME: Theme = {
  ...
  // Rewritten 2026-08-16: one pot, no bounty envelope, and a mainframe's slice
  ...
  pitch:
    "Buy and hold any listed mainframe, from $5. Every dollar you hold earns Signal every day. Every mainframe is worth a slice of the season pool, and at season end it pays that slice to its own holders in proportion to how far it got: all of it once the mainframe is LIBERATED, 60 percent of it if the wall reached 60 percent, and nothing it never earned.",
};
...
/**
 * Merge a parsed `s6_theme` JSON override onto DEFAULT_THEME. Tolerant of
