 ...
 * watch the loop at all. `?demo=1` gives it to you: a funded pilot, the
 ...
 * crosses to the server, no leaderboard moves, no Scrap are minted. The banner
 * (see DemoBadge) is there so nobody is confused about which they are looking
 * at, not because the state is dangerous.
 */
// TYPE-ONLY import from the S5 tree (erased at compile time, zero runtime
// coupling): the s6 app tree does not exist yet. Re-point at
// "@/app/s6/hq/panels" when the day-3 surfaces port lands.
import type { HqMe } from "@/app/s6/hq/panels";
import { resolveTank, STAT_MILESTONE_TANKS } from "@/lib/s6/model";
import { statNextPrice, type StatKey } from "@/lib/s6/games";
import { TANK_ROSTER, TANK_TIER_PRICES } from "@/lib/s6/tanks";
...
/**
 * A pilot who can afford EVERYTHING.
 *
 * This started at 420 Scrap so the can't-afford path stayed reachable, which
...
export function demoHqMe(): HqMe {
  return {
    session: true,
    // The demo pilot has no public garage to link to, so the "see your own
    ...
    // FIELD names track the S5 HqMe alias above until the s6 app tree lands
    // (day 3): flip commander/ownedPilots to pilot/ownedPilots with it.
    pilot: "diesel",
...
export function demoReply(url: string, body: unknown): Record<string, unknown> {
  ...
  if (url.includes("/tank-unlock")) {
    ...
    if (price > wallet.shells) {
      return { error: `Not enough Scrap. ${price} needed, ${wallet.shells} banked.` };
    }
    wallet.shells -= price;
  ...
  if (url.includes("/vault-buy")) {
    ...
    if (deal.price > wallet.shells) {
      return { error: `Not enough Scrap. ${deal.price} needed, ${wallet.shells} banked.` };
    }
  ...
  // /api/s6/upgrade: the stat shop, simulated with the REAL price curve and
  ...
  // /api/s6/hq: cosmetic sets (tank, pilot, camo) are free. Mutate the
  ...
  return {
    ok: true,
    // panels sends `set: { pilot }`; checking `set.commander` made pilot
    // picking a silent no-op under ?demo=1 (fixed 2026-08-15)
    ...(typeof set.pilot === "string" ? { pilot: set.pilot } : {}),
