/**
 * S5 ARMORY VAULT, the daily deal registry (#221, ADR-0067).
 *
 ...
 * and price always shown, Shells only, cosmetics only. The rotation reuses the
 * existing decal registry (lib/s5/ftue.ts DECAL_LABELS): the registries are
 ...
 * purpose: every camo is already a free swap (/api/s5/hq), so there is
 * nothing to sell. first-colors is NOT here either: it is the FTUE chain's
 * earned moment and stays quest-only.
 *
 * The server route (/api/s5/vault-buy) computes today's deal from THIS list
...
export type VaultItem = {
  ...
  key: string;
  /** Server price in Shells (play currency, never cash). */
  price: number;
};
 ...
 * price can never be charged today. Prices sit inside the existing Shells
...
/**
 * A stronghold's War Effort decal key. CLIENT-SAFE MIRROR of the server-only
 * warEffortDecal (lib/s5/warEffort.ts, itself mirroring the bot): the
