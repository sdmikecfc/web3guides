/**
 * Season 5, the session -> player helpers shared by the authed s5 API routes
 * (/api/s5/me, /api/s5/hq, /api/s5/tank-unlock, /api/s5/funding-status,
 * /api/s5/claim-ftue). Server-only.
 *
 * The hq JSONB is SHARED with the bot: the web owns hq.tank / hq.camo /
 * hq.commander / hq.tanks_owned, the bot owns hq.crates / hq.streak /
 ...
 */
import "server-only";
import type { s5Db } from "./server";
import { ownedCamoKeys, ownedCommanderKeys, ownedTankKeys, resolveCommanderKey } from "./model";

type Db = ReturnType<typeof s5Db>;

/**
 * Resolve the wallet behind a play-session token (the token /api/s5/game-session
...
export type HqView = {
  ...
  ownedCamos: string[];
  /** Earned prize commanders (hq.commanders_owned; today bigmike or nothing). */
...
export function buildHqView(hqRaw: unknown): HqView {
  ...
  return {
    commander: resolveCommanderKey(hq),
    ownedTanks: ownedTankKeys(hq),
    ownedCamos: ownedCamoKeys(hq),
    ownedCommanders: ownedCommanderKeys(hq),
    bondsTier,
    streakDays,
    crates,
    peakUsd,
