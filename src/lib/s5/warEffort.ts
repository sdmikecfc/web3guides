/**
 * THE WAR EFFORT (web side), Launch Wars S5.
 *
 * A commander COMMITS Shells (the play currency) to ONE stronghold as declared
 * war effort. Breach = the commitment returns DOUBLED plus that stronghold's
 * cosmetic decal. Fail = the Shells are gone. The window CLOSES once a
 * stronghold enters its sprint band (the 85% tripwire), so nobody can back a
 ...
 * touches Medals, the pool, or any USD figure: settlement weight stays
 * Medals x breadth and no cash cut can move because of this file.
 *
...
export const WAR_EFFORT_TABLE = "launch_wars_s5_war_effort";
 ...
 * so committing Shells to it was close to risk free and the "bet" was a
...
/** Public, per-stronghold aggregate. No wallets, no dollars. */
export type WarEffortTotal = {
  domain: string;
  /** Shells riding on this wall right now (open commitments only). */
  committed: number;
  /** How many commanders have declared for it. */
  commanders: number;
...
/** A stronghold's own decal key (mirrors modules/season5 warEffortDecal). */
...
/**
 * Is the commitment window open on this stronghold? The web copy of the bot's
...
export async function warEffortTotals(db?: Db): Promise<Map<string, WarEffortTotal>> {
  ...
  try {
    ...
    for (const r of data || []) {
      ...
      const agg = out.get(domain) || { domain, committed: 0, commanders: 0, won: 0, lost: 0 };
