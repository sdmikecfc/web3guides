/**
 * SEASON 5 (IRON SIEGE) — THE WORLD DESCRIPTOR.
 *
 ...
 * strongholds or Medals. S6 = copy this file, move the coordinates, rename the
 ...
 *   UPPER TWO     THE FRONT. Eleven strongholds in a serpentine advance, in
 *   THIRDS        LISTING ORDER (first listing bottom-left, last top-left), so
 *                 the map reads as a campaign moving up the board.
 *   RIGHT EDGE    The muster field (other commanders) and the Power Station,
 *                 fenced off and cold.
 *
 ...
 * English on purpose: STRINGS in lib/s5/strings.ts is typed `S5Dict = typeof
...
const S5_WORLD_AUTHORED: WorldConfig = {
  ...
  /**
   * A painted PLACE maps to whichever game currently sits on it. The S5 slate
   ...
   * strongholds stand in the scorched west - t0-t4 the near rank along the
   ...
   *   2. the front road following the line of the strongholds
  ...
  sites: [
    ...
    //    popups off the base instead. ────────────────────────────────────────────
    {
      ...
      kind: "home",
      label: "Your Base",
      blurb: "Your tank, your commander, your kit.",
      pos: { x: 0.815, y: 0.76 },
    ...
    // ── THE RIGHT EDGE ───────────────────────────────────────────────────────
    {
      ...
      glyph: "signpost",
      accent: TAN,
      // WAS a route to /s5/explore, a second whole page for looking at other
      // commanders. They stand on THIS map now, so that page was showing an
 ...
 */
export const S5_TARGET_SCALE = [1.05, 1.3, 1.04, 1.0, 1.08, 1.22, 1.16, 1.5, 1.06, 1.06];
...
export const S5_CAMPS = FIT_CAMPS;

export const S5_GAME_FALLBACK = {
  label: "Proving Ground",
  blurb: "Field exercises. Small Medals, big bragging rights.",
  href: "/s5/play",
};
 ...
 */
export const S5_WORLD_PATHS = FIT_PATHS.length ? FIT_PATHS : FIT_ROADS;

export const S5_WORLD_PADS = FIT_PADS;
export const S5_WORLD_TRACKS = FIT_TRACKS;

export const S5_WORLD: WorldConfig = {
  ...S5_WORLD_AUTHORED,
  ...
  sites: S5_WORLD_AUTHORED.sites.map(
    (s): WorldSite => {
      const pos = FIT_SITES[s.key] || FIT_SITES[FIT_KEY_ALIAS[s.key]];
      return pos ? { ...s, pos } : s;
    },
  ),
  targetSlots: FIT_SLOTS.length ? FIT_SLOTS : S5_WORLD_AUTHORED.targetSlots,
};
