/**
 * S5 funding wizard constants (client-safe: shared by the HQ wizard UI and the
 * /api/s5/funding-status route). Chain facts for Doma plus the two funding
...
/** Deep link to buy one stronghold on the Doma app. */
...
/** Enough USDC.e to enter a stronghold: $5 (in 6-decimal units). */
...
export type FundingState =
  ...
  | "FUNDED_NOT_BOUGHT" // armed: USDC.e + gas, no stronghold token yet
  | "HOLDER" // holds at least one stronghold token
...
export type FundingStatus = {
  ...
  usdcUsd: number;
  /** Stronghold tokens with a balance > 0 (raw token units as strings;
