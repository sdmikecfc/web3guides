/**
 * Launch Wars S5, i18n runtime (mirrors src/lib/s4/i18n.ts).
 *
 * Cookie + dictionary, no routing. A server component calls getLocale(), which
 * reads the `s5_lang` cookie (Next 14 cookies() is synchronous), validates it,
...
import { STRINGS, type S5Dict } from "./strings";
...
export function dict(locale: Locale): S5Dict {
