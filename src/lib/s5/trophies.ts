/**
 * S5 TROPHY SHELF, server-only: relics a wallet earned in PAST seasons.
 *
 ...
 *   S3  launch_wars_s3_pilots by wallet: the Starfall hull. ship.hull (a
 *       1..12 hull number when the shop set one) falls back to the pilot's
...
export async function getTrophies(wallet: string): Promise<Trophies> {
  ...
  const [s4r, s3r] = await Promise.all([s4Trophy(db, w), s3Trophy(db, w)]);
