import "server-only";
/**
 * THE ROSTER — every commander in the season, readable by anyone.
 *
 * PUBLIC-SAFE BY CONSTRUCTION, and this is the load-bearing rule of the file,
 * copied deliberately from lib/s5/publicHq.ts: no row that leaves here may
 ...
 *
 * WHY ITS OWN FILE rather than an extension of lib/s5/data.ts: getSeasonSnapshot
 * is one cached blob that EVERY s5 page awaits, including the landing. Folding a
 * 500-row roster read into it would make the camp pay for the roster. This
 * follows the lib/s5/warEffort.ts precedent instead — a sibling server-only
...
/** One commander, safe for any public surface. */
export type RosterRow = {
  /** Position in the Medals ordering, 1-based. */
  rank: number;
  name: string;
  /** Public garage handle for /s5/hq/<handle>. May be "" when the wallet is
  ...
  commanderKey: string;
  stats: RosterStats;
  /** Which stronghold they camp at on the explore map, or null to muster. */
  domain: string | null;
};
...
/** 0..5 from a raw skin value. Mirrors statsOf in /api/s5/me. */
 ...
 * to "everyone musters in the staging area". Mirrors readTopCommanders. */
type Db = NonNullable<ReturnType<typeof s5Db>>;
...
async function readRoster(cursorRaw: string | null, pageSize: number): Promise<RosterPage> {
  ...
  try {
    // KEYSET, never OFFSET. Medals move continuously (every banked run calls
    // s5_grant), so an OFFSET page 2 taken a second after page 1 duplicates or
    // drops rows. The (points, id) tiebreak is not optional either: everyone
    // starts on 0 Medals, so ties are the COMMON case in week one and without
...
/**
 * The commanders DRAWN on the explore map.
 *
