/**
 * Chroma-key the magenta plate out of the raw Season 4 (s4-art) renders.
 * Sibling of key-stars-art.js (which targets stars-art); identical logic, additive.
 *
 *   node key-s4-art.js                  # key every public/s4-art/_raw/*.png
 *   node key-s4-art.js cast-trustfund   # key one (basename, no .png)
 *
 * Reads public/s4-art/_raw/<name>.png (gpt-image-2 render on flat #FF00FF),
 * writes a transparent public/s4-art/<name>.png. Distance-based key + edge
 * feather; despill runs ONLY on feathered edge pixels so saturated subject
 * colors (pink bikini, gold jewellery) are never touched.
 */
const fs = require("fs");
const path = require("path");
const { PNG } = require("pngjs");

const ART = path.resolve(__dirname, "public", "s4-art");
