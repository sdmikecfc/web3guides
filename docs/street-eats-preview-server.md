...
The public entry is a connection-only open beta. Anyone with a connected wallet can choose **Play beta**; no signature or transaction is requested. The entry and settings explain that **all beta data and progress will be wiped for the official launch**. This does not require SQL, a Supabase Web3 provider, or `DINER_PREVIEW_SERVER_ENABLED` to be enabled.

Beta progress and preferences are browser-local, under `domain_kitchen_beta_v1:<lowercase-wallet>:save` and `:preferences`. Reconnect the same wallet in the same browser to continue. Clearing browser data removes that local progress; another device does not receive it. Disconnecting or switching wallets unmounts play and pauses/checkpoints the old service. Friend visits, trades, cloud saves and token rewards are unavailable in this mode; postcard export remains available. Existing guest saves (`street_eats_preview_v1`) and verified accounts are neither imported nor overwritten. The official launch must use its own save namespace and must not import these disposable beta saves.

The separate verified-account implementation remains under `/api/chef/diner`, with database tables beginning `diner_preview_`. Its signature/session checks have not been relaxed; a connected address is never accepted as API authentication. The public beta does not call these account endpoints. The instructions below document that separate online-account implementation for future integration.
...
Public beta play uses `BetaEntry` and never waits for the online account service. `WalletEntry` retains the separate signature-required entry for future online accounts; it is not the public page. Its development sandbox remains restricted to development builds on localhost/loopback. Build/deploy a clean checkout of the intended commit: a Vercel CLI deployment from a dirty workspace can include unrelated uncommitted files. Only the owner releases production with `vercel --prod`.
...
If the public homepage still says **“Wallet accounts are being prepared”**, it is running the earlier account-gated entry. Release the updated beta entry with the owner's `vercel --prod`. An `enabled=false` response from `/api/chef/diner/status` is expected when the separate account service is off and does not block the open beta. For future verified accounts, applying SQL alone does not enable that service flag; all three migrations and the provider/environment setup below are needed.
