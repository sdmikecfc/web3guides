...
2. Enable the project's Ethereum Web3 sign-in provider and keep email/anonymous creation out of the diner flow. Allow the exact sign-in URI `https://domainkitchen.xyz/chef/diner-preview` and, if used, `https://www.domainkitchen.xyz/chef/diner-preview`. Give staging its own exact HTTPS origin and URI; do not broadly allow arbitrary preview domains. Confirm the provider's current URL matching behavior against its [official setup instructions](https://supabase.com/docs/guides/auth/auth-web3).
