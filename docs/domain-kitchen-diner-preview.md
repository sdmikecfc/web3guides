...
Fries use one potato portion cut at the prep counter, followed by an unattended fryer basket. When it is ready, the player explicitly raises the basket out of the oil; the tier-three fryer lifts it automatically. The raised batch supplies **three boxed servings**; each interaction consumes one saved portion and the third empties the basket. An unraised lower-tier basket can burn. Batch state and portion counts persist through reloads and cannot be refilled by repeating the raise action.
...
| --- | --- |
| Opening trip | A fresh diner starts with classic burger only, one table and one chair. Untimed setup begins with portable machines in a separate trailer. The first customer teaches the meal and washing loop before another arrives. New trips have no forced defeat or automatic fryer/recipe grant. The first visited market offers equipment/upgrades; recipes enter random stock from the second market. Purchases stay off the menu until explicitly selected. Older active trips retain their promised rewards. |
| Manual cooking | One held item. Prep ahead runs real cooking before customer admission. Cold patties and vegetables come from the fridge; buns, noodles and dry ingredients from the pantry. Burgers require assembly and reusable plates; noodle dishes require boiling, draining, assembly and reusable bowls. Drinks use cups; fries need cutting, frying, a basket lift and one box per portion. Tier-three fryers lift automatically; tier-three sinks wash loaded vessels automatically. Keyboard WASD/arrows move; E interacts and holds manual work. |
| Outdoor service | Starter interior is 7×3 with a connected ramp and 10×4 pavement. Burger and fries equipment, cold/dry storage and both serving supplies fit inside around a clear aisle; bins may also stand outside. Singles use independent seats; families of three or four reserve a complete four-seat table. New guests can sit before the previous vessel is cleared. Pick up that old dish before serving their new order; clearing and later washing preserve the current guest and their meal receipt. |
...
| Truck growth | Tiers are 7×3, 8×3, 9×3 and 10×4, with larger pavements, menus of 2/3/4/4 recipes and 0/1/1/2 helpers. Purchased tables must fit with reachable chairs; growth grants no free seating. Route finales open larger trucks without requiring every recipe. Setup changes, station working sides, table rotations, menu and cosmetics persist. Existing layouts and larger saved menus remain usable. Cup, plate, bowl and sink capacity upgrades retain their real stock/slot limits. |
...
| Home progression | Dishes have stable ownership and levels 0–10. First production-machine discovery grants a truck unlock and one restaurant copy, independently of its recipes. Utilities remain truck items; additional restaurant copies cost banked coins. New pasta/ramen recipes automatically enter home service when compatible reachable machines are installed; existing dishes retain their home menu rules. Old possessions and kit receipts remain owned. |
