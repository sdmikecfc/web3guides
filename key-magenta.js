/**
 * Chroma-key the magenta plate out of the raw seas-art ship/emblem renders.
 *
 *   node key-magenta.js                 # key every public/seas-art/_raw/*.png
 *   node key-magenta.js ship-sovereign  # key one (basename, no .png)
 *
 * Reads public/seas-art/_raw/<name>.png (gpt-image-2 render on flat #FF00FF),
 * writes a transparent public/seas-art/<name>.png. Distance-based key with an
 * edge feather; despill runs ONLY on feathered edge pixels so saturated subject
 * colors (red ensigns, etc.) are never touched.
 */
const fs = require("fs");
...
function keyOne(name) {
  ...
  for (let i = 0; i < d.length; i += 4) {
    ...
    if (dist2 <= T_IN) {
      ...
      // edge feather: alpha ramps 0..255 across the key boundary
      const t = (dist2 - T_IN) / (T_OUT - T_IN);
      d[i + 3] = Math.round(255 * t);
      // despill only here: pull magenta tint out of the fringe
      const m = (r + b) / 2;
