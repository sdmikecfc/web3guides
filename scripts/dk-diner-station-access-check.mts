...
test('all facings can use a reachable side when the illustrated front is blocked',()=>{
 for(const facing of [0,1,2,3] as const){const loadout=buildServiceLoadout(4,['classic_burger']);const grill=loadout.stations.find(st=>st.kind==='grill')!;grill.x=3;grill.y=1;grill.facing=facing;const front=stationWorkingCell(grill);const blocker=makeStation('front-blocker','bin',front.x,front.y);const stations=loadout.stations.filter(st=>!['bin','fridge','plates'].includes(st.kind)&&!(st.x===blocker.x&&st.y===blocker.y));stations.push(blocker);
...
test('fresh seven-by-three fits the burger kit inside with an outdoor bin, without moving old saves',()=>{
...
test('outdoor supplies keep queue, ramp and hot appliances bounded by the actual floor',()=>{
 ...
 for(const point of [{x:-1,y:4},{x:1,y:3},{x:1,y:2},{x:9,y:4}]){const moved=s.stations.map(st=>st.kind==='bin'?{...st,...point}:st);assert(validateServiceLayout(1,moved,s.tables),JSON.stringify(point));}
