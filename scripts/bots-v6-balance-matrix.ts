...
import {createFightV6,incomingThreatV6,acceptSpecialV6,controlledV6,resultV6,replayV6,stepFightV6,FAMILIES_V6,MAX_FRAMES_V6,RULES_V6,WEAPON_KINDS_V6,cardV6,cloneV6,collisionManifestV6,presetV6,runFightV6,snapshotBuildV6,type BuildV6,type DefensePlanV6,type StyleV6,type TierV6,type WeaponKindV6} from "../src/lib/bots/v6";
...
for(const tier of [1,2,3,4] as const){cohorts.push({kind:"mirror",tier,count:16,id:`mirror-t${tier}`},{kind:"manual",tier,count:12,id:`manual-t${tier}`},{kind:"role",tier,count:referenceSeeds,id:`role-t${tier}`},{kind:"hybrid",tier,count:8,id:`hybrid-t${tier}`});if(tier>=2)cohorts.push({kind:"arsenal",tier,count:20,id:`arsenal-t${tier}`});if(tier>=3)cohorts.push({kind:"signature",tier,count:16,id:`signature-t${tier}`},{kind:"splash",tier,count:8,id:`splash-t${tier}`});}
const selected=process.env.MK_V6_BENCH_COHORT??"all",splitFilter=process.env.MK_V6_BENCH_SPLIT??"both",sampleOverride=process.env.MK_V6_BENCH_SEEDS?Number(process.env.MK_V6_BENCH_SEEDS):null;
assert(selected==="all"||cohorts.some(c=>c.id===selected));assert(["train","heldout","both"].includes(splitFilter));if(sampleOverride!==null)assert(Number.isInteger(sampleOverride)&&sampleOverride>=2&&sampleOverride<=64);
...
function build(style:StyleV6,cohort:Cohort,index:number,side:number,familyIndex?:number):BuildV6 {
 ...
 if(kind==="arsenal"){
  const available=WEAPON_KINDS_V6.filter(w=>cardV6(`mk6.t${tier}.weapon.${w}`));const weapon=available[(index+side*3)%available.length];return presetV6(style,tier,{family:chosenFamily,weapon,collisionVersion:version});
 }
...
for(const split of ["train","heldout"] as const){if(splitFilter!=="both"&&splitFilter!==split)continue;const bank=styles.map(()=>({wins:0,games:0}));let bankSideWins=0,bankGames=0,referenceTiers=0;
 for(const cohort of cohorts){if(selected!=="all"&&selected!==cohort.id)continue;const count=sampleOverride??cohort.count,totals=styles.map(()=>({wins:0,games:0})),reference=cohort.kind==="role",familyCombinations=reference?4:1;let cohortSideWins=0,cohortGames=0;
  for(let a=0;a<3;a++)for(let b=cohort.kind==="mirror"?a:a+1;b<(cohort.kind==="mirror"?a+1:3);b++){if(pairFilter&&`${styles[a]}/${styles[b]}`!==pairFilter)continue;let wins=0,sideWins=0,timeouts=0,disabled=0,shots=0,duration=0;const subgroups:Record<string,{wins:number;matches:number;shots:number}>={};for(let i=0;i<count;i++)for(let combination=0;combination<familyCombinations;combination++){
   const seed=((split==="train"?0x19e52103:0x6387af31)+(i+1)*(split==="train"?104729:1299709)+cohort.tier*8191)>>>0,A=build(styles[a],cohort,i,0,reference?Math.floor(combination/2):undefined),B=build(styles[b],cohort,i,1,reference?combination%2:undefined);assert.equal(A.gp,B.gp);
   const planA=plans[i%3],planB=cohort.kind==="mirror"?planA:plans[Math.floor(i/3)%3],subgroup=`${A.parts.torso.family}/${A.capabilities.weapon}/${planA} vs ${B.parts.torso.family}/${B.capabilities.weapon}/${planB}`,sub=subgroups[subgroup]??={wins:0,matches:0,shots:0};for(const swap of [0,1]){const actualPlans:[DefensePlanV6,DefensePlanV6]=swap?[planB,planA]:[planA,planB],r=cohort.kind==="manual"?manualMatch(seed,swap?B:A,swap?A:B,actualPlans,((i%2+swap)%2) as 0|1,i===0&&swap===0):runFightV6(seed,swap?B:A,swap?A:B,{autoSpecial:[true,true],defensePlans:actualPlans});const first=r.winner===swap,shotCount=r.events.filter(e=>e.kind==="shot").length;wins+=Number(first);sideWins+=Number(r.winner===0);totals[first?a:b].wins++;totals[a].games++;totals[b].games++;timeouts+=Number(r.frames===MAX_FRAMES_V6);disabled+=Number(r.events.at(-1)?.defeatReason==="disabled");shots+=shotCount;duration+=r.frames;sub.wins+=Number(first);sub.matches++;sub.shots+=shotCount;games++;}
  }
  const matches=count*2*familyCombinations,rate=wins/matches,label=`${split} ${cohort.id} ${styles[a]}/${styles[b]}`,row={split,cohort:cohort.id,pair:`${styles[a]}/${styles[b]}`,reference,independentSeeds:count,familyCombinations,matches,firstStyleWinRate:rate,firstSideWinRate:sideWins/matches,timeouts,disabled,shots,meanSeconds:duration/(matches*60),subgroups};records.push(row);console.log(JSON.stringify(row));if(reference&&(rate<.3||rate>.7))failures.push(`${label} ${(rate*100).toFixed(2)}% outside 30–70%`);if(timeouts)failures.push(`${label}: ${timeouts} timeouts`);cohortSideWins+=sideWins;cohortGames+=matches;allTimeouts+=timeouts;
  }
...
const complete=selected==="all"&&splitFilter==="both"&&sampleOverride===null&&pairFilter===null;
