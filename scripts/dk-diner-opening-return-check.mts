...
import {createDiner,dispatchDiner,generateDinerMap,sanitizeDinerSave,type DinerCommand,type DinerState} from '../src/lib/chef/diner/progression';
...
check('an untouched opening route can return without earning its gift or skipping onboarding',()=>{
 let state=started();const bank=state.coins;state=act(state,{type:'goHome'});assert.equal(state.run,null);assert.equal(state.coins,bank);assert.equal(state.tutorial.finished,false);assert.equal(state.tutorial.fryerGifted,false);assert.equal(state.equipment.fryer.homeCopies,0);assert.equal(act(state,{type:'startRun'}).run!.tutorial,true);
});
...
check('active services and stops still require their normal exit, without losing progress',()=>{
 for(const kind of ['slow','shop','bonus','event'] as const){let state=started();const node=state.run!.map.find(n=>n.kind===kind);if(!node)continue;state.run!.available=[node.id];state=act(state,{type:'chooseNode',nodeId:node.id});const result=dispatchDiner(state,{type:'goHome'},{now});assert.equal(result.code,'between_stops');assert.deepEqual(result.state,state);}
});
