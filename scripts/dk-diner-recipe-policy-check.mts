...
check('fresh ownership and both menu defaults contain one burger; fryer waits for discovery',()=>{
  ...
  assert.equal(state.equipment.fryer.truckOwned,false);assert.equal(state.equipment.fryer.homeCopies,0);assert(!truckStorage(state).some(item=>item.equipmentId==='fryer'));assert.match(truckRecipeShop(state).find(offer=>offer.recipeId==='fries')!.reason!,/fryer/);
...
check('a purchased recipe is optional and menu selection preserves every placed piece',()=>{
  ...
  state=act(state,{type:'buyTruckRecipe',recipeId:'cheeseburger'});assert.equal(state.coins,before-DINER_RULES.recipeCost);assert.equal(state.recipes.cheeseburger.level,0);assert.deepEqual(state.truckConfig.menu,['classic_burger']);assert.deepEqual(state.home.menu,menu);assert.deepEqual(state.truckConfig.stations,layout);
...
check('setup purchases check route, owned machines, price and known fields atomically',()=>{
  ...
  const bought=act(state,{type:'buyTruckRecipe',recipeId:'cheeseburger'});reject(bought,{type:'buyTruckRecipe',recipeId:'cheeseburger'},'recipe_unavailable');
...
check('tutorial discovery grants fryer ownership and one home copy but no unchosen fries recipe',()=>{
  ...
  assert.equal(state.equipment.fryer.truckOwned,true);assert.equal(state.equipment.fryer.homeCopies,1);assert(state.lastRun!.equipment.includes('fryer'));assert.equal(state.recipes.fries,undefined);assert.deepEqual(state.truckConfig.menu,['classic_burger']);
  const coins=state.coins;state=act(state,{type:'buyTruckRecipe',recipeId:'fries'});assert.equal(state.coins,coins-DINER_RULES.recipeCost);assert.equal(state.recipes.fries.level,0);assert.deepEqual(state.truckConfig.menu,['classic_burger']);assert.deepEqual(state.home.menu.starter,[]);
  state=act(state,{type:'startRun'});state=act(state,{type:'goHome'});assert.equal(state.equipment.fryer.homeCopies,1);
});
